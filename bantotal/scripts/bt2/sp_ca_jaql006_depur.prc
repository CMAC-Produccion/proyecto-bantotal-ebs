CREATE OR REPLACE PROCEDURE SP_CA_JAQL006_DEPUR(P_FECHA DATE) IS
  ...
  cursor c_part is
   ...
   WHERE table_owner='BANTPROD' and table_name='JAQL006';
  CURSOR C_IDX IS
    SELECT INDEX_NAME INDICE
      FROM DBA_INDEXES
     WHERE TABLE_NAME = 'JAQL006'
       AND TABLE_OWNER = 'BANTPROD';
  CURSOR C_PARTIDX IS
    SELECT partition_name,partition_position
      FROM dba_tab_partitions
   WHERE table_owner='BANTPROD' and table_name='JAQL006' and partition_position<5 order by 2;
begin
  ...
  IF(C_PARTITION_NAME IS NOT NULL) THEN
    --1RA ITERACIÓN
    UPDATE JAQL006 A
       SET A.JAQL6COTRA = '999999'
     WHERE A.JAQL6FETRA < P_FECHA;
    COMMIT;
    --2DA ITERACIÓN
    UPDATE JAQL006 A
       SET A.JAQL6COTRA = '888888'
     WHERE A.JAQL6FETRA < P_FECHA;
    COMMIT;
    --3RA ITERACIÓN
    UPDATE JAQL006 A
       SET A.JAQL6COTRA = '777777'
     WHERE A.JAQL6FETRA < P_FECHA;
    COMMIT;
    --DEPURACIÓN
    execute immediate 'ALTER TABLE JAQL006 DROP PARTITION '||c_partition_name;
    COMMIT;
    ...
    END LOOP;
    --LOG
    INSERT INTO AQPB884 VALUES (USER, SYSDATE, 'SP_CA_JAQL006_DEPUR');
    COMMIT;
  end if;
END SP_CA_JAQL006_DEPUR;
/
