CREATE OR REPLACE PROCEDURE SP_AH_DEPO_CV(VPGCOD  number,
                                          VSCSUC  number,
                                          VSCMOD  number,
                                          VSCMDA  number,
                                          VSCPAP  number,
  ...
  begin
    ...
    select hsucor, hfvc, hcmod, htran, hnrel, Hcimp1
      ...
      from (select /*+NO_MERGE(e)*/
             e.hsucor, g.hfvc, e.hcmod, e.htran, e.hnrel, e.hcimp1
              from (select h16.*
                      from fsh016 h16
                     where h16.pgcod = 1
                       and h16.hfvco between VFECINI and VFECFIN) e,
                   fsh015 g,
                   fst198 h
            ...
            --and Hcodmo = 2
             order by g.hfvc, g.hhora, e.Hcimp1 desc) V 
     where rownum = 1;
  exception
