create or replace package PQ_CR_RATIODESEMBOLSO is

  -- Author  : MPOSTIGOC
  -- Created : 17/12/2019 1:04:35 p. m.
  -- Purpose : RTE DE RATIO CUOTA RESULTADO RESULTADO NETO Y EXCEDENTE MENSUAL EN DESEMBOLSO
...
end PQ_CR_RATIODESEMBOLSO;
/

create or replace package body PQ_CR_RATIODESEMBOLSO is
  ...
  procedure sp_cr_Inicio(ln_pgcodt in number,
    ...
    pn_top           number;
    ln_Instancia     number;
    ln_cuotprop      number(17, 2) := 0.00;
    ln_cuotaprob     number(17, 2) := 0.00;
    ln_pgcod         number;
    ln_suc           number;
    ln_mod           number;
    ln_mda           number;
    ...
    ln_periodo       number;
    ln_permensual    number;
    ln_NewCuota      number(17, 2) := 0.00;
    ...
    if lc_SegmntoActual = 1 then
      ...
      if ln_cuotprop = ln_cuotaprob then
        ...
        if ln_cuotprop <> ln_cuotaprob then
          ...
          begin
            insert into jaqz832
              ...
              select J.JAQY140CORR,
                     ...
                     '1',
                     J.JAQY140EST,
                     to_char(sysdate, 'HH24:MI:SS'), --to_char(sysdate, 'HH24:MI:SS'),
                     J.JAQY140USER
                from jaqy140 j
               where j.jaqy140inst = ln_Instancia
                 and j.jaqy140est = 'H';
          ...
          begin
            ln_permensual := ln_periodo / 30;
          end;
        
          if ln_permensual >= 1 then
          ...
          begin
            update jaqz832 j
               set j.jaqz832ratio =
                   (j.jaqz832capcaja + j.jaqz832sldext + j.jaqz832ccontg +
                   j.jaqz832cpoten) /
                   (j.jaqz832resnet + j.jaqz832sldext + j.jaqz832cpoten)
             where j.jaqz832inst = ln_Instancia
               and j.jaqz832est = 'H';
