create or replace package PQ_CR_REPORTE_EXPERIAN is
    -- *****************************************************************
    -- Nombre                     : paquete para EXPERIAN
    -- Sistema                    : BANTOTAL
...
------------------------------------------------------------------------------
procedure SP_CR_INSERT_JAQY772(
                                   v_Inst   in number,
                                   v_Fecha  in date,
                                   v_NuOpe  in number                        
                              ) ;  
...
procedure SP_CR_Calif_SBS(v_Cta in number, v_Calif out number, v_Desc out varchar2);
procedure sp_cr_Cal_SBS_Titular(v_Tdoc in number, v_Ndoc in varchar2, d_Date in date, v_Desc1 out varchar2);
procedure sp_cr_Cal_SBS_Conyugue(v_Tdoc in number, v_Ndoc in varchar2, v_Desc2 out varchar2);
-----------------------------------------------------------------------             
end PQ_CR_REPORTE_EXPERIAN;
/

create or replace package body PQ_CR_REPORTE_EXPERIAN is
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
---------------------------------------------------------------------------------------------
procedure SP_CR_INSERT_JAQY772(
                                  v_Inst   in number,
       ...
       Begin
        ...
        If ln_772inst = 0 then
           Begin
             ...
             lc_Fecha := TO_DATE (TO_CHAR (ld_Time, 'YYYY-MON-DD HH24:MI:SS'),'YYYY-MON-DD HH24:MI:SS');
             ...
             If lc_dia = 'PM' then
                ...
                If lc_Hra3 = '01' then lc_Hra4 := '13'; 
                Else
                   If lc_Hra3 = '02' then lc_Hra4 := '14'; 
                   Else
                      If lc_Hra3 = '03' then lc_Hra4 := '15';
                      Else
                         If lc_Hra3 = '04' then lc_Hra4 := '16';
                         Else
                            If lc_Hra3 = '05' then lc_Hra4 := '17';
                            Else
                               If lc_Hra3 = '06' then lc_Hra4 := '18'; 
                               Else
                                  If lc_Hra3 = '07' then lc_Hra4 := '19';
                                  Else
                                     If lc_Hra3 = '08' then lc_Hra4 := '20';
                                     Else
                                        If lc_Hra3 = '09' then lc_Hra4 := '21';
                                        Else
                                           If lc_Hra3 = '10' then lc_Hra4 := '22';
                                            Else
                                               If lc_Hra3 = '11' then lc_Hra4 := '23';
                                               Else
                                                 If lc_Hra3 = '12' then lc_Hra4 := '24';
                                                 End If;
                                               End If;
                                             End If;
                                          End If;
             ...
             select max(jaqy728hor)
              into lc_Hra2
              from jaqy728 
             where jaqy728fec  = v_Fecha 
               and jaqy728inst = v_Inst
               and Substr(jaqy728hor,1,5) <= Substr(lc_HraF,1,5); 
           Exception when no_data_found then
                     lc_Hra2  := null;
           end;
...
----------------------------------------------------------------------------------------
 ...
 ln_score  number(10,2);  
  ...
  begin
    begin
       select JAQL546Accio, JAQL546Respp, JAQL546Score
         into lc_accion,
              lc_respp,
              ln_score
         from Jaql546 
        Where JAQL546NUCON = v_NuOpe
          and JAQL546Accio in ('O','R')
          and jaql546hoenv in ( select max(jaql546hoenv)
                                  from Jaql546 
                                 Where JAQL546NUCON = v_NuOpe
                                   and JAQL546Accio in ('O','R'));
    Exception when no_data_found then
...
----------------------------------------------------------------------------------------
 procedure SP_CR_Calif_SBS(v_Cta in number, v_Calif out number, v_Desc out varchar2) is
 ...
 ----------------------------------------------------------------------------------------
 procedure sp_cr_Cal_SBS_Titular(v_Tdoc in number, v_Ndoc in varchar2, d_Date in date, v_Desc1 out varchar2) is
 ...
 Begin
   ...
   If (ln_tdoc1 <> 9) then 
     ...
     If (ln_cal0 = 0 and ln_cal1 = 0 and ln_cal2 = 0 and ln_cal3 = 0 and ln_cal4 = 0) then
         lc_desc1 := 'Sin Calificacion';
     Else   
         If ln_cal0 > 0 then
              lc_desc1 := 'Normal'; --1
...
----------------------------------------------------------------------------------------
 procedure sp_cr_Cal_SBS_Conyugue(v_Tdoc in number, v_Ndoc in varchar2, v_Desc2 out varchar2) is
