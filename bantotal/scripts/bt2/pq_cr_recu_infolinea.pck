CREATE OR REPLACE PACKAGE PQ_CR_RECU_INFOLINEA IS
...
PROCEDURE sp_obtener_cargos_jud(pn_emp    in number,
                                 pn_mod    in number,
                                 pn_suc    in number,
                                 pn_mda    in number,
                                 pn_pap    in number,
...
END PQ_CR_RECU_INFOLINEA;
/

CREATE OR REPLACE PACKAGE BODY PQ_CR_RECU_INFOLINEA IS
...
begin
  ...
  BEGIN  
    PQ_CR_RECU_INFOLINEA.sp_consolidado_cuotas(pn_emp,
                                                pn_mod,
                                                pn_suc,
                                                pn_mda,
                                                pn_pap,
  ...
  BEGIN  
    ...
    PQ_CR_RECU_INFOLINEA.sp_obtener_nro_refin(pn_emp,
                                                pn_mod,
                                                pn_suc,
                                                pn_mda,
                                                pn_pap,
  ...
  BEGIN  
    ...
    PQ_CR_RECU_INFOLINEA.sp_obtener_fec_castigo(pn_emp,
                                                 pn_mod,
                                                 pn_suc,
                                                 pn_mda,
                                                 pn_pap,
  ...
  BEGIN  
    ...
    PQ_CR_RECU_INFOLINEA.sp_obtener_informacion_rl(pn_emp,
                                                 pn_mod,
                                                 pn_suc,
                                                 pn_mda,
                                                 pn_pap,
  ...
  BEGIN
    UPDATE aqpb601 a
       set 
           a.aqpb601scap = ABS(vi_scap),
           a.aqpb601icon = ABS(vi_ic),
           a.aqpb601icv  = ABS(vi_icv),
           a.aqpb601mora = ABS(vi_mor),a.aqpb601pena= ABS(vi_pen),a.aqpb601otro = ABS(vi_otr),a.aqpb601segu = ABS(vi_seg),
           a.aqpb601dtot = ABS(vi_dtot), a.aqpb601agen = vi_agen,
...
PROCEDURE sp_consolidado_cuotas(pn_emp in number,
  ...
  begin
       ...
       vo_datr :=0;
        ...
        IF pn_mod = 200 OR pn_mod = 33 then   --JUDICIAL O CASTIGADO.  
            BEGIN
               sp_obtener_cargos_jud(pn_emp,
                                     pn_mod,
                                     pn_suc,
                                     pn_mda,
                                     pn_pap,
                                     ...
                                     vs_stat);
            END;
            vo_datr := fn_dias_atraso(pd_fecpro, --fecha de proceso
                                      pn_emp,
                                      pn_mod,
                                      pn_suc,
                                      pn_mda,
...
PROCEDURE sp_obtener_lista_pna(ve_pais in number, ve_tipodoc in number, ve_numdoc in varchar, ubuser in varchar) IS
  ...
  cursor lista_pna is
  --select * from fsr008 f where f.pepais=ve_pais and f.petdoc=ve_tipodoc and trim(f.pendoc)=TRIM(ve_numdoc);
           ...
           IF vi_cta <> 0 THEN
              ...
              BEGIN
                ...
                AQPB601 A (A.aqpb601pais,A.aqpb601ptdc,A.aqpb601pnd,A.aqpb601emp,A.AQPB601SUC,A.AQPB601MDA,A.AQPB601PAP,A.AQPB601CTA,A.AQPB601OPER,A.AQPB601SBOP,A.AQPB601TOP,A.AQPB601MOD,
                          ...
                          PQ_CR_RECU_INFOLINEA.sp_cuota_cabecera(vi_cod,vi_modulo,vi_suc,vi_mon,    
                          ...
                          PQ_CR_RECU_INFOLINEA.sp_DetalleCuotaMora(vi_cod,vi_modulo,vi_suc,vi_mon,    
                                                                   vi_pap,vi_cta,vi_ope,vi_subope,    
                                                                   vi_tipope,vi_fecpro,ubuser,ID_AQPB601COD);
              EXCEPTION WHEN OTHERS THEN
                 DBMS_OUTPUT.put_line(SQLERRM);
  ...
  PROCEDURE sp_DetalleCuotaMora(pn_emp    in number,
   ...
   select a.pgcod,
       ...
       where a.pgcod = pn_emp
         ...
         and a.d601co = 'S'
         and a.ppfpag > pd_fecpro
         and a.ppfpag <= TRUNC(LAST_DAY(pd_fecpro))
       ) s;       
       --order by s.ppfpag;
      ...
      IF pn_mod = 200 OR pn_mod = 33 then   --JUDICIAL O CASTIGADO.  
        BEGIN
           sp_obtener_cargos_jud(pn_emp,
                                 ...
                                 vi_stat);
        END;
        vi_datr := fn_dias_atraso(pd_fecpro, --fecha de proceso
                                  pn_emp,
                                  pn_mod,
                                  pn_suc,
                                  pn_mda,
        ...
        BEGIN
            ...
            AQPB601D (aqpb601dcodp,
                      ...
                      aqpb601dusrr,
                      aqpb601dfecr )
                      VALUES (id_aqpb601cod,1,vi_fven,vi_datr,abs(vi_cap),abs(vi_ic),abs(vi_icv),abs(vi_mor),abs(vi_pen),abs(vi_seg),abs(vi_otr),abs(vi_tot),ubuser,sysdate);
             COMMIT;
          EXCEPTION WHEN OTHERS THEN
             DBMS_OUTPUT.put_line(SQLERRM);
          END;              
...
PROCEDURE sp_obtener_cargos_jud(pn_emp    in number,
                                 ...
                                 ve_tot out number,
                                 ve_fvenc out date,
                                 ve_stat out number) is
   vi_rubro_im number(18);
   vi_rubro_icv number(18);
   vi_rubro_pen number(18);
   vi_rubro_int varchar(18);
   vi_rubro_seg number(18);
   vi_rubro_otr number(18);                              
   begin
     ...
     --OBTENER IM
     IF pn_mda = 0 then
        vi_rubro_im  := 9300073000000;
        vi_rubro_icv := 9300076000000;
        vi_rubro_pen := 9300072000000;
        vi_rubro_seg := 9300077000000;
        vi_rubro_otr := 9300075000000;
        vi_rubro_int := '811403%'; 
     ELSE
        vi_rubro_im  := 9300073100000;
        vi_rubro_icv := 9300076100000;
        vi_rubro_pen := 9300072100000;
        vi_rubro_seg := 9300077100000;
        vi_rubro_otr := 9300075100000;
        vi_rubro_int := '812403%';
     END IF;
     --obtener mora
     BEGIN
       ...
       INTO ve_mor
       FROM FSD011 D1
       WHERE  D1.PGCOD = pn_emp
         AND  D1.Scrub = vi_rubro_im
         AND  D1.SCCTA = pn_cta
         AND  D1.SCOPER= pn_ope;
     EXCEPTION 
       WHEN OTHERS THEN
            ve_mor := 0;      
       END;
     ...
     BEGIN
       ve_cap := 0;
       SELECT D1.SCSDO,D1.SCFVTO,D1.SCSTAT
       INTO ve_cap,ve_fvenc,ve_stat
       FROM FSD011 D1
       WHERE  D1.PGCOD = pn_emp
         AND  D1.SCMOD = pn_mod 
         AND  D1.SCSUC = pn_suc 
         ...
         AND  D1.SCCTA = pn_cta 
         AND  D1.SCOPER = pn_ope 
         AND  D1.SCSBOP = pn_sbo 
         AND  D1.SCTOPE = pn_top ;
     EXCEPTION 
       WHEN OTHERS THEN
            ve_cap := 0;      
       END; 
     ...
     BEGIN
       ...
       FROM FSD011 D1
       WHERE  D1.PGCOD = pn_emp
         AND  D1.Scrub LIKE vi_rubro_int
         AND  D1.SCCTA = pn_cta
         AND  D1.SCOPER= pn_ope;
     EXCEPTION 
       WHEN OTHERS THEN
     ...
     END;
     --OBTENER TOTAL
     ve_tot := ve_cap+ve_icv+ve_int+ve_mor+ve_otr+ve_pen+ve_seg;       
 END;
 function fn_dias_atraso(
                         v_Pgfape in date, --fecha de proceso
                         v_Pgcod  in number,
  ...
  -- *****************************************************************
    ...
    --                              2020.08.29 DCASTRO - se modifico calculo para retorno dias atraso covid
    -- *****************************************************************
