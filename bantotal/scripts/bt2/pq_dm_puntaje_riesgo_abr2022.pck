create or replace package PQ_DM_PUNTAJE_RIESGO_ABR2022 is
  ...
  Procedure SP_JOBS_PUNTAJE_RIESGO;
  Procedure SP_OCUM_PUNTAJE_RIESGO(PV_FECPRO In Varchar2,
                                   PN_NIVRIE In Number,
                                   PN_GEOGRA In Number
                                  );

end PQ_DM_PUNTAJE_RIESGO_ABR2022;
/

create or replace package body PQ_DM_PUNTAJE_RIESGO_ABR2022 is
  ...
  Is
    ...
    ld_fecpro date := to_date('20220430','rrrrmmdd');--last_day(add_months(trunc(sysdate),-1));  
    lv_Fecpro varchar2(10) := to_char(ld_fecpro,'rrrrmmdd');   
    ln_mjob   number(10);
    ln_job    number(10) := 0;
  ...
  Begin
       ...
       For n in c_nivrie Loop
           For g in c_geogra Loop
               ln_job := ln_job + 1; 
               lv_nproc := 'Begin '||'PQ_DM_PUNTAJE_RIESGO_ABR2022.SP_OCUM_PUNTAJE_RIESGO('''||lv_Fecpro||
               ...
               Loop
                   select count(*) into ln_mjob from dba_scheduler_jobs; --user_scheduler_jobs;
  ...
  Is
   ...
   ln_perini Number(6):= 202201;--to_number(to_char(add_months(ld_fecpro,-6),'rrrrmm'));
   
   Function fn_corr_jaqy474_abr(PN_PAIS IN Number,PN_TDOC In Number,PC_NDOC In Varchar2) Return Number
   Is
     ln_nreg Number(10);
   Begin
      Begin
...
Begin
     ...
     If to_char(ld_fecpro,'MM') not in ('01','07','04') Then  --AGREGA MES ABRI 04
        /*Nivel de Riesgo Medio,Alto y Muy Alto*/
        lv_query := lv_query||' and l.nivrie_key > 2 and l.ind_produ =2 and l.periodo = :3'; 
     Else
        lv_query  := lv_query||' and l.ind_produ =1 and l.periodo = :3';
     End If;                
     ...
     LOOP
       ...
       LIMIT 1000;            
          ...
          FOR x IN 1 .. n_pais.COUNT Loop   
             ln_numrg:=fn_corr_jaqy474_abr(n_pais(x),n_tdoc(x),trim(c_ndoc(x)));
             Begin
