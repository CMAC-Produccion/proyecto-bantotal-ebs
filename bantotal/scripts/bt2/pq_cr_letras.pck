create or replace package pq_cr_letras is

  -- ***************************************************************************************************************************
  -- Nombre                     : Paquete para el Módulo de Acuerdos - Devuelve el monto en letras
  -- Sistema                    : BANTOTAL
  ...
  -- Acceso                     : Público
  -- *************************************************************************************************************************** 
procedure sp_letras ( p_numero in number, 
                              vc_numero out varchar2);
FUNCTION monto_escrito (p_monto  IN NUMBER) 
                        RETURN VARCHAR2;                              
...
create or replace package body pq_cr_letras is

procedure sp_letras ( p_numero in number, 
                              vc_numero out varchar2) is                             
begin
  vc_numero := monto_escrito(p_numero);
end sp_letras;
FUNCTION monto_escrito (p_monto  IN NUMBER) 
                        RETURN VARCHAR2 IS
...
BEGIN
  ...
  tempo := SUBSTR(LTRIM(TO_CHAR(p_monto,'00000000000000.00')),2,16);
  num_letra := tempo;
  contador  := 1;
  -- Evalua la primera terna (Millardos) del monto
  IF SUBSTR(num_letra,2,3) <> '000' THEN
    -- verifica si hay valor en la centena
    IF SUBSTR(num_letra,2,1) <> '0' THEN
      aux      := SUBSTR(num_letra,1,1);
      cientos (aux,p_monto,parte,longitud,SUBSTR(num_letra,3,2));
      contador := contador + longitud;
      p_letra  := parte;
    END IF;
    -- Evalua si hay valor en las decenas
    IF SUBSTR(num_letra,3,2) <> '00' THEN
      aux      := SUBSTR(num_letra,3,2);
  ...
  IF SUBSTR(num_letra,5,3) <> '000' THEN
    -- verifica si hay valor en la centena
    IF SUBSTR(num_letra,5,1) <> '0' THEN
      aux      := SUBSTR(num_letra,4,1);
      cientos (aux,p_monto,parte,longitud,SUBSTR(num_letra,6,2));
      contador := contador + longitud;
      p_letra  := parte;
    END IF;
    -- evalua si hay valor en las decenas
    IF SUBSTR(num_letra,6,2) <> '00' THEN
      aux      := SUBSTR(num_letra,6,2);
      decenas (aux,parte,longitud);
      p_letra  := SUBSTR(p_letra,1,contador)||parte;
      contador := contador + longitud;
    END IF;
  ...
  IF SUBSTR(num_letra,8,3) <> '000' THEN
    -- Evalua si las centenas tienen valor
    IF SUBSTR(num_letra,8,1) <> '0' THEN
      aux      := SUBSTR(num_letra,8,1);
      cientos(aux,p_monto,parte,longitud,SUBSTR(num_letra,9,2));
      p_letra  := SUBSTR(p_letra,1,contador)||parte;
      contador := contador + longitud;
    END IF;
    -- Evalua si las decenas tienen valor
    IF SUBSTR(num_letra,9,2) <> '00' THEN
      aux      := SUBSTR(num_letra,9,2);
      decenas(aux,parte,longitud);
      p_letra  := SUBSTR(p_letra,1,contador)||parte;
      contador := contador + longitud;
    END IF;
  ...
  IF SUBSTR(num_letra,11,3) <> '000' THEN -- se modifico 10 x 12
    -- Evalua si la centena tiene valor
    IF SUBSTR(num_letra,11,1) <> '0' THEN -- se modifico 10 x 12
      aux      := SUBSTR(num_letra,11,1); -- se modifico 10 x 12
      cientos(aux,p_monto,parte,longitud,SUBSTR(num_letra,12,2)); -- se modifico 11 x 13
      p_letra  := SUBSTR(p_letra,1,contador)||parte;
      contador := contador + longitud;
    END IF;
    -- Evalua si la decena tiene valor
    IF SUBSTR(num_letra,12,2) <> '00' THEN  -- se modifico 11 x 10
      aux      := SUBSTR(num_letra,12,2);   -- se modifico 11 x 10
      decenas(aux,parte,longitud);
      p_letra  := SUBSTR(p_letra,1,contador)||parte;
      contador := contador + longitud;
    END IF;
  ...
  IF SUBSTR(num_letra,1,6) != '000000' AND  SUBSTR(num_letra,7,6) = '000000'  THEN
    p_letra := mil_mil || SUBSTR(p_letra,1,contador+1) ||
               --' SOLES CON ' ||               
               ' CON ' ||
              SUBSTR(num_letra,15,2) ||'/100 SOLES';
  ELSIF SUBSTR(num_letra,4,5) = '00000' AND SUBSTR(p_monto,1,1)=',' THEN
     p_letra := mil_mil || p_letra ||' SOLES';
  ELSE
     p_letra := mil_mil || SUBSTR(p_letra,1,contador+1) ||
               --' SOLES CON ' ||               
               ' CON ' ||
