create or replace package PQ_CV_MONITOREO_ACF_GK is
  -- ------------------------------------------------------------------------------------------------
  -- Nombre                : PQ_CV_MONITOREO_ACF_GK
  -- Sistema               : BANTOTAL
  -- Módulo                : AGENTES KASNET
  -- Versión               : 1.0
  -- Fecha de Creación     : 12/12/2018
  -- Autor de Creación     : JPINTO
  -- Uso                   : Construccion de la trama a enviar a UNIBANCA para el monitoreo de GK - ACF+
  -- Estado                : Activo
  -- Acceso                : Público
...
create or replace package body PQ_CV_MONITOREO_ACF_GK is
  ...
  *******************************************************************************************/
  ...
  begin
    --//
    select a.jaql673nutar
      into lc_cmp011
      from jaql673 a
     where a.jaql673seint = pn_numtra
       and a.jaql673fetra = pd_fectra
       and a.jaql673hotra = pc_hortra;
  ...
  begin
    --//    
    select lpad(trim(a.jaql673cotra), 6, '0')
      into lc_cmp012
      from jaql673 a
     where a.jaql673seint = pn_numtra
       and a.jaql673fetra = pd_fectra
  ...
  begin
    ...
    --//
    --jlunaf 10/06/2022 Se añade variable para obtener código de transacción
    select a.jaql674imptr, a.jaql674comot, a.jaql674cotra
      into ln_aux004, ln_codmda, lc_aux005
      from jaql674 a
     where a.jaql674seint = pn_numtra
       and a.jaql674fetra = pd_fectra
       and a.jaql674hotra = pc_hortra;
    --jlunaf 10/06/2022 Se añade condicion para transacción de consulta de saldos
    if lc_aux005 = '930000' then
       ln_aux004 := 0.0;
    end if;
  ...
  function fn_trama_indice016(pn_numtra in number,
    ...
    lc_aux005 char(6); --jlunaf 10/06/2022
  begin
    --//
    --jlunaf 10/06/2022 Se añade variable para obtener código de transacción
    select a.jaql674imptr, a.jaql674cotra
      into ln_aux004, lc_aux005
      from jaql674 a
     where a.jaql674seint = pn_numtra
       and a.jaql674fetra = pd_fectra
       and a.jaql674hotra = pc_hortra;
  ...
  function fn_trama_indice026(pn_numtra in number,
                              pd_fectra in date,
                              pc_hortra in varchar2) return varchar2 is
    lc_cmp026 varchar2(100) := '0';
  begin
    --//
    --JAQL6Gineg
    select nvl(trim(a.jaql673gineg), '6010')
      into lc_cmp026
      from jaql673 a
     where a.jaql673seint = pn_numtra
       and a.jaql673fetra = pd_fectra
  ...
  begin
    --//
    /*select nvl(substr(jaql673inadq, -6), '0') into lc_cmp034
    from jaql673 a
    where a.jaql673seint=pn_numtra 
    and a.jaql673fetra= pd_fectra
    and a.jaql673hotra= pc_hortra;     */
  ...
  begin
    --//Solo se muestra Codigo Comercio, no se tiene el nombre.
    select nvl(jaql674Cisoc, ' ')
      into lc_cmp042
      from jaql674 a
     where a.jaql674seint = pn_numtra
       and a.jaql674fetra = pd_fectra
  ...
  begin
    --//No se tiene la localidad del comercio
  
    /*select nvl(trim(d.locnom), ' ') into lc_cmp043
    from jaql674 a  
    inner join jaql009 c on c.jaql9cisoc= a.jaql674Cisoc
    inner join jaql004 b on c.jaql4cocom=b.jaql4cocom
    inner join fst070 d on b.jaql4depar = d.depcod and b.jaql4provi=d.loccod
    where a.jaql674seint=pn_numtra 
    and a.jaql674fetra= pd_fectra
    and a.jaql674hotra= pc_hortra;*/
  ...
  begin
    ...
    select decode(a.jaql674mda02, 0, '604', 101, '840')
      into lc_cmp049
      from jaql674 a
     where a.jaql674seint = pn_numtra
       and a.jaql674fetra = pd_fectra
  ...
  begin
    ...
    select substr(a.jaql673nutar, 1, 16) into lc_auxtar from jaql673 a where a.jaql673seint = pn_numtra
       and a.jaql673fetra = pd_fectra and a.jaql673hotra = pc_hortra;
  ...
  begin
    lc_cmp053 := ' ';
    -- Cuenta Asociada      
    -- Dmanriquec 29/05/2022 - Se modifica para condicionar los tipos de transaccion y se habilita  la obtención
    select a1. jaql674cttra 
    into lc_trans
        from jaql674 a1
       where a1.jaql674seint = pn_numtra
         and a1.jaql674fetra = pd_fectra
         and a1.jaql674hotra = pc_hortra;
    -- Dmanriquec 24/05/2023 - Se añade la transaccion 11 Retiro - Globokas
    if lc_trans  = 11 then
      select lpad(to_char(a1.jaql674cta02), 9, '0') ||
             lpad(to_char(a1.jaql674mod02), 3, '0') ||
             lpad(to_char(a1.jaql674mda02), 3, '0') ||
             lpad(to_char(a1.jaql674sbo02), 2, '0') ||
             lpad(to_char(a1.jaql674top02), 3, '0')
        into lc_cmp053
        from jaql674 a1
       where a1.jaql674seint = pn_numtra
         and a1.jaql674fetra = pd_fectra
  ...
  begin
    ...
    /*
    select lpad(to_char(a1.jaql674cta01), 9, '0') ||
           lpad(to_char(a1.jaql674mod01), 3, '0') ||
           lpad(to_char(a1.jaql674mda01), 3, '0') ||
           lpad(to_char(a1.jaql674sbo01), 2, '0') ||
           lpad(to_char(a1.jaql674top01), 3, '0')
      into lc_cmp054
      from jaql674 a1    
    where a1.jaql674seint=pn_numtra 
    and a1.jaql674fetra= pd_fectra
  ...
  begin
    ...
    /* select a.jaql674cta02 into ln_numcta
    from jaql674 a
    where a.jaql674seint=pn_numtra 
    and a.jaql674fetra= pd_fectra
    and a.jaql674hotra= pc_hortra;            
