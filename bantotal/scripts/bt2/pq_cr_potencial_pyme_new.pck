create or replace package pq_cr_potencial_pyme_new is
  -- *****************************************************************
  ...
  --  
  -- *****************************************************************
  Function fn_potencial_cliente(pd_fecha in date,
                                pn_pais  in number,
                                pn_tdoc  in number,
                                pc_ndoc  in char) return number;

  Procedure sp_potencial_cliente(pd_fecha in date,
                                 pn_pais  in number,
                                 pn_tdoc  in number,
                                 pc_ndoc  in char,
                                 pn_poten out number);

  Procedure sp_tiempo_potencial(pd_fecha in date, pn_resp out number);

  procedure sp_volumen_credito(pd_fecha in date, pn_result out number);

  procedure sp_cr_saldos_cuo_pyme(pn_pgpais  number,
                                  pn_tipdoc  number,
                                  pc_numdoc  char,
  ...
  procedure sp_recurrencia_credito(pd_fecha in date, pn_result out number);
  procedure sp_cr_recurrencia_cliente(pd_fecfin      date,
                                      pn_meses_eva   number,
                                      pn_pgpais      number,
                                      pn_tipdoc      number,
                                      pc_numdoc      char,
  ...
  Function fn_codsbs_cliente(pd_fecini in date,
                             pd_fecfin in date,
                             pn_pgpais in number,
                             pn_tipdoc in number,
                             pc_numdoc in char) return varchar;

  Procedure sp_job_tiempo_potencial(pd_fecha in date, pn_jobs in number);

  Procedure sp_tiempo_potencial_j(pv_cod   in varchar,
                                  pn_jobs  in number,
                                  pd_fecha in date);

  Procedure sp_job_volumen_credito(pd_fecha in date, pn_jobs in number);

  Procedure sp_volumen_credito_j(pv_cod in number, pd_fecha in date);

  procedure sp_volumen_cliente_j(pd_fecfin    date,
                                 pn_meses_eva number,
                                 pn_pgpais    number,
                                 pn_tipdoc    number,
                                 pc_numdoc    char,
  ...
  Procedure sp_job_recurrencia_credito(pd_fecha in date, pn_jobs in number);

  Procedure sp_recurrencia_credito_j(pv_cod in number, pd_fecha in date);

  procedure sp_recurrencia_cliente_j(pd_fecfin      date,
                                     pn_meses_eva   number,
                                     pn_pgpais      number,
                                     pn_tipdoc      number,
                                     pc_numdoc      char,
  ...
  Procedure sp_job_resultado_credito(pd_fecha in date, pn_jobs in number);

  Procedure sp_resultado_credito_j(pv_cod in number, pd_fecha in date);

  Procedure sp_potencial_cliente2(pd_fecha in date,
                                  pn_pais  in number,
                                  pn_tdoc  in number,
                                  pc_ndoc  in char,
                                  pn_poten out number);

  Procedure sp_potencial_cliente_linea(pd_fecha in date,
                                       pn_pais  in number,
                                       pn_tdoc  in number,
                                       pc_ndoc  in char,
                                       pn_poten out number);

  Procedure sp_tiempo_potencial_l(pd_fecha in date,
                                  pn_pais  in number,
                                  pn_tdoc  in number,
                                  pc_ndoc  in char);

  procedure sp_volumen_credito_l(pd_fecha in date,
                                 pn_pais  in number,
                                 pn_tdoc  in number,
                                 pc_ndoc  in char);

  procedure sp_recurrencia_credito_l(pd_fecha in date,
                                     pn_pais  in number,
                                     pn_tdoc  in number,
                                     pc_ndoc  in char);

  procedure sp_resultado_credito_l(pd_fecha in date,
                                   pn_pais  in number,
                                   pn_tdoc  in number,
                                   pc_ndoc  in char,
                                   pn_pote  out number);

  Procedure sp_potencial_cliente_l(pd_fecha in date,
                                   pn_pais  in number,
                                   pn_tdoc  in number,
                                   pc_ndoc  in char,
                                   pn_poten out number);
end pq_cr_potencial_pyme_new;
/

create or replace package body pq_cr_potencial_pyme_new is
  ...
  begin
    ...
    begin
      ...
      select decode(t.aqpb833va1, 0, 1, t.aqpb833va1) *
    ...
    exception
      when others then
        ...
        begin
          ...
          pq_cr_potencial_pyme_new.sp_potencial_cliente_linea(pd_fecha => pd_fecha,
                                                              pn_pais  => pn_pais,
                                                              pn_tdoc  => pn_tdoc,
                                                              pc_ndoc  => pc_ndoc,
                                                              pn_poten => ln_potval);
  ...
  Procedure sp_tiempo_potencial(pd_fecha in date, pn_resp out number) is
    ...
    /*proceso de edad*/
      select /*+ all_rows */
       ...
       case
         when edad > 58 then
          1
         when edad > 51 and edad <= 58 then
          2
         when edad > 44 and edad <= 51 then
          3
         when edad > 36 and edad <= 44 then
          4
         when edad >= 0 and edad <= 36 then
          5
       end valor,
  ...
  begin
    ...
    begin
      select /*+ all_rows */
       sum(c.n_salcap), count(distinct c.d_fecpre) --*  --sum(c.n_salcap) INACTIVO-- into pn_result  
  ...
  -----------------------------------------------------------------------------
  procedure sp_volumen_credito(pd_fecha in date, pn_result out number) is
    -- *****************************************************************
    ...
    cursor titulares is
      select /*+ all_rows */
      distinct t.aqpb833fec, t.aqpb833pai, t.aqpb833tdo, t.aqpb833ndo
        from aqpb833 t
       where t.aqpb833fec = pd_fecha
  ...
  begin
    ...
    for ti in titulares loop
      ...
      begin
        pq_cr_potencial_pyme_new.sp_cr_saldos_cuo_pyme(pn_pgpais  => ti.aqpb833pai,
  ...
  procedure sp_recurrencia_credito(pd_fecha in date, pn_result out number) is
    -- *****************************************************************
  ...
  begin
    ...
    for ti in titulares loop
      ...
      --t.aqpb833fec, t.aqpb833pai, t.aqpb833tdo, t.aqpb833ndo
      begin
        pq_cr_potencial_pyme_new.sp_cr_recurrencia_cliente(pd_fecfin      => ld_fecrcc,
  ...
  begin
    ...
    begin
      select /*+ all_rows */
       min(c.d_fecpre) fecha_ini, count(distinct c.d_fecpre) --*  --sum(c.n_salcap) INACTIVO-- into pn_result  
  ...
  Function fn_codsbs_cliente(pd_fecini in date,
                             ...
                             pn_pgpais in number,
                             pn_tipdoc in number,
                             pc_numdoc in char) return varchar is
  ...
  Procedure sp_job_tiempo_potencial(pd_fecha in date, pn_jobs in number) is
    -- *****************************************************************
...
--                        instance  => 2,
  ...
  begin
    ...
    for c in c_cliente(ld_fecrcci, ld_fecrcc) loop
      begin
        ln_dig := round(substr(trim(c.pfndoc), -3, 3) * pn_jobs / 999, 0);
      exception
        when others then
          ln_dig := 0;
      end;
  ...
  -----------------------------------------------------------------------------
  Procedure sp_job_volumen_credito(pd_fecha in date, pn_jobs in number) is
    -- *****************************************************************
  ...
  begin
    ...
    for i in 0 .. pn_jobs loop
      lc_variable := ' begin ' ||
                     '  pq_cr_potencial_pyme_new.sp_volumen_credito_j(''' || i ||
                     ''',' || 'TO_DATE( ''' || lv_fecha ||
                     ''', ''DD/MM/RRRR''  ' || '));' || ' End; ';
      ln_job      := ln_job + 1;
      --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
...
--                        instance  => 2,
  ...
  procedure sp_volumen_credito_j(pv_cod in number, pd_fecha in date) is
    -- *****************************************************************
  ...
  begin
    ...
    for ti in titulares loop
      ...
      --t.aqpb833fec, t.aqpb833pai, t.aqpb833tdo, t.aqpb833ndo
      begin
        pq_cr_potencial_pyme_new.sp_volumen_cliente_j(pd_fecfin    => ld_fecrcc,
  ...
  Procedure sp_job_recurrencia_credito(pd_fecha in date, pn_jobs in number) is
    -- *****************************************************************
  ...
  begin
    ...
    for i in 0 .. pn_jobs loop
      lc_variable := ' begin ' ||
                     '  pq_cr_potencial_pyme_new.sp_recurrencia_credito_j(''' || i ||
                     ''',' || 'TO_DATE( ''' || lv_fecha ||
                     ''', ''DD/MM/RRRR''  ' || '));' || ' End; ';
      ln_job      := ln_job + 1;
      --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
...
--                        instance  => 2,
  ...
  -------------------------------------------------------------------------------------
  procedure sp_recurrencia_credito_j(pv_cod in number, pd_fecha in date) is
    -- *****************************************************************
  ...
  begin
    ...
    for ti in titulares loop
      ...
      --t.aqpb833fec, t.aqpb833pai, t.aqpb833tdo, t.aqpb833ndo
      begin
        pq_cr_potencial_pyme_new.sp_recurrencia_cliente_j(pd_fecfin      => ld_fecrcc,
                                                          pn_meses_eva   => ln_meses_eva,
                                                          pn_pgpais      => ti.aqpb833pai,
                                                          pn_tipdoc      => ti.aqpb833tdo,
                                                          pc_numdoc      => ti.aqpb833ndo,
  ...
  begin
    ...
    begin
      ...
      begin
        select /*+ all_rows */
         count(distinct c.d_fecpre) --*  --sum(c.n_salcap) INACTIVO-- into pn_result  
          into pn_meses_act
          FROM CLDRCCS C
         WHERE C.C_CODSBS = pv_codsbs --A.CODSBS
           AND C.D_FECPRE between ld_fecrcci and pd_fecfin
      ...
      begin
        select /*+ all_rows */
         min(c.d_fecpre) fecha_ini --*  --sum(c.n_salcap) INACTIVO-- into pn_result  
          into ld_fecini
          FROM CLDRCCS C
         WHERE C.C_CODSBS = pv_codsbs --A.CODSBS
              -- AND C.D_FECPRE between ld_fecrcci and pd_fecfin
  ...
  Procedure sp_job_resultado_credito(pd_fecha in date, pn_jobs in number) is
    -- *****************************************************************
    -- Nombre                     : sp_job_recurrencia_credito
    -- Sistema                    : BANTOTAL
    -- Módulo                     : Creditos
  ...
  begin
    ...
    for i in 0 .. pn_jobs loop
      lc_variable := ' begin ' ||
                     '  pq_cr_potencial_pyme_new.sp_resultado_credito_j(''' || i ||
                     ''',' || 'TO_DATE( ''' || lv_fecha ||
                     ''', ''DD/MM/RRRR''  ' || '));' || ' End; ';
      ln_job      := ln_job + 1;
      --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
...
--                        instance  => 2,
  ...
  -------------------------------------------------------------------------------------
  procedure sp_resultado_credito_j(pv_cod in number, pd_fecha in date) is
    -- *****************************************************************
    -- Nombre                     : sp_recurrencia_credito_j
    -- Sistema                    : BANTOTAL
    -- Módulo                     : Créditos - Activas
  ...
  begin
    ...
    for ti in titulares loop
      ...
      begin
        ...
        pq_cr_potencial_pyme_new.sp_potencial_cliente2(pd_fecha => ti.aqpb833fec,
  ...
  begin
    ...
    /*calculo de tiempo*/
    begin
      pq_cr_potencial_pyme_new.sp_tiempo_potencial_l(pd_fecha => pd_fecha,
                                                     pn_pais  => pn_pais,
                                                     pn_tdoc  => pn_tdoc,
                                                     pc_ndoc  => pc_ndoc);
    exception
    ...
    /*calculo de volumen*/
    begin
      pq_cr_potencial_pyme_new.sp_volumen_credito_l(pd_fecha => pd_fecha,
                                                    pn_pais  => pn_pais,
                                                    pn_tdoc  => pn_tdoc,
                                                    pc_ndoc  => pc_ndoc);
    exception
    ...
    begin
      pq_cr_potencial_pyme_new.sp_recurrencia_credito_l(pd_fecha => pd_fecha,
                                                        pn_pais  => pn_pais,
                                                        pn_tdoc  => pn_tdoc,
                                                        pc_ndoc  => pc_ndoc);
    exception
    ...
    /*calculo resultado */
    begin
      pq_cr_potencial_pyme_new.sp_resultado_credito_l(pd_fecha => pd_fecha,
                                                      pn_pais  => pn_pais,
                                                      pn_tdoc  => pn_tdoc,
                                                      pc_ndoc  => pc_ndoc,
                                                      pn_pote  => pn_poten);
  ...
  Procedure sp_tiempo_potencial_l(pd_fecha in date,
    ...
    /*proceso de edad*/
      select /*+ all_rows */
       ...
       pq_cr_potencial_pyme_new.fn_codsbs_cliente(pd_fecini => p_fecini,
                                                  pd_fecfin => p_fecfin,
                                                  pn_pgpais => pfpais,
                                                  pn_tipdoc => pftdoc,
       ...
       tipper
        from (
              ...
              select /*+ all_rows */
              distinct 604 pfpais,
                        decode(t.aqpb834tipdoc, 1, 21, 0) pftdoc,
                        t.aqpb834numdoc pfndoc,
                        pc_ndoc pfndoc2,
                        to_date(t.aqpb834naci, 'dd/mm/rr') pffnac,
                        case
                          when trunc(months_between(pd_fecha,
                                                    to_date(t.aqpb834naci,
                                                            'dd/mm/rrrr')) / 12) <= 0 then
                           trunc(months_between(pd_fecha,
                                                to_date(substr(t.aqpb834naci,
                                                               1,
                                                               6) || '19' ||
                                                        substr(t.aqpb834naci,
                                                               7,
                                                               2),
                                                        'dd/mm/rrrr')) / 12)
                          else
                        ...
                        'N' tipper
                from aqpb834 t
               where t.aqpb834numdoc = trim(pc_ndoc));
  ...
  begin
    ...
    for ti in titulares loop
      ...
      begin
        ...
        pq_cr_potencial_pyme_new.sp_potencial_cliente_l(pd_fecha => ti.aqpb835fec,
  ...
  begin
    ...
    begin
      ...
      /*obtener el valor del potencial del cliente*/
      select decode(t.aqpb835va1, 0, 1, t.aqpb835va1) *
