create or replace package pq_datos_sbs_2023 is
    -- *****************************************************************
...
function fn_ubi (pn_cta in number) return varchar2 ;
function fn_seg (pn_pais in number, pn_tipdoc in number, pc_numdoc in varchar2, pd_fecape in date) return varchar2;

function fn_seg_des (pn_instancia in number, 
                     pn_mod in number, 
                     pn_top in number,
                     pn_sub in number,
                     pn_rubro in number)  return varchar2 ;
function fn_segs(pn_emp in number, pn_mod in number, pn_suc in number, pn_mda in number,
                          pn_pap in number, pn_cta in number, pn_oper in number,
                          pn_sbop in number,pn_top in number, pd_fecpag in date ) return number ;


function fn_Saldo_relacion_mo(pn_emp in number, pn_mod in number, pn_suc in number, pn_mda in number,
                           pn_pap in number, pn_cta in number, pn_oper in number,
                           pn_sbop in number,pn_top in number, pd_fecpro in date, pn_nrorel in number,
                           pn_rubro in number) return number;
 
 Procedure sp_job_procesa_MDCR (pd_fecpro in varchar2 );
    Procedure sp_procesa_MBC1 (pn_numsuc in number, pd_fecpro in varchar2 );
 end pq_datos_sbs_2023;
/

create or replace package body pq_datos_sbs_2023 is
...
Procedure sp_job_procesa_datos (pd_fecpro in varchar2 ) is
  ...
  begin
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_datos('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
  ...
  for i in rubro loop
     begin
       ...
       select/*+all_rows parallel(sal,2,1)*/
             ...
             pq_datos_sbs_2023.fn_csbs ( cta.pepais, cta.petdoc, cta.pendoc,ld_fecpro),--csbs
             /*case when cta.petdoc = 21 then 1
                  when cta.petdoc = 9 then 6
                  when cta.petdoc = 5 then 5
                  else 0 end*/
             pq_datos_sbs_2023.fn_tipdoc_sbs(cta.petdoc), --tid
             cta.pendoc, --nid
             pq_datos_sbs_2023.fn_ncl ( cta.pepais, cta.petdoc, cta.pendoc),--ncl
             sal.bcoper, --ccr
             sal.bcmda, --mon
             pq_datos_sbs_2023.fn_morg(sal.bcemp , sal.bcmod , sal.bcsuc , sal.bcmda ,
                                  sal.bcpap , sal.bccta , sal.bcoper, sal.bcsbop,
                                  sal.bctop , pro.aoimp, ln_tipcam, abs(sal.bcsdmo) ), -- morg
             --skcr
             -- tcr
             pq_datos_sbs_2023.fn_cal(sal.bcemp, sal.bccta, sal.bcfech,4), --substr(sal.bccate,1,1), --cal
             pq_datos_sbs_2023.fn_cal(sal.bcemp, sal.bccta, sal.bcfech,9),-- calint
             --dak,
             --dakr
             --dapr
             --pci
             ...
             case when sal.bcmod = 141 then 3 
                  when sal.bcmod not in (141,117) then 4  else 0 end,--fcc 50%
             --sin
             --csin
             --sid
             --ccsid,
             ...
             --Dgr
             --fppk
             Case when sal.bcfvto = '01/01/0001' then pro.aofvto
                  else sal.bcfvto end,--fveg
             --fvep
             --pcuo
             --ncpr
             --ncpa
            ...
            /* case when sal.bcmod = 116 then 0
             ...
             sal.bcrubr,
             PRO.AOTMOR
        from  bantprod.fsh012 sal,  bantprod.fsr008 cta,  bantprod.fsd010 pro,
              bantprod.fst004 top,  bantprod.fst003 modu
       where sal.bcemp = 1
...
begin
  begin
   select sum(s.bcsdmn)
     into ln_saldo
     from bantprod.fsr014 r, bantprod.fsh012 s
...
begin
  begin
   select sum(s.bcsdmo)
     into ln_saldo
     from bantprod.fsr014 r, bantprod.fsh012 s --fsh012@bthist  s
    where s.bcemp = pn_emp
      and s.bcsuc = pn_suc
      and r.rubro in ( select rubro
                         from   bantprod.fsd014
...
begin
  begin
    select lpad(b.incol,2,'0')
      into lc_tcr
      from bantprod.fsi006 a, bantprod.fsi003 b
...
--08/2018
     select  round(avg(case when nvl(d602.pp1fech,pd_fecpro/*trunc(sysdate)*/) - d601.ppfpag < 0 then 0
       ...
       into ln_prodia
       from bantprod.fsd602 d602, bantprod.fsd601 d601
         ...
         select distinct s.bcsdmn, s.bcrubr
           ...
           from bantprod.fsr014 r, bantprod.fsh012 s,
                             (select rubro
                               from  bantprod.fsd014
                              where pcnivc in (select modulo
                                                 from  bantprod.fst111 where dscod = 50
                                                  and modulo not in (29, 33, 120, 144)
                                                union all select 117 from dual
                                                union all select 141 from dual   )
                                and pcimpu = 'S'
   ...
   ---------------------------------------       
       ...
       if  pn_mod not in (117,200) or ( pn_mod <> 200 and  pn_top not in (550) and pn_mod <> 117) then
           ...
           begin
             select min(b.aofval)
               into ld_fecdes
               from bantprod.fsr011 a, bantprod.fsd010 b
              where a.relcod = 46
                and a.R2MOD	 = pn_mod
                and a.R2CTA  = pn_cta
...
procedure sp_dgr_pcuo_ncpr(pn_emp in number, pn_mod in number, pn_suc in number, pn_mda in number,
       ...
       begin 
             select ((select min(n.ppfpag)
                       ...
                       where m.xllaocta  = n.ppcta
                         ...
                         nvl(pq_datos_sbs_2023.fn_ncpr(pn_emp,pn_mod, pn_suc,pn_mda,
  ...
  --                    and m.xllaosuc  = pn_suc
    ...
    if nvl(ln_cancuo,0) = 0 then
      ln_cancuo := pq_datos_sbs_2023.fn_ncpr(pn_emp,pn_mod, pn_suc,pn_mda,
...
begin
  ...
  select xx.actcod2--15.03.2013
    into ln_activi
    from bantprod.sngc60 aa,  bantprod.fst750 xx,  bantprod.fsr008 pp
   where pp.pgcod = 1
     and pp.ctnro = pn_cta
     and pp.cttfir ='T'
...
begin
     ...
     -- covid
     ln_sdocov := pq_datos_sbs_2023.fn_Saldo_relacion(pn_emp,pn_mod,pn_suc,pn_mda,pn_pap,
                  pn_cta,pn_oper, pn_sbop ,pn_top ,pd_fecpro ,345, pn_rubro ) ;
     if  nvl(ln_sdocov,0) <> 0 then 
         lc_covid := 'S';
     else
         lc_covid := 'N';
     end if;                  
      ...
      /*begin
             select distinct 'S'
               into lc_covid
               from  fsh015 aa,  fsh016 bb
              where aa.pgcod  = bb.pgcod 
                and aa.hcmod  = bb.hcmod 
                and aa.hsucor = bb.hsucor
                and aa.htran  = bb.htran 
                and aa.hnrel  = bb.hnrel 
                and aa.hfcon  = bb.hfcon 
                and aa.hccorr = 0
                and aa.hcmod  = 30
                and aa.htran  = 356
                and bb.hcord  = 10 
                and bb.hcta  = pn_cta
                and bb.hoper = pn_oper
                and bb.hmda = pn_mda
                and bb.hpap = pn_pap
                and aa.hfvc <= pd_fecpro;
           exception
             when no_data_found then
              begin 
                select distinct 'S'
                  into lc_covid
                   from  aqpb002 c
                  where AQPB002EST    ='C'
                    and c.AQPB002revr is null 
                    and c.aqpb002emp = pn_emp
                    and c.aqpb002suc = pn_suc
                    and c.aqpb002mod = pn_mod
                    and c.aqpb002mda = pn_mda
                    and c.aqpb002pap = pn_pap
                    and c.aqpb002cta = pn_cta
                    and c.aqpb002ope = pn_oper
                    and c.aqpb002sbo = pn_sbop
                    and c.aqpb002top = pn_top
                    and c.AQPB002FEP	<= pd_fecpro
                  union 
                 select 'S'
                   from  AQPB002 r 
                  where aqpb002est = 'C'
                    and AQPB002REVR = 'S'
                    and r.aqpb002feca > pd_fecpro + 1
                    and r.aqpb002emp = pn_emp
                    and r.aqpb002suc = pn_suc
                    and r.aqpb002mod = pn_mod
                    and r.aqpb002mda = pn_mda
                    and r.aqpb002pap = pn_pap
                    and r.aqpb002cta = pn_cta
                    and r.aqpb002ope = pn_oper
                    and r.aqpb002sbo = pn_sbop
                    and r.aqpb002top = pn_top;
               exception 
                 when no_data_found then 
                    begin 
                       select distinct 'S'
                         into lc_covid
                         from  jaqz698 v
                        where jaqz698est in ('X', 'C')
                          and v.jaqz698emp = pn_emp
                          and v.jaqz698suc = pn_suc
                          and v.jaqz698mod = pn_mod
                          and v.jaqz698mda = pn_mda
                          and v.jaqz698pap = pn_pap
                          and v.jaqz698cta = pn_cta
                          and v.jaqz698ope = pn_oper
                          and v.jaqz698sbo = pn_sbop
                          and v.jaqz698top = pn_top  
                          and (jaqz698emp, jaqz698mod, jaqz698suc, jaqz698mda, jaqz698pap,
                               jaqz698cta, jaqz698ope, jaqz698sbo, jaqz698top) not in
                               (select aqpb002emp,
                                       aqpb002mod,
                                       aqpb002suc,
                                       aqpb002mda,
                                  ...
                                  from  aqpb002 x
                                 where aqpb002est = 'C'
                                   and (aqpb002revr is null or aqpb002revr <> 'S')
                                   and x.aqpb002emp = pn_emp
                                   and x.aqpb002suc = pn_suc
                                   and x.aqpb002mod = pn_mod
                                   and x.aqpb002mda = pn_mda
                                   and x.aqpb002pap = pn_pap
                                   and x.aqpb002cta = pn_cta
                                   and x.aqpb002ope = pn_oper
                                   and x.aqpb002sbo = pn_sbop
                                   and x.aqpb002top = pn_top)
                        and jaqz698fep <=  pd_fecpro + 1;
                   exception
                     when others then 
                        lc_covid:= null;
     ...
     pq_datos_sbs_2023.sp_relcredi_nr(pn_pai => pn_pais,-- pais
     ...
     case when substr(pn_rubro,4,1) IN (5,6) and substr(pn_rubro,7,2) = 19 then
         ...
         /* when substr(pn_rubro,4,1) = 4 and pn_stat  = 33 then
          ...
          /*when pn_mod = 105 or ( pn_mod = 106 and pn_top in(2,4,6)) then
               lc_modalid:= 'PARALELO';*/
          when lc_covid = 'S' then
               lc_modalid:= 'COVID_19';     
          when lc_covid = 'N' and pq_datos_sbs_2023.fn_reprogramado (pn_mod,pn_suc,pn_mda,
          ...
          when lc_covid = 'N' and lc_existe = 'S' then
               lc_modalid:= 'REP_CAMBIO_FECHA';
          When lc_covid = 'N' and pc_indamp = 'S' then 
               lc_modalid:= 'AMPLIADO';  
          when lc_covid = 'N' and pn_mod = 105 or ( pn_mod = 106 and pn_top in(2,4,6)) then
               lc_modalid:= 'PARALELO';     
...
begin
    ...
    /*    select nvl(SNG001ORI,0), SNG001FIN
       ...
       select 4,a.HFCON
         into ln_amplia, ld_fecamp
         from bantprod.fsh016 a, bantprod.fsh015 b 
        where a.pgcod = b.pgcod
          ...
          and a.hrubro in (select /*+all_rows*/rubro 
                             from bantprod.fsd014 
                            where pcimpu ='S' 
                              and pcnivc in(select modulo from  bantprod.fst111 
                                             where dscod = 50 and modulo not in (29,120,144)
                                             union all
                                            select modulo from  bantprod.fst003 where modulo in (117,141))
                                               and pmtit <> 9) ;
      ...
      if pn_mod = 200  or pn_top = 550 then 
         begin
              select count(*), max (d.r011fc)
               ...
               from bantprod.fsr011 s, bantprod.fsr011 d
              where s.relcod = 52
                and s.r2cod  = 1
                and s.r2mod  = pn_mod
                and s.r2suc  = pn_suc
          ...
          begin
           ...
           -- reproramaciones Covit
           begin
             select 1, max(aa.hfvc)
               into ln_repcov, ld_feccov
               from bantprod.fsh015 aa, bantprod.fsh016 bb
              where aa.pgcod  = bb.pgcod 
                and aa.hcmod  = bb.hcmod 
                and aa.hsucor = bb.hsucor
                and aa.htran  = bb.htran 
           ...
           if  ld_feccov is null then 
              begin 
                select 1, max (AQPB002FEP) 
                into ln_repcov, ld_feccov
                from ( 
                 select c.AQPB002FEP
                   from bantprod.aqpb002 c
                  where AQPB002EST    ='C'
                    and c.AQPB002revr is null 
                    and c.aqpb002emp = pn_emp
                    and c.aqpb002suc = pn_suc
            ...
            if  ld_feccov is null then       
                    begin 
                       select 1, max(v.jaqz698fep)
                         into ln_repcov, ld_feccov
                         from bantprod.jaqz698 v
                        where jaqz698est in ('X', 'C')
                          and v.jaqz698emp = pn_emp
                          and v.jaqz698suc = pn_suc
                          and v.jaqz698mod = pn_mod
     ...
     if  ln_numrep > 0 and  ld_fulrep is null and ld_fdesatre is null then
        ...
        begin
           select max(s.d_fecrrp), count(*)
             ...
             from  bantprod.JAQL122/*MIGRA2.crdaprr */s, bantprod.bnj096 b
     ...
     -- verfica ultima reprogramacion.
     if ld_fulrep is not null and ld_feccov is not null and ld_feccov > ld_fulrep then 
        ld_fulrep := ld_fulrep;  
     end if; 
     if ln_numrep > 0 and ln_repcov > 0 then 
        ln_numrep := ln_numrep +  ln_repcov;
     elsif ln_numrep = 0  and ln_repcov > 0 then 
         ln_numrep := ln_repcov;
     end if;    
     -- Se modifico 
        ...
        begin
            select/*+parallel(b,2)*/  b.bcemp ,b.bcsuc ,b.bcmda ,b.bcpap, --15.03.2015
              ...
              into ln_emp ,ln_suc ,ln_mda ,ln_pap,
                   ...
                   ln_rubro,ln_lco
              from bantprod.fsr011 a, bantprod.fsh012 b
           ...
           pq_datos_sbs_2023.sp_cta_saldo_relacion(ln_emp, ln_mod, ln_suc, ln_mda,
                                              ln_pap, ln_cta, ln_oper,ln_sbop,
                                              ln_top, pd_fecpro, 18,
                                              to_char(ln_rubro), ln_lcnu , ln_cclcnu);
        else
           ln_lco    := pn_sdmn;
           pq_datos_sbs_2023.sp_cta_saldo_relacion(pn_emp, pn_mod, pn_suc, pn_mda,
...
Procedure sp_job_procesa_BDC01 (pd_fecpro in varchar2 ) is
  ...
  begin
    ...
    /*begin
     ...
     for i in sucursal loop
            ln_ini := i.sucurs;
            lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_BDC01('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
            ln_job := ln_job +1;
...
select ccl,csbs,tid,nid,ncl ,
 ...
 group by ccl,csbs,tid,nid,ncl ,
          bcmod,ccr,mon,aofval,morg,cal,calint,
...
--
     ...
     if i.bcmod not in (144) then
         ln_skcr := pq_datos_sbs_2023.fn_skcr(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
     ...
     if i.bcmod = 144 then
        ...
        lc_tcr := pq_datos_sbs_2023.fn_tcr(i.bcrubr);
     end if;
     
     ln_cbtea := pq_datos_sbs_2023.fn_cambia_tea(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                        i.bccta, i.ccr, i.bcsbop,i.bctop,ld_fecpro);
     ...
     if nvl(ln_tea,0) =0 then 
        ln_tea := pq_datos_sbs_2023.fn_tea(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
     ...
     ln_dakr :=  fn_dias_atraso_up(ld_fecpro/*trunc(sysdate)*/, i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                        i.bccta, i.ccr, i.bcsbop,i.bctop,i.bcprod,i.fveg);
     ln_dapr := pq_datos_sbs_2023.fn_dapr(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                     i.bccta, i.ccr, i.bcsbop,i.bctop,ld_fecpro);
     if i.bcmod not in (117) then
         if i.bcmod  = 141 then
            ln_pci  :=  pq_datos_sbs_2023.fn_Saldo_relacion3(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                             i.bccta, i.ccr, i.bcsbop,i.bctop,ld_fecpro, 35,'27_1%'/*i.bcrubr */) +
                        pq_datos_sbs_2023.fn_Saldo_relacion3(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
         ...
         if i.bcmod not in (141) then
             pq_datos_sbs_2023.sp_cta_saldo_relacion2(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
         ...
         ln_pci  := 0/*pq_datos_sbs_2023.fn_Saldo_relacion3(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
     ...
     pq_datos_sbs_2023.sp_nrprg_fultrprg (i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                     i.bccta, i.ccr, i.bcsbop,i.bctop, nvl(i.n_instan,0),ld_fecpro,
                                     ln_nrprg, ld_fultrprg, lc_indamp);
     if nvl(ln_nrprg,0) = 0 then
        ld_fultrprg := null;
     else
        if ld_fultrprg is null then 
        ld_fultrprg := pq_datos_sbs_2023.fn_ult_rprg (i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                     i.bccta, i.ccr, i.bcsbop,i.bctop, nvl(i.n_instan,0),ld_fecpro,i.bcrubr);
        end if;                              
     end if;                                
     --if i.aofval is null then                                 
       ld_fot  := pq_datos_sbs_2023.fn_fot(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                      i.bccta, i.ccr, i.bcsbop,i.bctop, 0/*nvl(ln_nrprg,0)*/ );
     /*else
       ld_fot  := i.aofval ;
     end if;*/                                  
     pq_datos_sbs_2023.sp_dgr_pcuo_ncpr(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                   i.bccta, i.ccr, i.bcsbop,i.bctop,/*nvl(ln_nrprg,0)*/0 ,ln_Dgr,ln_pcuo,ln_ncpr );
     ld_fppk := pq_datos_sbs_2023.fn_fppk(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                    i.bccta, i.ccr, i.bcsbop,i.bctop,/*nvl(ln_nrprg,0)*/0, ld_fot);
     ld_fvep := pq_datos_sbs_2023.fn_fvep(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                    i.bccta, i.ccr, i.bcsbop,i.bctop,ld_fecpro);
     ln_ncpa := pq_datos_sbs_2023.fn_ncpa (i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                    i.bccta, i.ccr, i.bcsbop,i.bctop,/*nvl(ln_nrprg,0)*/0,ld_fecpro);
     ln_ciiu := pq_datos_sbs_2023.fn_ciiu(i.bccta);
     begin
       select w.pepais, w.petdoc, w.pendoc
         into ln_pais, ln_tipdoc, lc_numdoc 
         from bantprod.fsr008 w
        where w.pgcod = 1
          and w.ctnro = i.bccta
          and w.cttfir = 'T';
     ...
     lc_mdcr := pq_datos_sbs_2023.fn_mdcr(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                    i.bccta, i.ccr, i.bcsbop,i.bctop, ld_fecpro,
                                    i.bcrubr, i.bcprod, i.n_instan,lc_indamp,nvl(ln_nrprg,0),
                                    ln_pais, ln_tipdoc, lc_numdoc );
    
     ln_indic := pq_datos_sbs_2023.fn_indic(i.bcmod,i.bcrubr);

     pq_datos_sbs_2023.sp_lco_lcnu_cclcnu(i.bcemp, i.bcmod, i.cage, i.mon,
     ...
     ln_secint := pq_datos_fin_mes.fn_cod_interno_pqn (i.bcmod, i.cage, i.mon,
                                     i.bcpap, i.bccta, i.ccr, i.bcsbop,
                                     i.bctop,ld_fecpro);
     lc_tcrint := pq_datos_sbs_2023.fn_tcrint(i.bcrubr,nvl(ln_secint,0));
     ln_esam   := pq_datos_sbs_2023.fn_esam(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                    i.bccta, i.ccr, i.bcsbop,i.bctop);
     ln_rfa    := 1; -- No refianicamos con recate agropecuario
     lc_tcri   := pq_datos_sbs_2023.fn_tcri(i.bcrubr,nvl(ln_secint,0));
     lc_TSPR   := pq_datos_sbs_2023.fn_tspi(i.bcmod,i.bctop);
     /*ln_CCND   := ln_lcnu;
     lc_CCCCND := ln_cclcnu;*/
     ln_rom    := pq_datos_sbs_2023.fn_rom(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                     i.bccta, i.ccr, i.bcsbop,i.bctop,ld_fecpro);
     -- verifica si la linea fue desembolsada
     lc_deslin := pq_datos_sbs_2023.fn_deslin(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                    i.bccta, i.ccr, i.bcsbop,i.bctop);
     ...
     lc_trasf := pq_datos_sbs_2023.fn_cartrasferida(i.bccta,i.ccr);
     lc_ubi   := pq_datos_sbs_2023.fn_ubi(i.bccta);
     lc_seg   := pq_datos_sbs_2023.fn_seg_des (nvl(i.n_instan,0),i.bcmod,i.bctop, i.bcsbop, i.bcrubr);
     lc_segcm   := pq_datos_sbs_2023.fn_seg( ln_pais, ln_tipdoc, lc_numdoc, ld_fecpro);
     ...
     if ln_skcr = 0
         ...
         and nvl(ln_lco, 0) = 0
         and nvl(ln_lcnu, 0) = 0
         and ( i.bcprod = 99 or i.bcmod = 144) then 
         null;
     else     
     ...
     begin
       insert into  BDC01  (ccl,csbs,tid,nid,ncl,ccr,mon,morg,
                          ...
                          TCRI,TSPR,CCND,CCCCND,ROM,TIPOC,CRACL,
                          ubi, SEG, SEGCM)--- ultimos
        values (i.ccl,nvl(i.csbs,'0000000000'),i.tid,i.nid,i.ncl,ln_ccr,i.mone,
             ...
             ln_dapr,
             ln_pci,
             i.kvi,case when i.kvi <> 0 then i.ccvi else null end,
             i.kre,case when i.kre <> 0 then i.ccre else null end,
             i.krf,case when i.krf <> 0 then i.ccrf else null end,
             i.kve,case when i.kve <> 0 then i.ccve else null end,
             i.kju,case when i.kju <> 0 then i.ccju else null end,
             i.kco,case when i.kco <> 0 then i.ccco else null end,
             i.fcc,
             ln_sin,case when ln_sin <> 0 then ln_csin else null end,
             ln_sid,case when ln_sid <> 0 then ln_ccsid else null end,
             ln_sis,case when ln_sis <> 0 then ln_ccsis else null end,
             nvl(ld_fot,ld_fvep),
             ln_esam,
             ln_Dgr,
             ld_fppk,
             Case when i.bcmod in (117) then null else i.fveg end,
             ld_fvep,
             ln_pcuo,
             ln_ncpr,
             ln_ncpa,
...
Procedure sp_job_procesa_BDC02 (pd_fecpro in varchar2 ) is
  ...
  begin
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_BDC02('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
        if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
            --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
            DBMS_JOB.SUBMIT(job => ln_job,
...
begin
  ...
  for i in prestamo loop
     ...
     for f in calendario (i.bcemp,i.cage, i.bcmod, i.bcpap, i.mon, i.bccta, i.ccr, i.bcsbop, i.bctop) loop
         ...
         ln_otr := null;
         begin
           ln_scom := pq_datos_sbs_2023.fn_scom (i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
           ...
           if ln_diaart < 0 then
              ...
              if nvl(f.stat,'X') = 'P' /*f.fecpag is null */ and ln_diaart > 0 then 
                  ln_sim  := abs(pq_datos_sbs_2023.fn_sim (i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
        ...
        for f in calendario2 (i.bcemp,i.cage, i.bcmod, i.bcpap, i.mon, i.bccta, i.ccr, i.bcsbop, i.bctop) loop
         ...
         begin
           ...
           if ln_diaart < 0 then
              ...
              ln_sim  := pq_datos_sbs_2023.fn_sim (i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                              i.bccta, i.ccr, i.bcsbop,i.bctop,f.capital,ln_diaart);
           end if;
           ...
           lc_trasf := pq_datos_sbs_2023.fn_cartrasferida(i.bccta,i.ccr);
           /*lc_hortra := pq_datos_sbs_2023.fn_hora_trasn(i.bcemp, i.bcmod,i.cage, i.bcpap,
         ...
         begin
           ...
           if ln_diaart < 0 then
              ...
              ln_sim  := f.mora;/*pq_datos_sbs_2023.fn_sim (i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
...
Procedure sp_job_procesa_BDC01P (pd_fecpro in varchar2 ) is
  ...
  begin
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_BDC01P('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
        if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
        --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
            DBMS_JOB.SUBMIT(job => ln_job,
...
begin
  begin
    select /*+index_ss(h12)*/ sum(h12.bcsdmn)
      into ln_salgar
      from bantprod.fsr011 r, bantprod.fsh012 h12
     where r.relcod = 50
       and r.r011co = 'S'
       and r.r1cod  = pn_emp
       and r.r1suc  = pn_suc
...
Procedure sp_job_procesa_BDC01_GP (pd_fecpro in varchar2 ) is
  ...
  lc_hostname varchar2(64);
  cursor sucursal is
  select sucurs from  bantprod.fst001 where pgcod =1 ;
  begin
     /*begin
         select host_name
           into lc_hostname
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_BDC01_GP('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
       if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
            --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
            DBMS_JOB.SUBMIT(job => ln_job,
...
--          p_c_error:=  lc_variable;
  ...
  begin
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_BDC01_DIF('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
        if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
            --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
            DBMS_JOB.SUBMIT(job => ln_job,
...
--  begin
     ...
     begin
          SELECT SUM(CASE WHEN  D.SNG026COD = 526 AND D.SNG023MTO <> 0 THEN Y.SNG120TCBI * D.SNG023MTO
            ...
            INTO ln_rpcn
            FROM bantprod.SNG023 D, bantprod.SNG021 X, bantprod.SNG120 Y
...
Procedure sp_job_procesa_BDC01_0102 (pd_fecpro in varchar2 ) is
  ...
  begin
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_BDC01_0102('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
        if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
            --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
            DBMS_JOB.SUBMIT(job => ln_job,
...
begin
  ...
  for i in prestamo loop
     begin
       pq_datos_sbs_2023.SP_IRCP( i.n_instan, i.tcr, i.nid,
 ...
 --  Deuda Indirecta SF
  ...
  -- Cuota Caja
   ...
   begin
      select max(a.ppcap + a.ppint)
        into ln_cuotaDCA
        from bantprod.fsd601 a, JAQL120 b
  ...
  begin
    select 0,  E.SNG065CAR --H.SNG055DSC
      into ln_cce, ln_nae
      from  bantprod.sng091 a, bantprod.sng065 e, bantprod.xwf700 g, bantprod.SNG055 H, bantprod.FPAE90 K,
            bantprod.fsr008 d, bantprod.fsd001 f, bantprod.fst014 j
  ...
  begin
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_ED01('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
        if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
            --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
            DBMS_JOB.SUBMIT(job => ln_job,
...
begin
  ...
  for i in prestamo loop
     begin
      ...
      pq_datos_sbs_2023.SP_DATOS_ED01(ld_Fecrcc,
                        i.csbs,
                        ln_tipcam,
                        i.bccta,
                        i.n_instan,
...
select a.pgcod,
       ...
       a.aostat
  from bantprod.fsd010 a, bantprod.fsr008 b
...
Procedure sp_job_procesa_BDC02_todo (pd_fecpro in varchar2 ) is
  ...
  begin
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_BDC02_todo('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
        if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
            --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
            DBMS_JOB.SUBMIT(job => ln_job,
...
Procedure sp_procesa_BDC02_todo (pn_numsuc in number,  pd_fecpro in varchar2 ) is
  ...
  cursor prestamo is
   ...
   select distinct a.ccl,a.bcemp,a.bcpap,a.bccta,a.bcsbop,
           a.bctop,a.bcmod,a.ccr,a.mon,a.cage,a.bcprod,a.d_fecpro, b.nrprg
      from jaql120 a, bdc01 b--, opebdc c
     where a.cage = pn_numsuc
       and a.d_fecpro = ld_fecpro
       and to_char(a.ccr)  = b.ccr (+)
       and a.ccl = b.ccl (+);
       /*
       and a.bccta = c.sccta
       and a.ccr = c.scoper*/   
...
/*select distinct to_char(b.pepais) || to_char(b.petdoc)||b.pendoc  ccl,  
           ...
           pq_datos_sbs_2023.fn_ncpr (a.bcemp, a.bcmod, a.bcsuc, a.bcmda,
                                 a.bcpap, a.bccta, a.bcoper, a.bcsbop, 
                                 a.bctop) nrprg
      from bantprod.fsh012 a, bantprod.fsr008 b, opebdc c
  ...
  cursor calendario (ln_emp in number, ln_suc in number, ln_mod in number,
                     ...
                     ln_oper in number, ln_sbop in number, ln_tope in number  ) is
  select case when d601.ppmda = 0 then 'PEN' else 'USD' end moneda,
         decode(sum(d602.pp1cap),null,sum(distinct d601.ppcap),sum(d602.pp1cap) )  capital,
         sum(distinct d601.ppcap) capital_c,
         sum(distinct d601.ppcap) - sum(nvl(d602.pp1cap,0)) capital_m,
         sum(nvl(d602.pp1cap,0)) capital_a,
         decode(sum(d602.pp1int),null,sum(distinct d601.ppint),sum(d602.pp1int) ) interes,
         sum(distinct d601.ppint) interes_c,
         sum(d602.pp1intm) mora, max(d602.pp1stat) stat,d601.ppsbop,
         d601.ppfvto fecvto, max(d602.pp1fech) fecpag,
         pq_datos_sbs_2023.fn_segs(ln_emp, ln_mod, ln_suc , ln_mda ,
                          ln_pap , ln_cta , ln_oper ,
                          ln_sbop, ln_tope , d601.ppfvto ) segs
 from   bantprod.FSD602 d602,  bantprod.FSD601 d601
      where d602.pgcod (+)   = d601.pgcod
        and d602.ppmod (+)   = d601.ppmod
        and d602.ppsuc (+)   = d601.ppsuc
...
group by d601.ppmda, d601.ppfvto,d601.ppsbop 
order by d601.ppsbop,d601.ppfvto ;
         ...
         begin
           ...
           if i.mon = 101 then
              if nvl(f.stat,'X') = 'P'  then 
                  ln_cap := round(f.capital_c * ln_tipcam,2);
                  ln_int := round(f.interes_c * ln_tipcam,2);
                  if f.fecpag <= ld_fecpro then 
                     ln_cap_a := round(f.capital_a * ln_tipcam,2);
                  end if;
              else
                  ln_cap := round(f.capital * ln_tipcam,2);
                  ln_int := round(f.interes * ln_tipcam,2);
              ...
              ln_mor := round(ln_sim * ln_tipcam,2);
              ln_otr := round(ln_scom * ln_tipcam,2);
              ln_segs:= round(f.segs * ln_tipcam,2);

           else
              if nvl(f.stat,'X') = 'P'  then 
                 ln_cap := round(f.capital_c,2);
                 if f.fecpag <= ld_fecpro then 
                    ln_cap_a := round(f.capital_a,2);
                 end if;
                 ln_int := round(f.interes_c,2);
              else
                 ln_cap := round(f.capital,2);
              ...
              ln_mor := round(ln_sim ,2);
              ln_otr := round(ln_scom,2);
              ln_segs:= round(f.segs ,2);
           end if;
           
           lc_trasf := pq_datos_sbs_2023.fn_cartrasferida(i.bccta,i.ccr);
           --if nvl(f.stat,'X') <> 'P' and  f.fecvto <= ld_fecpro then  
               insert into  BDC02  (ccl,ccr,ncuo,mon,mcuo,
                                  sic, scom, sim, fvep, fcan,segs, CRACL,Hora,MCPA)
                values (i.ccl, i.ccr,ln_numcuo,f.moneda,ln_cap,
                        ln_int,ln_otr,ln_mor, f.fecvto, decode( nvl(f.stat,'X'),'P', null, f.fecpag),ln_segs,lc_trasf,f.ppsbop,ln_cap_a);
           --end if;             
         exception
         when others then
               lc_coderr:=sqlerrm;
      ...
      if   ln_numcuo = 0 then
        for f in calendario2 (i.bcemp,i.cage, i.bcmod, i.bcpap, i.mon, i.bccta, i.ccr, i.bcsbop, i.bctop) loop
         ...
         begin
           ...
           --if nvl(f.stat,'X') <> 'P' and  f.fecvto <= ld_fecpro then  
              insert into  BDC02  (ccl,ccr,ncuo,mon,mcuo,
                              sic, scom, sim, fvep, fcan,CRACL,Hora,SEGS,NCRO,MCPA)
              values (i.ccl, i.ccr,ln_numcuo,f.moneda,ln_cap,
                    ln_int,ln_otr,ln_mor, f.fecvto, decode( nvl(f.stat,'X'),'P', null, f.fecpag),lc_trasf,f.ppsbop, ln_segs, f.ppsbop,ln_cap_a);
           --end if;
...
*/     
     pq_datos_sbs_2023.sp_TIPCLI(ld_fecpro,
                                       ln_pais,
                                       ln_tipdoc,
                                       lc_numdoc,
                                       'BANTPROD',
     ...
     lc_tipclic:= pq_datos_sbs_2023.fn_tipli_cierre(ln_pais,ln_tipdoc,lc_numdoc, ld_fecpro) ;   
     --- 
     begin 
        select nvl(trunc(max(w.wfiteminit)), i.FOT)
         into ld_Fecsol
         from bantprod.WFWRKITEMS w 
    ...
    /* 
     begin
           select q.jaqy067ncal 
             into lc_segcre
             from bantprod.jaqy066 z, bantprod.jaqy067 q 
        ...
        -- Segmento Persona
           ...
           begin
              select to_number(p.pae71val)
                into ln_segmer
                from bantprod.fpae70 r, bantprod.fpae71 p
           ...
           if ln_segmer = 1 then
              ...
              begin 
                  select p.pae71val
                    into lc_segcre
                    from bantprod.xwf070 x, bantprod.fpae70 r, bantprod.fpae71 p
...
Procedure sp_job_procesa_BDC01_O (pd_fecpro in varchar2 ) is
  ...
  begin
    ...
    /*begin
     ...
     for i in sucursal loop
        ln_ini := i.sucurs;
        lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_BDC01_O('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
        ln_job := ln_job +1;
        if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
             --DBMS_JOB.SUBMIT(ln_job, lc_variable, sysdate+1/(24*180));
             DBMS_JOB.SUBMIT(job => ln_job,
...
begin
    ...
    /*    select nvl(SNG001ORI,0), SNG001FIN
     ...
     if ld_fecref is not null and ld_feccov is not null and ld_feccov > ld_fulrep then  
        ld_fulcam := to_char(ld_fecref,'dd/mm/yyyy');
     elsif ld_fdesatre is not null and ld_fdesatre <> to_date('19010101','yyyymmdd') then 
        if ld_feccov is not null and ld_feccov > ld_fdesatre then
           ld_fulcam := to_char(ld_feccov,'dd/mm/yyyy');
        else
           ld_fulcam := to_char(ld_fdesatre,'dd/mm/yyyy');
        end if;    
     elsif ld_fdesatre = to_date('19010101','yyyymmdd') then 
        ...
        if ld_feccov is not null and ld_feccov > ld_fulrep then
          ld_fulcam := to_char(ld_feccov,'dd/mm/yyyy');
        else
          ld_fulcam := to_char(ld_fulrep,'dd/mm/yyyy');
        end if;   
...
begin
  ...
  begin
    select b.segcod
      into ln_SegLinea
      from bantprod.sngc60 a, bantprod.sngc07 b
  ...
  if ln_SegLinea = 1 then
    ...
    begin
      ...
      if P_N_CODCAl is null then
         bantprod.pq_cr_segmentacion_aplic.sp_carga_data(pd_fecpro => P_D_FECHA, --fecha del dia
    ...
    if ln_SegLinea = 2 then
      ...
      /*begin
        select trim(v.pae71val)
          into P_C_DESCAL
          from bantprod.fpae70 n, bantprod.fpae71 v
...
function fn_ubi (pn_cta in number) return varchar2 is
lc_dir varchar2(10);
begin
  begin
  select aa.sngc13ugeo
    into lc_dir
    from bantprod.sngc13 aa,bantprod.fsr008 bb
...
function fn_seg (pn_pais in number, pn_tipdoc in number, pc_numdoc in varchar2, pd_fecape in date) return varchar2 is
lc_segcre varchar2(100);
lc_numdoc jaqy066.jaqy066tndoc%type;
begin
  lc_numdoc := trim(pc_numdoc);
  begin
      select q.jaqy067ncal 
        ...
        where z.jaqy066paic  = pn_pais 
          and z.jaqy066tdoc  = pn_tipdoc 
          and z.jaqy066tndoc = lc_numdoc
          and z.jaqy066pano  = to_number(to_char(pd_fecape,'yyyy'))
          and z.jaqy066pmes  = to_number(to_char(pd_fecape,'mm'))
          and z.jaqy066nse   = 'S'
          and z.jaqy066calf  = q.jaqy067ccal; 
...
function fn_seg_des (pn_instancia in number, 
                     ...
                     pn_top in number,
                     pn_sub in number,
                     pn_rubro in number) return varchar2 is
lc_sgm varchar2(100);
ln_segmer number;
ln_item   number;
lc_subo varchar2(100);
begin
  -- Segmento Persona
      begin
          select to_number(p.pae71val)
           ...
           begin
              select to_number(p.pae71val)
               ...
               where r.pae70ins = pn_instancia
                 ...
                 and r.pae51eva = 2
                 and r.pae70nro = (select max(d.pae70nro)
                                     from bantprod.fpae70 d, bantprod.fpae71 pp
                                    where d.pae70ins = r.pae70ins
                                      and d.pae51eva = 2
                                      and d.pae51eva = pp.pae51eva
                                      and d.pae70nro = pp.pae70nro)
                 and trim(p.pae71val) in('2','1')
              group by p.pae71val ;
           exception
             when others  then 
      ...
      begin
        ...
        select trim(v.pae71val)
              ...
              from bantprod.fpae70 n, bantprod.fpae71 v
             where n.pae70nro = v.pae70nro
               and v.pae71ite = ln_item --265
               and n.pae51eva = v.pae51eva
               and v.pae51eva = 1
               and n.pae70ins = pn_instancia--numero de instancia
               and n.pae70nro = (select max(pae70nro)
                                   from bantprod.fpae70 f
                                  where f.pae70ins = n.pae70ins
                                    and f.pae51eva = 1);
       
      exception
         when no_data_found then
             if ln_segmer <> 1 then  -- item antiguo
                 begin 
                     select trim(v.pae71val)
                       into lc_sgm
                      from bantprod.fpae70 n, bantprod.fpae71 v
      ...
      if pn_mod  = 107 then 
         if pn_top in (1,4) then 
            lc_sgm   := 'TIPO A';
         elsif pn_top in (10) then 
            lc_sgm   := 'TIPO B';
         elsif pn_top in (30,33) then 
            lc_sgm   := 'PREFERENTE';   
         end if;    
      end if;  
      if pn_mod  = 106  and pn_top not in (1,2,3,5,6,7,8,27,28,29,35,36,37,40,41,58,59,60,61,62,63,65,66,70) AND ln_segmer = 2 then 
         lc_sgm   := 'NO CORRESPONDE';   
      elsif pn_mod  = 106  and ln_segmer = 1 then
         lc_sgm   := 'NO CORRESPONDE';   
      end if;
      if substr(pn_rubro,4,1) = 4 or pn_sub <> 0  then -- refinanciado
         lc_sgm   := 'NO CORRESPONDE';   
      end if;
      
      if pn_mod  = 110  and pn_top not in (50,60,65,66,138,139 ) AND ln_segmer = 2 then 
         lc_sgm   := 'NO CORRESPONDE';   
      end if;
      lc_subo := pq_datos_sbs_2023.fn_tspi(pn_mod,pn_top);
      if instr(lc_subo, 'DPF') > 0 or instr(lc_subo, 'CTS') > 0 or instr(lc_subo, 'Seguro') > 0 then -- DPF
         lc_sgm   := 'NO CORRESPONDE';   
      end if;
 return  lc_sgm;    
end fn_seg_des; 
...
function fn_segs(pn_emp in number, pn_mod in number, pn_suc in number, pn_mda in number,
                          pn_pap in number, pn_cta in number, pn_oper in number,
                          pn_sbop in number,pn_top in number, pd_fecpag in date ) return number is
ln_segs number;
lc_desgar varchar2(200);
...
Procedure sp_job_procesa_MDCR (pd_fecpro in varchar2 ) is
  ln_max number;
  ln_inc number;
  ln_ini number;
  ln_fin number;
  ...
  begin
    ...
    -- execute immediate ('truncate table  JAQL121 ');
     ...
     for i in sucursal loop
            ln_ini := i.sucurs;
            lc_variable := ' begin '|| '  pq_datos_sbs_2023.sp_procesa_MBC1('||ln_ini||','||''''||Pd_FECpro||'''' ||');'|| ' End; ';
            ln_job := ln_job +1;
             ...
             if UPPER(lc_hostname) in ('SC01ZDBADM010101','SC01ZDBADM020101')  then
                  ...
                  INSERT INTO  tab_jobs  (c_Codage,n_Numer1,c_detjob)
                  VALUES('MDC1',ln_ini,lc_variable);
                  COMMIT;
             else
                 DBMS_JOB.SUBMIT(job => ln_job,
                                what => lc_variable,
  ...
  exception
     when others then
           lc_coderr:=sqlerrm;
           INSERT INTO  LOG_ERROR_BANDEJA (N_NRO,C_CODBDJ,C_DESERR,D_FECERR)
           VALUES(0,'MDC01',lc_variable, TRUNC(SYSDATE));
           COMMIT;
--          p_c_error:=  lc_variable;
   ...
   Procedure sp_procesa_MBC1 (pn_numsuc in number, pd_fecpro in varchar2 ) is

  ld_fecpro date ;
  lc_coderr varchar2(300);
  cursor covid is
    select c.*, a.mdcr, decode(c.tid,1,21,9) tdoc, c.nid ndoc from bdc01 a, bantprod.jaqz698 b, jaql120 c
    where a.ccr = b.jaqz698ope
    and a.ccl = c.ccl
    and a.ccr = c.ccr
    and a.cage =  pn_numsuc
    and a.mdcr not like 'COVI%'
    and (jaqz698emp, jaqz698mod, jaqz698suc, jaqz698mda, jaqz698pap,
                               jaqz698cta, jaqz698ope, jaqz698sbo, jaqz698top) not in
                               (select aqpb002emp,
                                       aqpb002mod,
                                 ...
                                 where aqpb002est = 'C'
                                   and (aqpb002revr is null or aqpb002revr <> 'S')
                                   and x.aqpb002emp = jaqz698emp
                                   and x.aqpb002suc = jaqz698suc
                                   and x.aqpb002mod = jaqz698mod
                                   and x.aqpb002mda = jaqz698mda
                                   and x.aqpb002pap = jaqz698pap
                                   and x.aqpb002cta = jaqz698cta
                                   and x.aqpb002ope = jaqz698ope
                                   and x.aqpb002sbo = jaqz698sbo
                                   and x.aqpb002top = jaqz698top)
                        and jaqz698fep <=  a.d_fecpro + 1;
    ...
    begin 
      ...
      for i in covid loop
        ...
        lc_mdcr :=pq_datos_sbs_2023.fn_mdcr(i.bcemp, i.bcmod, i.cage, i.mon,i.bcpap,
                                        i.bccta, i.ccr, i.bcsbop,i.bctop, i.d_fecpro,
                                        i.bcrubr, i.bcprod, i.n_instan,'N',0,
                                        604, i.tdoc,i.ndoc  );
                                        
        
          if lc_mdcr <> i.mdcr then   
             begin                          
                update bdc01
                   set mdcr = lc_mdcr
                 where ccl = i.ccl
            ...
            exception
               when others then
                     lc_coderr:=sqlerrm;
                     INSERT INTO  LOG_ERROR_BANDEJA (N_NRO,C_CODBDJ,C_DESERR,D_FECERR)
                     VALUES(0,'MDC1',lc_coderr, TRUNC(SYSDATE));
                     COMMIT;
               end;
               commit;
          end if;
