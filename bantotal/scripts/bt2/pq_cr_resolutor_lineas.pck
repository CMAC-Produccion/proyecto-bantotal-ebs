create or replace package pq_cr_resolutor_lineas is
  ...
  -- Fecha de Modificación      : 16/05/2019
  -- Autor de la Modificación   : MPOSTIGOC
  -- Descripción de Modificación: Modificacion a procesos  sp_update_extornado, sp_cr_VerifUpLogs, sp_resultadonetolinea
  ...
  -- Autor de la Modificación   :  Maria Postigo
  -- Descripción de Modificación: Se agrego descomento la cuoto potencial en el calculo del ratio
  -- Fecha de Modificación      : 16/04/2024
  -- Autor de la Modificación   :  Maria Postigo
  -- Descripción de Modificación: Se considero nueva logica para atio en disposicion de lineas
  ...
  procedure sp_empresa_lineatmp1(ln_instancia in number,
                                 lv_flag90    out varchar2,
                                 ln_valor     out number);
  procedure sp_cr_verloglineas(ln_pgcod    in number,
                               ln_suc      in number,
                               ln_itmod    in number,
                               ln_ittran   in number,
                               ln_itrel    in number,
  ...
  procedure sp_update_extornado(ln_pgcod  in number,
                                ...
                                ln_itord  in number,
                                ln_itsbor in number);
  procedure sp_cr_VerifUpLogs(ln_pgcod  in number,
                              ln_suc    in number,
                              ln_itmod  in number,
                              ln_ittran in number,
                              ln_itrel  in number,
                              ln_itord  in number,
                              ln_itsbor in number);
  procedure sp_cr_VerExcepCalcRatio(ln_Instancia in number,
                                    lc_Programa  in varchar2,
                                    lc_Variable  in varchar2,
                                    ld_Fech      in date,
                                    lc_FlagExcep out varchar2);

end pq_cr_resolutor_lineas;
/

create or replace package body pq_cr_resolutor_lineas is
  -- *****************************************************************
  ...
  procedure sp_resultadonetolinea(ln_instancia in number,
    ...
    ln_InstUltEval           number;
    --   lc_exceptuado char(1);
    ln_SaldExtSoles NUMBER(17, 2) := 0;
    ln_SaldExtDolar NUMBER(17, 2) := 0;
  ...
  Begin
    ...
    Begin
      ...
      /* begin
        -- Call the procedure
        pq_cr_resolutor_lineas.sp_cr_verexcepcalcratio(ln_instancia => ln_instancia,
                                                       lc_programa  => 'EXC_DPF',
                                                       lc_variable  => 'EXC_DPF',
                                                       ld_fech      => ld_fechaactual,
                                                       lc_flagexcep => lc_exceptuado);
      end;
      
      if lc_exceptuado = 'N' then
        ---no exceptuado , calcula ratio*/ --17/05/2019 mpostigoc
      begin
      
        pq_cr_resolutor_caplineas.sp_cuentas(ln_pepais,
                                             ln_Petdoc,
                                             lc_pendoc,
                                             tipodecambio,
                                             ln_instancia,
      ...
      pq_cr_resolutor_lineas.sp_cr_CuotaContinAval(ln_Instancia      => ln_instancia,
                                                   lc_flgprg         => 'S',
      ...
      pq_cr_resolutor_lineas.sp_cr_CuotaContinCF(ln_Instancia    => ln_instancia,
                                                 lc_flgprg       => 'S',
                                                 LN_ITPGCOD      => LN_ITPGCOD,
                                                 LN_ITSUC        => LN_ITSUC,
                                                 LN_ITMOD        => LN_ITMOD,
      ...
      pq_cr_resolutor_lineas.sp_cR_CuotaPotencial(ln_Instancia   => ln_instancia,
        ...
        if ld_fecha >= jaqz515fecha then
          ...
          begin
            select s.sng021sol, s.sng021fec, S.SNG021SOL
              into ln_SolEval, ld_FchRegEval, ln_InstUltEval
              from sng021 s
          ...
          begin
            select sum(j.jaqy327gfin)
              into ln_SaldExtSoles
              from jaqy327 j
             where j.jaqy327inst = ln_InstUltEval
               and j.jaqy327esta = 'S'
               and j.jaqy327chek = '1'
          ...
          begin
            ...
            ln_SaldExtDolar := nvl(ln_SaldExtDolar, 0) * tipodecambio;
          
          end;
        
          ln_resultadorcc := nvl(ln_SaldExtDolar, 0) +
                             nvl(ln_SaldExtSoles, 0);
        
          /* -- 19/04/2024 MPOSTIGOC
          begin
            select sng023Mto
          ...
          if jaqz515fecha <= ld_fechaactual then
            ...
            ln_resultadonetoelse := (ln_resultnetosoleselse + (ln_resultnetodolareselse *
                                    tipodecambio)); --ojisto el tipo de cambio
          
            ln_resultadoneto    := nvl(ln_resultadonetoelse, 0);
            ln_resultadorccelse := ((ln_resultrccsoleselse +
                                   ln_resultrcc2soleselse) + ((ln_resultrccdolareselse +
                                   ln_resultrcc2dolareselse) *
                                   tipodecambio)); --ojisto el tipo de cambio
            ...
            ln_denominadorelse := (ln_resultadonetoelse +
                                  ln_resultadorccelse + ln_MntPotncial);
      ...
      /* elsif lc_exceptuado = 'S' then
        ---16052019
        ln_resultado := 0; --
      end if;*/ --17/05/2019 mpostigoc 
  ...
  begin
    ...
    if ln_pais is not null then
      ...
      for l in lista_CredVigCF(ln_pais, ln_tdoc, lc_ndoc) loop
        ...
        begin
          pq_cr_resolutor_caplineas.sp_cr_LogCuentas(lnPepais     => ln_pais,
                                                     lnPetdoc     => ln_tdoc,
                                                     lnPendoc     => lc_ndoc,
      ...
      for l in lista_CredVigAval(ln_paiscy, ln_tdoccy, lc_ndoccy) loop
        ...
        begin
          select count(*)
            into ln_EsConsd
            from aqpa271 j
           where j.aqpa271pgcod = l.ln_pgcod10
             and j.aqpa271mod = l.ln_mod10
             and j.aqpa271suc = l.ln_suc10
             and j.aqpa271mda = l.ln_mda10
             and j.aqpa271pap = l.ln_pap10
             and j.aqpa271cta = l.ln_cta10
             and j.aqpa271ope = l.ln_oper10
             and j.aqpa271sbop = l.ln_sbop10
             and j.aqpa271tope = l.ln_tope10
             and j.aqpa271inst = ln_Instancia
  ...
  Begin
    ...
    begin
      /*            ln_resultadorcc=:nvl(ln_resultadorcc,0);
      ln_resultadoneto:=nvl(ln_resultadoneto,0);
      ln_resultado:=nvl(ln_resultado,0);*/
      begin
  ...
  begin
    ...
    if LN_TIPOCRED = 0 then
      ...
      if LN_TIPOCRED = 0 then
        ...
        -- caso contrario buscamos el tipo de credito de acuerdo a la fecha de desembolso
  ...
  procedure sp_cr_verloglineas(ln_pgcod    in number,
                               ...
                               ln_itord    in number,
                               ln_itsbor   in number,
                               lc_TieneLog out varchar2) is
  
    ln_TieneLogCab number := 0;
    ln_TieneLogDet number := 0;
    ld_FchSistema  date;
    ld_FchCont     date;
    lc_Usuario     varchar2(10);
    lc_hora        char(8);
    lc_exceptuado  varchar2(1);
    ln_instancia   number;
    ld_fchsist     date;
    ln_modulo      number;
    ln_tipoper     number;
  ...
  begin
    ...
    begin
      select f.pgfape into ld_FchSistema from fst017 f where f.pgcod = 1;
    end;
    ...
    begin
      select count(*)
       ...
       where a.aqpa270pgcod = ln_pgcod
         ...
         and a.aqpa270itord = ln_itord
         and a.aqpa270itsbor = ln_itsbor
         and a.aqpa270fec = ld_FchSistema;
    exception
      when others then
        ln_TieneLogCab := 0;
    end;
    ...
    ln_TieneLogCab := nvl(ln_TieneLogCab, 0);
    ln_TieneLogDet := nvl(ln_TieneLogDet, 0);
  
    if ln_TieneLogCab > 0 and ln_TieneLogDet > 0 then
      lc_TieneLog := 'N';
    end if;
  
    if ln_TieneLogCab = 0 then
      lc_TieneLog := 'R';
    end if;
  
    if ln_TieneLogDet = 0 then
      lc_TieneLog := 'R';
    end if;
  
    if lc_TieneLog = 'N' then
      begin
        --Verifica Log con Ratio 0
        -- 30/05/2019 mpostigoc dcastro
        select a.aqpa270ratio, a.aqpa270resnet, a.aqpa270capcaja
          into ln_ratio, ln_resneto, ln_cuocmac
          from aqpa270 a
         where a.aqpa270pgcod = ln_pgcod
           and a.aqpa270itsuc = ln_suc
           and a.aqpa270itmod = ln_itmod
           ...
           and a.aqpa270fec = ld_FchSistema
           and a.aqpa270hora =
               (select max(b.aqpa270hora)
                  from aqpa270 b
                 where b.aqpa270pgcod = a.aqpa270pgcod
                   and b.aqpa270itsuc = a.aqpa270itsuc
                   and b.aqpa270itmod = a.aqpa270itmod
                   and b.aqpa270ittran = a.aqpa270ittran
                   and b.aqpa270itnrel = a.aqpa270itnrel
                   and b.aqpa270itord = a.aqpa270itord
                   and b.aqpa270fec = a.aqpa270fec);
      
      end;
    
      if ln_ratio is null or ln_ratio <= 0 then
        lc_TieneLog := 'R';
      end if;
    
      if ln_resneto is null or ln_resneto <= 0 then
        lc_TieneLog := 'R';
      end if;
    
      if ln_cuocmac is null or ln_cuocmac <= 0 then
        --mpostigoc 20200107
        lc_TieneLog := 'R';
      end if;
    ...
    if lc_TieneLog = 'R' then
    
      begin
        select f.itfcon, f.ituing
          into ld_FchCont, lc_Usuario
          from fsd015 f
         where f.pgcod = ln_pgcod
           and f.itsuc = ln_suc
  ...
  procedure sp_cr_VerifUpLogs(ln_pgcod  in number,
    ...
    ln_fechasistem date;
    ln_intancia    number;
    ln_UpLogDet    number := 0;
    ln_UpLogCab    number := 0;
  ...
  begin
    ...
    if ln_UpLogCab = 0 or ln_UpLogDet = 0 then
      ...
      begin
        ...
        if ln_intancia is not null then
          ...
          delete from aqpa270
           where aqpa270.aqpa270inst = ln_intancia
             and aqpa270.aqpa270pgcod = ln_pgcod --16/05/2019
             AND aqpa270.aqpa270itsuc = ln_suc
             AND aqpa270.aqpa270itmod = ln_itmod
             AND aqpa270.aqpa270ittran = ln_ittran
             AND aqpa270.aqpa270itnrel = ln_itrel
             AND aqpa270.aqpa270itord = ln_itord
             AND aqpa270.aqpa270itsbor = ln_itsbor
             and aqpa270.aqpa270est not in ('E', 'H');
          delete from aqpa271
           where aqpa271.aqpa271inst = ln_intancia
             and aqpa271.aqpa271pgcod = ln_pgcod --16/05/2019
             AND aqpa271.aqpa271itsuc = ln_suc
             AND aqpa271.aqpa271itmod = ln_itmod
             AND aqpa271.aqpa271ittran = ln_ittran
             AND aqpa271.aqpa271itnrel = ln_itrel
             AND aqpa271.aqpa271itord = ln_itord
             AND aqpa271.aqpa271itsbor = ln_itsbor
             and aqpa271.aqpa271est not in ('E', 'H');
        end if;
      end;
    
    end if;
  
  end sp_cr_VerifUpLogs;
  ----------------------------------------------------------
  procedure sp_cr_VerExcepCalcRatio(ln_Instancia in number,
                                    lc_Programa  in varchar2,
                                    lc_Variable  in varchar2,
                                    ld_Fech      in date,
                                    lc_FlagExcep out varchar2) is
    ln_cont number := 0;
  ...
  begin
    ...
    begin
      select count(*)
        into ln_cont
        from jaqz840 j
       where j.jaqz840prgm = lc_Programa
         and j.jaqz840nmbvar = lc_Variable
         and j.jaqz840varin1 = ln_Instancia
         and j.jaqz840vardaux1 >= ld_Fech
         and j.jaqz840vardaux2 <= ld_Fech;
    exception
      when others then
        ln_cont := 0;
    end;
