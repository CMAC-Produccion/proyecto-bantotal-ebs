CREATE OR REPLACE TRIGGER TG_FSR005_AD_01
	AFTER DELETE ON FSR005
		FOR EACH ROW
		 WHEN (OLD.PETDOC <> 0 AND OLD.PEPAIS <> 0) DECLARE
			...
			BEGIN
				...
				THEN
					IF lc_tel IS NOT NULL THEN 
						lc_tel :=  '<TELÉFONO>' || TRIM(:OLD.DOTELFP) || '</TELÉFONO>';
					END IF;
					INSERT INTO JAQN82(
					JAQN82FEC,
					JAQN82HOR,
				...
				ELSE
					IF lc_tel IS NOT NULL THEN 
						lc_tel :=  '<CELULAR>' || TRIM(:OLD.DOTELFP) || '</CELULAR>';
					END IF;
					INSERT INTO JAQN82(
					JAQN82FEC,
					JAQN82HOR,
