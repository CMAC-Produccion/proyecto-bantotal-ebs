create or replace package pq_cr_reprog_sin_cap is
  ...
  PROCEDURE SP_CRD_TIPO_REPROGRAMACION(codigo in number,instancia in number,vo_tprpg out number); 
  PROCEDURE SP_CRD_ARBOL_APROBACION(codigo in number,instancia in number,vo_cargo out number);
  PROCEDURE SP_CRD_ARBOL_APROBACION_CAJA2024(codigo in number,instancia in number,vo_cargo out number,vo_perfil out varchar);
  PROCEDURE SP_CRD_ARBOL_APROBACION_RNG(instancia in number,vo_ind_error out varchar2,vo_tip_error out varchar2);
  procedure SP_CREDITOS_UPD_RCAJA(
   ...
   PROCEDURE SP_CRD_OBTENER_APROBADOR(
                                     ve_cargo number,
                                     ve_sucursal number,
                                     vo_usuario OUT varchar,
                                     vo_usuario_sup OUT varchar
                                    ); 
   PROCEDURE SP_OBTENER_TASA_CAJA(
                                  ve_aqpb556cod number,
                                  ve_instancia number,
   ...
   PROCEDURE sp_grabar_cronograma(
                                 ve_codigo  IN  number,
   ...
   PROCEDURE SP_CARGAR_AUTORIZANTES(
                                    VE_AQPB556COD IN NUMBER,
                                    VE_INSTANCIA IN NUMBER,
   ...
   PROCEDURE sp_obtener_data_jaqa400(
                                     VE_AQPB556COD IN NUMBER,
                                     VE_INSTANCIA IN NUMBER,
  ...
  PROCEDURE SP_GRABAR_APROBACION(
                                  VE_FECHAREG IN DATE,
  ...
  PROCEDURE SP_GRABAR_LOG_ERRORES(
                                   VE_INSTANCIA IN NUMBER,
  ...
  PROCEDURE SP_ENVIAR_CORREO_PAPR(
                                   VE_FECHAREG    IN DATE,
                                   VE_CODREPROG   IN NUMBER,
                                   VE_INSTANCIA   IN NUMBER,
                                   VE_ESTADO      IN VARCHAR,
  ...
  PROCEDURE SP_OBTENER_CORREOS_AQPD157( 
                                    VE_FECHAREG       IN DATE,
                                    VE_CODREPROG      IN NUMBER,
                                    VE_INSTANCIA      IN NUMBER,
...
create or replace package body pq_cr_reprog_sin_cap is
  ...
  begin
    ...
    begin
      select aqpb556tprg
      into vi_aqpb556tprg
      from aqpb556 where aqpb556est='P'
         ...
         BEGIN
            for x in lista_aqpb556 loop
              --ACTUALIZANDO EL ESTADO EN LA TABLA DE REPROGRMACION DEL ANALISTA
               UPDATE JAQA400 J SET J.JAQA400AC1= ve_est   
                     WHERE J.JAQA400EMP = x.aqpb556emp
                       AND J.JAQA400MOD = x.aqpb556mod
                       AND J.JAQA400SUC = x.aqpb556suc
                       AND J.JAQA400MDA = x.aqpb556mda
  ...
  */
  ...
  begin    
      ...
      begin --ACTUALIZAR TIPO DE REPROGRAMACION, 
            --for x in lista_aqpb556 loop
               SELECT TRIM(J.JAQA400AC1) INTO TRPGCOVIT FROM JAQA400 J, AQPB556 T 
        ...
        BEGIN
          IF TRPGCOVIT IN ('C','U') THEN
              UPDATE AQPB556 SET AQPB556TCOV = TRPGCOVIT 
              WHERE AQPB556COD=ve_cod 
                AND AQPB556EST='A' 
                AND AQPB556EHAB='H'
  ...
  */
  ...
  if p_c_coderr = '00' then
    begin
      select y.penom, x.aqpb556scap,x.aqpb556usrr,x.aqpb556scons,x.aqpb556pais,x.aqpb556ptdc,x.aqpb556dni,x.Aqpb556inst
        into vcliente, vcapital, vanalista, vsaldocons,vi_aqpb556pais,vi_AQPB556PTDC,vi_AQPB556DNI,vi_AQPB556INST
  ...
  if p_c_coderr = '00' then
    ...
    lv_cuentat := 'cuenta ' || trim(to_char(P_N_SCCTA)) || ' y operaci?n ' ||
    ...
    Case
      WHEN P_V_TIPO = 'T' THEN    
        ...
        */
        ...
        lv_mensaje    := '<p "font-family: Arial, sans-serif; font-size: 14px;">Estimado '||vi_dcargo||',</p>' ||
                         ...
                         lv_cuentat ||'<br />'||trim(v_mensaje_a)||'.</p>';     
    end case;
    lv_remitente := 'notificacionesbantotal@cajaarequipa.pe';
    lv_asunto    := 'REPROGRAMACION CLIENTE COVIT ' || upper(trim(vcliente));
  ...
  PROCEDURE SP_CRD_TIPO_REPROGRAMACION(codigo in number,instancia in number,vo_tprpg out number) IS 
  vi_cuenta number(9);
  ...
  BEGIN
    ...
    BEGIN
      ...
      INTO  vi_cant_cov
      from  reprog F
      where SUBSTR(F.CUENTAOPERACION, 0, INSTR(F.CUENTAOPERACION, '-') - 1) = vi_cuenta
   ...
   select TIPOREPROGRAMACION
    ...
    BEGIN
      ...
      INNER JOIN LEY31050_CREDITOSFACILIDAD F
         ...
         WHERE L.ESTADOSOLICITUD = 'BT'
           ...
           AND SUBSTR(F.CUENTAOPERACION,INSTR(F.CUENTAOPERACION, '-') + 1,99)  = vi_operacion; --AGREGAR CLAVE DEL CREDITO
    EXCEPTION 
      WHEN OTHERS THEN
        vi_cant_caja := 0;
    END;
    BEGIN
      ...
      INNER JOIN LEY31050_CREDITOSFACILIDAD F
         ...
         WHERE  L.ESTADOSOLICITUD = 'BT'
           ...
           AND SUBSTR(F.CUENTAOPERACION, 0, INSTR(F.CUENTAOPERACION, '-') - 1) = vi_cuenta
           AND SUBSTR(F.CUENTAOPERACION,INSTR(F.CUENTAOPERACION, '-') + 1,99)  = vi_operacion; -- AGREGAR CLAVE CREDITO.
    EXCEPTION 
      WHEN OTHERS THEN
        vi_cant_caja := 0;
    END;
    ...
    BEGIN
      ...
      UPDATE  aqpb556 a set a.aqpb556tprg=vi_tireprog,a.aqpb556fecrpg = to_date(TO_CHAR(AQPB556FECR,'dd/mm/rrrr'),'DD/MM/RRRR')
    ...
    BEGIN
      PQ_CR_REPROG_SIN_CAP.SP_CARGAR_AUTORIZANTES(
                                                     codigo,
                                                     instancia,                                                     
                                                     VO_COD_ERROR,
                                                     VO_MSG_ERROR
  ...
  PROCEDURE SP_CRD_ARBOL_APROBACION(codigo in number,instancia in number,vo_cargo out number)is 
    vo_plazoh number(4);
    vo_plazoc number(4);
    vo_gracia number(4);
    vo_plazor number(4);
  ...
  BEGIN
      ...
      BEGIN
        ...
        SELECT T.AQPB556SCAP
          ...
          FROM AQPB556 T WHERE T.AQPB556INST = instancia
           --AND T.AQPB556EST='P'
           AND T.AQPB556EHAB='H'
           AND AQPB556COD = (SELECT MAX(AQPB556COD) FROM AQPB556 A WHERE A.AQPB556INST = instancia
           AND A.AQPB556EHAB='H');
      EXCEPTION WHEN OTHERS THEN
        NULL;     
      END;
      ...
      PQ_CR_RNG_REPROG_HS.sp_cr_plazo_matriz(instancia,
                                             vo_plazoh );
      PQ_CR_RNG_REPROG_HS.sp_cr_plazo_crd_caj(instancia,
                                              vo_plazoc);
      --IF vo_plazoh<vo_plazoc THEN 2024.01.22 Comentado.
         --PQ_CR_RNG_REPROG_HS.sp_cr_plazo_residual(instancia,
         --                                         vo_plazor);
         
         PQ_CR_RNG_REPROG_HS.sp_cr_plazo_residual_2024(instancia,
                                                       vo_plazor);
          BEGIN
      ...
      PQ_CR_RNG_REPROG_HS.sp_cr_gracia_caj(instancia,
                                           vo_gracia,
                                           vo_monto );
      BEGIN
        SELECT F.TP1NRO1
      ...
      --------------------------------------------------                                     
      --VALIDAR GRADIENTE
      PQ_CR_RNG_REPROG_HS.sp_cr_gradiente_caj(instancia,
                          vo_gradiente);
      BEGIN
        SELECT F.TP1NRO1
          INTO vi_cargo_grad
      ...
      BEGIN
        ...
        WHERE A.JAQA400AI1 = instancia
          AND A.JAQA400FEC = (SELECT MAX(B.JAQA400FEC)
                              FROM JAQA400 B
  ...
  END;
  PROCEDURE SP_CRD_ARBOL_APROBACION_CAJA2024(codigo in number,instancia in number,vo_cargo out number,vo_perfil out varchar)is 
    vo_plazoh number(4);
    vo_plazoc number(4);
    vo_gracia number(4);
    vo_plazor number(4);
  ...
  BEGIN
      ...
      IF vi_perfil_pzo='JZON01' or vi_perfil_grac='JZON01' or vi_perfil_grad='JZON01' or vi_perfil_mont='JZON01' then
         vo_perfil:= 'JZON01';
      end if;
      IF vi_perfil_pzo='GREG01' or vi_perfil_grac='GREG01' or vi_perfil_grad='GREG01' or vi_perfil_mont='GREG01' then
         vo_perfil:= 'GREG01';
      end if;
      IF vi_perfil_pzo='GCRE01' or vi_perfil_grac='GCRE01' or vi_perfil_grad='GCRE01' or vi_perfil_mont='GCRE01' then
         vo_perfil:= 'GCRE01';
      end if;
  END;
  PROCEDURE SP_CR_Tasa_Interes(P_Emp       in number,
  ...
  BEGIN  
    ...
    BEGIN
      SELECT AQPB556INST
      INTO lv_instancia
      FROM AQPB556 WHERE AQPB556INST = PN_INST AND AQPB556EHAB='H' AND AQPB556EST='P';   
    EXCEPTION
      WHEN no_data_found THEN
        PN_CODE := 1;
    ...
    IF PN_CODE = 0 THEN
      BEGIN 
        SELECT TP1DESC  
        INTO PN_USR
        FROM fst198 WHERE TP1COD=1 AND TP1COD1=10899 AND TP1CORR1=400000 AND TP1CORR2=10 AND TP1NRO1=PN_INST;
      EXCEPTION
        WHEN no_data_found THEN
          PN_CODE := 4;
  ...
  BEGIN
       IF  VE_AQPB556VAR = 'TASA_PROPUESTA' THEN
           BEGIN
                 SELECT TRIM(J.JAQA400AI2) INTO VI_TASAPROPUESTA FROM JAQA400 J, AQPB556 T 
                       WHERE J.JAQA400EMP = T.aqpb556emp
                         AND J.JAQA400MOD = T.aqpb556mod
                         AND J.JAQA400SUC = T.aqpb556suc
                         AND J.JAQA400MDA = T.aqpb556mda
             ...
             END;
           BEGIN
             PQ_CR_REPROG_SIN_CAP.SP_OBTENER_TASA_CAJA(
                                                        VE_AQPB556COD ,
                                                        VE_AQPB556INST ,
                                                        vi_nuevaTasa  ,
                                                        vi_preduccion  ,
  ...
  PROCEDURE SP_CRD_ARBOL_APROBACION_RNG(instancia in number,vo_ind_error out varchar2,vo_tip_error out varchar2)is 
    vo_plazoh number(4);
    vo_plazoc number(4);
    vo_gracia number(4);
    vo_plazor number(4);
      ...
      BEGIN
          select     j.jaqa400emp,
               ...
               where j.jaqa400ai1 = instancia
                 and j.jaqa400fec = (select max(jaqa400fec) from jaqa400 where jaqa400ai1=instancia);
      EXCEPTION 
        WHEN OTHERS THEN
          NULL;
      END;
  ...
  BEGIN
    ...
    BEGIN
       SELECT F.FACILIDAD,F.NUEVATASA,F.PORCENTAJEREDUCCION,F.MONTOCAPITALIZACION,F.FACILIDAD
         INTO ve_TpFndo,ve_nuevaTasa,ve_preduccion,ve_montocap,ve_facilidad
...
END;                                          
  ...
  BEGIN
       ...
       BEGIN
            ...
            WHERE A.AQPB556EMP = ve_pgcod
              ...
              AND AQPB556EHAB  = 'H'
              AND A.AQPB556FECR = (SELECT MAX(AQPB556FECR) FROM AQPB556 A
                                                          WHERE A.AQPB556EMP = ve_pgcod
                                                            AND A.AQPB556MOD = ve_mod
                                                            AND A.AQPB556SUC = ve_suc
                                                            AND A.AQPB556MDA = ve_mda
  ...
  cursor lista_jaqz698 is
      select t.JAQZ698FEP, t.JAQZ698COR, 
             t.JAQZ698EMP, t.JAQZ698MOD, t.JAQZ698SUC, 
             t.JAQZ698MDA, t.JAQZ698PAP, t.JAQZ698CTA,
             t.JAQZ698OPE, t.JAQZ698SBO, t.JAQZ698TOP
        from jaqz698 t
  ...
  BEGIN
     ...
     EXCEPTION
       WHEN OTHERS THEN
         ...
         PQ_CR_REPROG_SIN_CAP.SP_GRABAR_LOG_ERRORES(VE_INSTANCIA,'','PQ_CR_REPROG_SIN_CAP','SP_CRD_LOG_APROB_AQPD157','',VO_COD_ERROR,VO_MSG_ERROR);
     ...
     IF VI_ESTADO IS NOT NULL AND VI_ESTADO = 'P' THEN
        BEGIN
          UPDATE AQPD157 B 
             SET B.AQPD157MPCORR = VE_MENSAJE,
                 B.AQPD157LOGPAPR= VE_CORREOS
  ...
  BEGIN
    ...
    BEGIN
       SELECT A.AQPB556TPRG,A.AQPB556FECRPG,A.AQPB556SUC,A.AQPB556USRR,ABS(A.AQPB556SCAP)
    ...
    EXCEPTION
      WHEN OTHERS THEN
        VO_COD_ERROR := '0001';
        VO_MSG_ERROR := SUBSTR(SQLERRM,1,150);
        PQ_CR_REPROG_SIN_CAP.SP_GRABAR_LOG_ERRORES(VE_INSTANCIA,'','PQ_CR_REPROG_SIN_CAP','SP_CARGAR_AUTORIZANTES','',VO_COD_ERROR,VO_MSG_ERROR);
        VO_MSG_ERROR := 'No se encontro arbol de autorización';
    END;
    --CARGAR LA LISTA DE AUTORIZANTES EN TABLA LOCAL.
    BEGIN
      BEGIN
          UPDATE AQPD157 A SET A.AQPD157EST = 'N'
           WHERE A.AQPD157CODREP = VE_AQPB556COD AND A.AQPD157INST = VE_INSTANCIA AND AQPD157FECAPRO = VI_FECHARPG;
        EXCEPTION 
          WHEN OTHERS THEN
            NULL;
      END;
      FOR X IN LISTA_CLIENTES_AUTORIZANTES(VI_TIPORPG,VI_SALDOCAP) LOOP
          ...
          IF VI_CARGO <= X.AQPC722CCARG THEN
             ...
             BEGIN
               UPDATE AQPB556 A 
                  SET A.AQPB556CARA = VI_CARGO,
                      A.AQPB556USRA = vio_aprobador
                WHERE A.AQPB556COD  = VE_AQPB556COD
                  AND A.AQPB556INST = VE_INSTANCIA
          ...
          END IF;
          BEGIN
            INSERT INTO AQPD157(AQPD157FECAPRO,AQPD157CODREP,AQPD157INST,AQPD157TIPREP,AQPD157NIAP,AQPD157NIDP,
                                AQPD157CODCAR,AQPD157PERUS,AQPD157UAPRO,AQPD157EST,AQPD157UREG,
                                AQPD157FECREG,AQPD157HORREG,AQPD157REQ)
            VALUES (VI_FECHARPG,VE_AQPB556COD,VE_INSTANCIA,VI_TIPORPG,X.AQPC722NAPR,X.AQPC722NDAPR,
                    X.AQPC722CCARG,X.AQPC722PCARG,vio_aprobador,'P',VI_ANALISTA,
                    TO_DATE(SYSDATE,'DD/MM/RRRR'),TO_CHAR(SYSDATE,'HH:Mi:SS'),X.AQPC722REQ);
            COMMIT;
          EXCEPTION
            WHEN OTHERS THEN
              VO_COD_ERROR := '0000';
  ...
  BEGIN
      ...
      BEGIN
        SELECT A.AQPB556FECRPG,A.AQPB556EMP,A.AQPB556MOD,A.AQPB556SUC,A.AQPB556MDA,
               A.AQPB556PAP,A.AQPB556CTA,A.AQPB556OPER,A.AQPB556SBOP,A.AQPB556TOP
        INTO   VI_FECHA_RPG,VI_EMP,VI_MOD,VI_SUC,VI_MDA,
               VI_PAP,VI_CTA,VI_OPER,VI_SBOP,VI_TOP
        FROM AQPB556 A
        WHERE A.AQPB556COD  = VE_AQPB556COD
          AND A.AQPB556INST = VE_INSTANCIA;
      EXCEPTION
        WHEN OTHERS THEN
              VO_COD_ERROR := '0001';
              VO_MSG_ERROR := SUBSTR(SQLERRM,1,150);
              PQ_CR_REPROG_SIN_CAP.SP_GRABAR_LOG_ERRORES(VE_INSTANCIA,'','PQ_CR_REPROG_SIN_CAP','sp_obtener_data_jaqa400','',VO_COD_ERROR,VO_MSG_ERROR);
        ...
        WHEN NO_DATA_FOUND THEN
           ...
           EXCEPTION
            WHEN OTHERS THEN
              ...
              PQ_CR_REPROG_SIN_CAP.SP_GRABAR_LOG_ERRORES(VE_INSTANCIA,'','PQ_CR_REPROG_SIN_CAP','SP_GRABAR_APROBACION','',VO_CODERROR,VO_MSGERROR);
          ...
          IF (TRIM(VI_USR_APROB)) IS NOT NULL OR  VI_USR_COD_CARGO > 0 THEN
                ...
                BEGIN
                    UPDATE AQPD157 A SET 
                     ...
                     BEGIN
                       ...
                       PQ_CR_REPROG_SIN_CAP.SP_ENVIAR_CORREO_PAPR(
                                                                  VE_FECHAREG,
                                                                  VE_CODREPROG,
                                                                  VE_INSTANCIA,
                                                                  VE_ESTADO,
             ...
             ELSE 
               IF VI_USR_ESTADO = 'P' THEN
                  VO_CODERROR := '0004';
                  VO_MSGERROR := 'No puede autorizar esta pendiente la autorización de '||VI_USR_APROB||' - '||VI_USR_COD_CARGO;
               END IF;
             END IF;  
          END IF;
        ELSE -- EN CASO NO TENGA DEPENDENCIAS DIRECTAMENTE SE GESTIONA LA DESICIÓN DEL AUTORIZANTE
          --SE  ACTUALIZA LAS CONDIONES DE LA AUTORIZACION DEL CREDITO A REPROGRAMAR
          BEGIN
        ...
        BEGIN
          ...
          IF VE_ESTADO = 'R' THEN
             BEGIN
               ...
               UPDATE AQPB556 A5 SET A5.AQPB556FECUAT=to_DATE(sysdate,'dd/mm/rrrr'), A5.AQPB556HORUAT=to_char(sysdate,'hh:mi:ss'),A5.AQPB556ESTAUT ='R', A5.AQPB556EST='R'
               WHERE A5.AQPB556COD = VE_CODREPROG AND A5.AQPB556INST= VE_INSTANCIA  AND A5.AQPB556FECRPG = VE_FECHAREG AND A5.AQPB556EST='P' AND A5.AQPB556EHAB='H';               
               --ACTUALIZO EL ESTADO DE TABLA PRINCIPAL DE ANALISTA PARA QUE VUELVA A SOLICITUD
               UPDATE JAQA400 J SET J.JAQA400EST='S'
          ...
          IF vi_total_autorizantes = vi_total_autorizaciones THEN
             BEGIN
               ...
               UPDATE AQPB556 A5 SET A5.AQPB556FECUAT=to_DATE(sysdate,'dd/mm/rrrr'), A5.AQPB556HORUAT=to_char(sysdate,'hh:mi:ss'),A5.AQPB556ESTAUT ='A'
               WHERE A5.AQPB556COD = VE_CODREPROG AND A5.AQPB556INST= VE_INSTANCIA  AND A5.AQPB556FECRPG = VE_FECHAREG AND A5.AQPB556EST='P' AND A5.AQPB556EHAB='H';
  ...
  BEGIN
      BEGIN
        ...
        INSERT INTO AQPC716(AQPC716FECR,AQPC716inst,
                            aqpc716prg,aqpc716pqt,aqpc716prd,aqpc716peje,
                            aqpc716err,aqpc716msge) VALUES
                            (VI_FECHA,VE_INSTANCIA,
                             VE_PROGRAMA,VE_PAQUETE,VE_PROCED,VE_PEJE,
                             VE_ERR,VE_MSG);
  ...
  BEGIN
    BEGIN
      ...
      --SE OBTIENE LOS DATOS DE LA GESTION
        ...
        BEGIN
          PQ_CR_REPROG_SIN_CAP.SP_VALIDAR_TIPO_REPROGD( 
                                                        VE_CODREPROG,
                                                        VE_INSTANCIA,
                                                        VOI_N_COD_TRPG,
                                                        VOI_V_DSC_TRPG
        ...
        BEGIN
          SELECT B.AQPB556CTA,B.AQPB556OPER,ABS(B.AQPB556SCAP),F.MOSIGN,D.PENOM,W.WFUSREMAIL          
            INTO VI_CUENTA, VI_OPERACION,VI_CAPITAL,VI_MONEDA,VI_CLIENTE,VI_MAIL_ANALISTA
        ...
        EXCEPTION
          WHEN OTHERS THEN
            VO_CODERROR := '0002';
            VO_MSGERROR := SUBSTR(SQLERRM,1,150);
            PQ_CR_REPROG_SIN_CAP.SP_GRABAR_LOG_ERRORES(VE_INSTANCIA,'','PQ_CR_REPROG_SIN_CAP','SP_ENVIAR_CORREO_PAPR','',VO_CODERROR,VO_MSGERROR);
      ...
      BEGIN
          LV_REMITENTE := 'notificacionesbantotal@cajaarequipa.pe';
          LV_ASUNTO    := 'APROBACION DE REPROGRAMACION CLIENTE ' || upper(trim(VI_CLIENTE));
          VI_DATOS     := 'cuenta ' || trim(to_char(VI_CUENTA)) || ' y operación ' ||
                          trim(to_char(VI_OPERACION)) || ' por ' || trim(VI_MONEDA) ||
          ...
          IF VE_ESTADO = 'A' THEN 
            -----
            BEGIN
              PQ_CR_REPROG_SIN_CAP.SP_OBTENER_SGTE_APR(
                                                        VE_FECHAREG,
                                                        VE_CODREPROG,
                                                        VE_INSTANCIA,
                                                        VO_USUARIO_APROB,
            ...
            BEGIN
              SELECT A.AQPD157CODCAR, A.AQPD157EST, A.AQPD157CMTA, S.SNG055DSC
                INTO VI_USR_COD_CARGO, VI_USR_ESTADO, VI_COMENTARIO,VI_CARGO_APR
            ...
            lv_mensaje    := '<p "font-family: Arial, sans-serif; font-size: 14px;">Estimado(s) ' ||
                             trim(VI_CARGO_APR) || ',</p>' ||
                             '<p "font-family: Arial, sans-serif; font-size: 14px;">Tiene pendiente de aprobación la reprogramación '||VOI_V_DSC_TRPG||' sin CRM de ' ||
                             trim(VI_DATOS);
            VE_MENSAJE_LOG:='Estimado(s)'||trim(VI_CARGO_APR)||','||'Tiene pendiente de aprobación la reprogramación '||VOI_V_DSC_TRPG||' sin CRM de '||trim(VI_DATOS);
            --OBTENER EL CORREO DEL APROBADOR.
            BEGIN
              SELECT W.WFUSREMAIL INTO lv_destinos_c FROM WFUSERS W WHERE W.WFUSRCOD=RPAD(TRIM(VE_USR_APROBR),30,' ');
            EXCEPTION
              WHEN OTHERS THEN
                NULL;  
            END;            
            --lv_destinos   := ;
            LV_DESTINOS_C := TRIM(lv_destinos_c)||';'||TRIM(VI_MAIL_ANALISTA);
          END IF;
          IF VE_ESTADO = 'AAA' THEN 
            ...
            lv_mensaje    := '<p "font-family: Arial, sans-serif; font-size: 14px;">Estimado Analista se realizaron todas las Aprobaciones, '||
                             'de la reprogramación por '||VOI_V_DSC_TRPG||' sin CRM de ' ||
                             trim(VI_DATOS) || '<p "font-family: Arial, sans-serif; font-size: 14px;">El cliente debe acercarse en ventanilla, para que se genere la reprogramación y recoger su nuevo cronograma';
            VE_MENSAJE_LOG:='Estimado Analista se realizaron todas las Aprobaciones'||'de la reprogramación por '||VOI_V_DSC_TRPG||'l sin CRM de'||
            ...
            BEGIN
              PQ_CR_REPROG_SIN_CAP.SP_OBTENER_CORREOS_AQPD157(
                                                              VE_FECHAREG,
                                                              VE_CODREPROG,
                                                              VE_INSTANCIA,
                                                              VO_CORREOS_APROB,
      ...
      BEGIN
        ...
        ---------------------------------------
        PQ_CR_REPROG_SIN_CAP.SP_LOG_CORREO(
                                            VE_FECHAREG,
                                            VE_CODREPROG,
                                            VE_INSTANCIA,
                                            VE_ESTADO,
             ...
             */
             ...
             if vi_contador = 0 then
                ...
                if lv_correog is not null then
                  VO_CORREOS_APROB := TRIM(VO_CORREOS_APROB)||';'||TRIM(lv_correog);
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
          VO_CODERROR := '0001';
          VO_MSGERROR := SUBSTR(SQLERRM,1,150);
          PQ_CR_REPROG_SIN_CAP.SP_GRABAR_LOG_ERRORES(VE_INSTANCIA,'','PQ_CR_REPROG_SIN_CAP','SP_OBTENER_SGTE_APR','',VO_CODERROR,VO_MSGERROR);
  ...
  PROCEDURE SP_LOG_CORREO(
   ...
   BEGIN
     ...
     BEGIN
       IF VE_ESTADO = 'A' THEN
         UPDATE AQPD157 A
            SET A.AQPD157LOGPAPR = 'para: '||TRIM(lv_destinos)||'***con copia:'||TRIM(lv_destinos_c),
       ...
       IF VE_ESTADO = 'AAA' THEN
          UPDATE AQPD157 A
            SET A.AQPD157LOGAPR = 'para: '||TRIM(lv_destinos)||'***con copia:'||TRIM(lv_destinos_c),
     ...
     EXCEPTION
       WHEN OTHERS THEN
         VO_CODERROR := '0001';
         VO_MSGERROR := SUBSTR(SQLERRM,1,150);
         PQ_CR_REPROG_SIN_CAP.SP_GRABAR_LOG_ERRORES(VE_INSTANCIA,'','PQ_CR_REPROG_SIN_CAP','SP_LOG_CORREO','',VO_CODERROR,VO_MSGERROR);
         VO_MSGERROR := 'No se pudo enviar el correo pero si se llego a gestionar';
     END;
   END;
   PROCEDURE SP_VALIDAR_TIPO_REPROGD( VIE_N_COREPROG IN NUMBER,
