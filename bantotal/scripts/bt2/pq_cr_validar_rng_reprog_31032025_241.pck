  ...
  begin
    ...
    IF LENGTH(VI_MENSAJE_CRM) = 0 or VI_MENSAJE_CRM IS NULL THEN
      ...
      ELSE
        ...
        BEGIN
          BEGIN
              PQ_CR_VALIDAR_RNG_REPROG.SP_BLOQUEO_CREDINKA(
                                                            VE_INSTANCIA,
                                                            '',
                                                            VE_RPTAC,
                                                            VO_CODERROR,
        ...
        BEGIN
          BEGIN
              PQ_CR_CTROL_OPI_RIE_REPROG_REFIN.SP_CNTROL_REPROG
                                                          (
                                                            VE_INSTANCIA,
                                                            '',
                                                            VE_RPTAC,
          ...
          ELSE
            IF LENGTH(TRIM(VE_MENSAJE)) > 0 and LENGTH(TRIM(VE_MENSAJE)) < 700 THEN
              VE_MENSAJE := VE_MENSAJE || ';' ||
                            'RSC: Requiere opinion de riesgos - '||VO_MSGERROR;
            ELSE
              ve_mensaje := 'RSC: Requiere opinion de riesgos - '||VO_MSGERROR;              
            END IF;
            RETURN;
          END IF;
        EXCEPTION
