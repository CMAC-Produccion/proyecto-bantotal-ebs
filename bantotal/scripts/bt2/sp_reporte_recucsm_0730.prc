create or replace procedure SP_REPORTE_RECUCSM_0730 (c_destinatario in varchar2) IS
  cursor C_UCSM3 is
  ...
  if v_found =1 then
    ...
    INSERT INTO REPUCSM3_3
      select 'T' tipo,
             ...
             to_char(sysdate - 1, 'yyyy/mm/dd') fecha,
             count(*) registros,
    ...
    v_Subject   := 'Cobros_'||to_char(sysdate-1, 'yymmdd')||'_3.538';--Cobros_160727_3.538
...
--    V_Conexion    := UTL_SMTP.OPEN_CONNECTION(C_SMTP_SERVER, 2528);
    ...
    msg           := 'Date: ' ||to_char(sysdate-1, 'Dy, DD Mon YYYY hh24:mi:ss') ||utl_tcp.crlf ||
    ...
    utl_smtp.write_data(V_Conexion,
                        ...
                        utl_tcp.crlf || utl_tcp.crlf || 'Adj. Cobros_'||to_char(sysdate-1, 'yymmdd')||'_3.538' || utl_tcp.crlf); --Message Body
    utl_smtp.write_data(V_Conexion,
                        '-------SECBOUND' || utl_tcp.crlf ||
                        --'Content-Type: text/plain;' || utl_tcp.crlf);--
                        'Content-Type: text/plain; charset=iso-8859-1' || utl_tcp.crlf);
    utl_smtp.write_data(V_Conexion,
                        ' name='|| 'Cobros_'||to_char(sysdate-1, 'yymmdd')||'_3.538' || utl_tcp.crlf); --TXT
    utl_smtp.write_data(V_Conexion,
                        'Content-Transfer_Encoding: 8bit' ||--8bit
                        utl_tcp.crlf);
    utl_smtp.write_data(V_Conexion,
                        ...
                        utl_tcp.crlf);
    utl_smtp.write_data(V_Conexion,
                            ' filename='|| 'Cobros_'||to_char(sysdate-1, 'yymmdd')||'_3.538'|| utl_tcp.crlf ||
                            utl_tcp.crlf);
