create or replace package pq_cr_reprog_caja is
  ...
  PROCEDURE SP_OBTENER_MONTO(v_Pgcod  in number,
                           ...
                           v_Sctope in number,
                           vo_saldo out number);
PROCEDURE SP_VALIDAR_CAJA_RT(v_Pgcod  in number,
                             v_Scmod  in number,
                             v_Scsuc  in number,
                             v_Scmda  in number,
                             v_Scpap  in number,
                             ...
                             v_Scsbop in number,
                             v_Sctope in number,
                             vo_validar out varchar);
                                                   
end pq_cr_reprog_caja;
/

create or replace package body pq_cr_reprog_caja is
...
procedure sp_calcular_monto(ve_instancia number,
  ...
  v_ttas   number(17,6);
  vi_gob   varchar2(1);
  VI_TEMP NUMBER(10); --BORRAR TEMPORAL
...
begin
       ...
       */
       BEGIN
           SELECT AQPB556TSAA,a.aqpb556TSAP,A.AQPB556COD
           INTO vi_tea,vi_tea_r,VI_TEMP
           FROM AQPB556 A
           WHERE A.AQPB556INST = ve_instancia
            ...
            AND A.AQPB556COD = (SELECT MAX(T.AQPB556COD) FROM  AQPB556 T 
                                                        WHERE T.AQPB556INST = ve_instancia
...
*/    
    ...
    BEGIN
      --OBTENER LA TASA CREDITO VIGENTE
       pq_cr_reprog_caja.sp_cr_tasa(v_pgcod => v_pgcod,
                                v_scmod => v_scmod,
                                v_scsuc => v_scsuc,
                                v_scmda => v_scmda,
                                v_scpap => v_scpap,
    ...
    BEGIN
      ...
      WHERE AQPB556INST = v_instancia
        ...
        --AND AQPB556EST  = 'P'
        AND AQPB556EHAB = 'H'
        AND AQPB556FECR = (SELECT MAX(AQPB556FECR) FROM AQPB556  WHERE AQPB556INST = v_instancia
        AND AQPB556EMP  = v_Pgcod  
        AND AQPB556MOD  = v_Scmod  
        AND AQPB556SUC  = v_Scsuc  
        AND AQPB556MDA  = v_Scmda  
...
BEGIN
      ...
      BEGIN --CONSULTAR TABLA 
        ...
        WHERE F.AQPA840EMP = v_Pgcod
          ...
          AND F.AQPA840TIP IN (1,5);   --TIPO DE REPROGRAMACION CAJA 1 REDUCCION TASA 5 SOL. JUNTOS
        -- caja
       EXCEPTION
         WHEN OTHERS THEN
           vo_saldo:=0; 
      ...
      ln_tipo := 0; 
         ...
         SELECT  1           
          ...
          WHERE /*L.ESTADOSOLICITUD = 'BT' 
              ...
              AND (F.facilidad like 'Redu%' OR UPPER(F.facilidad) like '%JUNTOS%') 
