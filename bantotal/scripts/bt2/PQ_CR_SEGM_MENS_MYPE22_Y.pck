create or replace package PQ_CR_SEGM_MENS_MYPE22_Y is
  -- *****************************************************************
...
end PQ_CR_SEGM_MENS_MYPE22_Y;
/
create or replace package body PQ_CR_SEGM_MENS_MYPE22_Y is
  -- *****************************************************************
  -- Nombre                     : PQ_CR_SEGM_MENS_MYPE22
  -- Sistema                    : BANTOTAL
  -- Módulo                     : Creditos
  ...
  procedure sp_job_carga(P_D_FECPRO IN DATE,
    ...
    select substr(trim(j.AQPD783ndoc), -1, 1) digito
      from AQPD783 j
     group by substr(trim(j.AQPD783ndoc), -1, 1);*/
  
    lc_fecpro   varchar2(10);
    lc_variable varchar2(4000);
    ln_job      number := 0;
    lc_hostname varchar2(64);
    --l_AQPD783pano  AQPD783.AQPD784pano%type;
    --l_AQPD783pmes  AQPD783..AQPD784pmes%type;
    ln_cont   number(2) := 0;
    ln_inst   number(1) := 1;
  ...
  BEGIN
    ...
    begin
      delete from AQPD785 a
       where a.AQPD785fech = P_D_FECPRO
         and a.AQPD785ndoc = P_N_DOC;
      commit;
    exception
      when others then
        null;
    ...
    begin
      PQ_CR_SEGM_MENS_MYPE22_y.Sp_Carga_data(P_D_FECPRO,
                                             P_N_JOBS,
                                             PC_USR,
                                             P_N_DOC);
    end;
    ...
    /*
      for i in 0 .. P_N_JOBS loop
        ...
        lc_variable := ' begin ' ||
                       ' PQ_CR_SEGM_MENS_MYPE22_y.Sp_Carga_data(TO_DATE(''' ||
        ...
        BEGIN
          SELECT count(*)
           ...
           WHERE upper(x.what) LIKE
                 '%PQ_CR_SEGM_MENS_MYPE22_Y.SP_CARGA_DATA%';
        EXCEPTION
          WHEN OTHERS THEN
            ln_numjob := 0;
        END;
  ...
  Procedure Sp_Carga_data(pd_fecpro  in date,
    ...
    cursor c_clientes(p_fecpro date) is
      select l.AQPD783pais, l.AQPD783tdoc, l.AQPD783ndoc
        from AQPD783 l
       where not exists (select 1
                from AQPD785 c
               where c.AQPD785pais = l.AQPD783pais
                 and c.AQPD785tdoc = l.AQPD783tdoc
                 and c.AQPD785ndoc = l.AQPD783ndoc
                 and c.AQPD785ndoc = pc_docu
                 and c.AQPD785fech = pd_fecpro)
            --and AQPD783NUME2 = P_N_DIGITO
         and l.AQPD783ndoc = pc_docu;
    ...
    TYPE tp_pais IS TABLE OF AQPD783.AQPD783pais%type INDEX BY BINARY_INTEGER;
    TYPE tp_tdoc IS TABLE OF AQPD783.AQPD783tdoc%type INDEX BY BINARY_INTEGER;
    TYPE tp_ndoc IS TABLE OF AQPD783.AQPD783ndoc%type INDEX BY BINARY_INTEGER;
  
    la_AQPD783pais tp_pais;
    la_AQPD783tdoc tp_tdoc;
    la_AQPD783ndoc tp_ndoc;
  
    --cursor creditos is
    --select * from AQPD783 a
    --where a.AQPD783ndoc = '29524923'
  ...
  begin
    ...
    FETCH c_clientes BULK COLLECT
      INTO la_AQPD783pais, la_AQPD783tdoc, la_AQPD783ndoc;
  
    IF la_AQPD783ndoc.count > 0 THEN
      FOR c IN la_AQPD783ndoc.FIRST .. la_AQPD783ndoc.LAST LOOP
      
        --for i in creditos loop
        BEGIN
          PQ_CR_SEGM_MENS_MYPE22.Sp_cr_Resolutores( /*i.AQPD783pais*/la_AQPD783pais(c),
                                                   /*i.AQPD783tdoc*/
                                                   la_AQPD783tdoc(c),
                                                   /*i.AQPD783ndoc*/
                                                   la_AQPD783ndoc(c),
        ...
        END;
        BEGIN
          PQ_CR_SEGM_MENS_MYPE22.Sp_Resolutores_NS( /*i.AQPD783pais*/la_AQPD783pais(c),
                                                   /*i.AQPD783tdoc*/
                                                   la_AQPD783tdoc(c),
                                                   /*i.AQPD783ndoc*/
                                                   la_AQPD783ndoc(c),
        ...
        BEGIN
          pQ_cr_segmentacion_PYME_NUEVAR.SP_CR_SEGM_ANT(la_AQPD783pais(c),
                                                        la_AQPD783tdoc(c),
                                                        la_AQPD783ndoc(c),
                                                        0,
                                                        ln_segpymeant);
        ...
        if ln_afecta = 1 then
          lv_ndocum := trim(la_AQPD783ndoc(c));
          begin
          ...
          begin
            PQ_CR_VAR_SEGMENTACION_CONG_RP.sp_cr_variables_cong(la_AQPD783pais(c),
                                                                la_AQPD783tdoc(c),
                                                                lv_ndocum,
                                                                1,
                                                                ln_afdatr,
        ...
        begin
          pq_cr_segmentacion_credinka.sp_antiguedad_rcc_cli(la_AQPD783pais(c),
                                                            la_AQPD783tdoc(c),
                                                            lv_ndocum,
                                                            ln_anticredinka,
                                                            ln_coder,
        ...
        lc_JAQZ082VAR16 := 'REL_CREDI_MAS_CDK' || '=' ||
                           trim(to_char(ln_anticredinka)); -- MHUAMANIA / Nueva variable 26/12/2024
