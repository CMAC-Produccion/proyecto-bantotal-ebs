create or replace package PQ_CV_MONITOREO_ACF_HB is
  -- ------------------------------------------------------------------------------------------------
...
end PQ_CV_MONITOREO_ACF_HB;
/

create or replace package body PQ_CV_MONITOREO_ACF_HB is
  -- ------------------------------------------------------------------------------------------------
  -- Nombre                : PQ_CV_MONITOREO_ACF_HB
  -- Sistema               : BANTOTAL
  -- Módulo                : HOMEBANKING - HB
  ...
  *******************************************************************************************/
  ...
  begin
    --//
    select substr(b.x9996drqcl, 1, 16)
      into lc_cmp011
      from x9996d b
     where x9996drpgc = pn_pgcod
       and x9996drsuc = pn_itsuc
       and x9996drmod = pn_itmod
       and x9996drtrn = pn_ittran
       and x9996drrel = pn_itnrel
       and x9996acnco = 5
       and rownum = 1;
  ...
  begin
    ...
    if ln_codmda = 0 then
      ...
      lc_cmp015 := lpad(trim(replace(replace(to_char(ln_conver, '9999999D99'), ',',''), '.','')), 14, '0');
    else
      lc_cmp015 := lpad(trim(replace(replace(to_char(ln_aux004, '9999999D99'), ',',''), '.','')), 14, '0');
    end if;
  ...
  begin
    --//     
    select lpad(trim(replace(replace(to_char(a.itimp1, '9999999D99'), ',', ''), '.','')), 14,
                '0')
      into lc_cmp016
      from fsd016 a
     where a.pgcod = pn_pgcod
  ...
  begin
    ...
    if ln_codmda = 0 then
      ln_conver := ln_aux004 / ln_tipcam;
      lc_cmp017 := lpad(trim(replace(replace(to_char(ln_conver, '9999999D99'), ',',''), '.','')), 14, '0');
    else
      lc_cmp017 := lpad(trim(replace(replace(to_char(ln_aux004, '9999999D99'), ',',''), '.','')), 14, '0');
    end if;
    --//               
    return lc_cmp017;
  exception
  ...
  function fn_trama_indice042(pn_pgcod  in number,
                              ...
                              pn_itord  in number,
                              pn_itsbor in number) return varchar2 is
    lc_cmp042 varchar2(150) := 'CAJA HOME BANKING';
  begin
    return lc_cmp042;
  exception
    when others then
