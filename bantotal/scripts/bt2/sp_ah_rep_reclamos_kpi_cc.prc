CREATE OR REPLACE PROCEDURE SP_AH_REP_RECLAMOS_KPI_CC(P_USER IN VARCHAR
...
, P_ERRMSG        OUT VARCHAR
  ...
  ---***
  ---*** COMO HAY FECHA CORTE INCLUIMOS LOS RESPONDIDOS
  ...
  LOOP
  -- SI SELECCIONO EN PROCESO PERO NO RESPONDIDOS
  ...
  END;
  ---***
  ---*** ESTADO DEL RECLAMO, FECHA DE RESPUESTA, TIPO SOLUCION, DIAS ATENCION
  ld_FECRES := XROW.JAQL420FCC;
  IF(XROW.JAQL420ESR = 3 AND XROW.JAQL420FCC > P_FECCORTE) THEN
    lc_AQPB520ESTDES := 'EN PROCESO';
    ld_FECRES := NULL;
    lc_AQPB520TIPSOL := 'EN TRAMITE';

    ln_AQPB520DIAATE := (P_FECCORTE - XROW.JAQL420FCR) + 1;
    ln_AQPB520DIAABS := ln_AQPB520DIAATE;

    XROW.JAQL420CAR := NULL;
    XROW.JAQL420CMS := NULL;
  ...
  , XROW.JAQL420FCR, XROW.JAQL420ESR, lc_AQPB520ESTDES, ld_FECRES, XROW.JAQL420AGE
  , lc_AQPB520AGEDES, lc_AQPB520RECREG, P_FECCORTE, ln_AQPB520DIAATE, ln_AQPB520DIAABS
