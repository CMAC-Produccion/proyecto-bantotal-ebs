create or replace package PQ_DM_PUNTAJE_RIESGO is

  -- Author  : PVARGAS
  -- Created : 27/02/2019 11:10:40 a.m.
  -- Purpose : Cargar en Bntotal el puntaje de riesgo calculado en DM
  -----------------------------------------------------------------------
  -- Modificaciones
  -----------------------------------------------------------------------
  -- Autor  : Paola Vargas
  -- Fecha  : 2019.03.30
  -- Cambio : Cargar en enero y junio el puntaje de riesgo de todos los 
  --          clientes, los otros meses sólo medio, alto y muy alto
  -----------------------------------------------------------------------
  -- Autor  : Paola Vargas
  -- Fecha  : 2026.01.07
  -- Cambio : Incluir en nombre del job en 'DM_PTJE_RIESGO_' en el control
  --          máximo de jobs generados (300)
  -----------------------------------------------------------------------
  
  Procedure SP_JOBS_PUNTAJE_RIESGO(pc_fecpro in varchar2);
  Procedure SP_OCUM_PUNTAJE_RIESGO(PV_FECPRO In Varchar2,
                                   PN_NIVRIE In Number,
                                   PN_GEOGRA In Number
                                  );

end PQ_DM_PUNTAJE_RIESGO;
/
create or replace package body PQ_DM_PUNTAJE_RIESGO is

  Procedure SP_JOBS_PUNTAJE_RIESGO (pc_fecpro in varchar2)
  -- Author  : PVARGAS
  -- Created :  2019.03.30
  -- Purpose : JOBS de carga de puntaje de riesgo
  -----------------------------------------------------------------------
  -- Modificaciones
  -----------------------------------------------------------------------
  -- Autor  : Paola Vargas
  -- Fecha  : 2020.08.06
  -- Cambio : Excluir sólo codigos de sucursales entre 800 y 899
  -----------------------------------------------------------------------
  Is
    Cursor c_nivrie 
        Is select nivrie_key from dm_pr_nivel@datawh;
    
    Cursor c_geogra
        Is select geografia_key 
             from dm_geografia@datawh where tipo_region = 'R' 
              and (codigo_agencia not between 800 and 899); 
              --and (codigo_agencia not between 900 and 903)  
              --and (codigo_agencia not between 905 and 999);
              
    ld_fecpro date :=  last_day(to_date( pc_fecpro,'yyyymmdd')); --last_day(add_months(trunc(sysdate),-2));  --Cambiar luego a menos un mes
    lv_Fecpro varchar2(10) :=  pc_fecpro; --to_char(ld_fecpro,'rrrrmmdd');   
    ln_mjob   number(10);
    ln_job    number(10) := 0;
  ...
  Begin
       ...
       Where exists (select 1 from jaqy474 c 
                      where c.jaqy474pais=d.jaqy474pais
                        and c.jaqy474tdoc=d.jaqy474tdoc 
                        and c.jaqy474ndoc=d.jaqy474ndoc
                        and c.jaqy474nreg=d.jaqy474nreg
                        and c.jaqy474tpr = 2 
                        and c.JAQY474PER = ld_fecpro
                    );
       Commit;
       ...
       Delete From jaql140 Where jaql140fec = ld_fecpro;
       Commit;
       
       -- Genera Jobs
       For n in c_nivrie Loop
           For g in c_geogra Loop
               ln_job := ln_job + 1; 
               lv_nproc := 'Begin '||'PQ_DM_PUNTAJE_RIESGO.SP_OCUM_PUNTAJE_RIESGO('''||lv_Fecpro||
                                     ''','||n.nivrie_key||','||g.geografia_key||'); End;';
               dbms_scheduler.create_job
                  (
                   job_name=> 'DM_PTJE_RIESGO_'||LPAD(TO_CHAR(ln_job),5,'0'),
                   job_type=> 'PLSQL_BLOCK',
                   job_action=> lv_nproc,
                   start_date => sysdate+1/(24*180),
                   enabled => true, 
                   auto_drop=> TRUE,
                   comments => 'COPIA PUNTAJE RIESGO DE DM'
                  );
             
               -- CONTROLA MAXIMO 300 JOBS
               Loop
                   select count(*) into ln_mjob from dba_scheduler_jobs where job_name like 'DM_PTJE_RIESGO_%'; --user_scheduler_jobs;
                   Exit When ln_mjob <= 300;
               End Loop;
            
           End Loop;
       End Loop;
       
  End SP_JOBS_PUNTAJE_RIESGO;
  -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
  Procedure SP_OCUM_PUNTAJE_RIESGO(PV_FECPRO In Varchar2,
                                   PN_NIVRIE In Number,
                                   PN_GEOGRA In Number
  ...
  -- Autor  : Paola Vargas
  -- Fecha  : 2020.08.06
  -- Cambio : En copia semestral copiar puntaje de riesgo de los meses del último semestre
  ----------------------------------------------------------------------------------------
  Is
   
   ld_fecpro date := to_date(PV_FECPRO,'rrrrmmdd');
   ...
   ln_perini Number(6):= to_number(to_char(add_months(ld_fecpro,-6),'rrrrmm'));
...
Begin
     lv_query := 'Select c.pais,c.tipo_documento,c.numero_documento,'||
                        ...
                        'decode(x.modulo,-1,null,x.modulo),decode(x.transaccion,-1,null,x.transaccion),'||
                        'l.n_reltrx,l.ordtx,l.sortx,l.n_mayimp,l.n_ptjrie,l.nivrie_key,'||
              ...
              'Where l.nivrie_key = :1 '||
                'and l.geografia_cli_key = :2';
                
     If to_char(ld_fecpro,'MM') not in ('01','07') Then
        /*Nivel de Riesgo Medio,Alto y Muy Alto*/
        lv_query := lv_query||' and l.nivrie_key > 2 and l.ind_produ =2 and l.periodo = :3'; 
     Else
        lv_query  := lv_query||' and l.ind_produ =1 and l.periodo > '||ln_perini||' and l.periodo <= :3';
     End If;                

     Open c_ptjrie FOR lv_query Using PN_NIVRIE,PN_GEOGRA,ln_period;
     LOOP
       ...
       LIMIT 1000;            
          ...
          FOR x IN 1 .. n_pais.COUNT Loop   
             ln_numrg:=fn_corr_jaqy474(n_pais(x),n_tdoc(x),trim(c_ndoc(x)));
             Begin
             Insert into JAQY474(JAQY474pais,JAQY474tdoc,JAQY474ndoc,JAQY474nreg,
                     ...
                     VALUES(n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,
                ...
                Begin
                    ...
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,1,n_codtpe(x),n_valtpe(x),n_portpe(x),nvl(n_valtpe(x),0)*nvl(n_portpe(x),0) From dual                     
                    union all 
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,2,n_coddom(x),n_valdom(x),n_pordom(x),nvl(n_valdom(x),0)*nvl(n_pordom(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,3,n_codres(x),n_valres(x),n_porres(x),nvl(n_valres(x),0)*nvl(n_porres(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,4,n_codtdo(x),n_valtdo(x),n_portdo(x),nvl(n_valtdo(x),0)*nvl(n_portdo(x),0) From dual 
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,5,n_codsjo(x),n_valsjo(x),n_porsjo(x),nvl(n_valsjo(x),0)*nvl(n_porsjo(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,6,n_codocu(x),n_valocu(x),n_porocu(x),nvl(n_valocu(x),0)*nvl(n_porocu(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,7,n_codpre(x),n_valpre(x),n_porpre(x),nvl(n_valpre(x),0)*nvl(n_porpre(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,8,n_codbne(x),n_valbne(x),n_porbne(x),nvl(n_valbne(x),0)*nvl(n_porbne(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,9,n_codttx(x),n_valttx(x),n_porttx(x),nvl(n_valttx(x),0)*nvl(n_porttx(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,10,n_codmtx(x),n_valmtx(x),n_porpre(x),nvl(n_valmtx(x),0)*nvl(n_pormtx(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,11,n_codstx(x),n_valstx(x),n_porstx(x),nvl(n_valstx(x),0)*nvl(n_porstx(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,12,n_codiac(x),n_valiac(x),n_poriac(x),nvl(n_valiac(x),0)*nvl(n_poriac(x),0) From dual
                    union all
                    Select n_pais(x),n_tdoc(x),trim(c_ndoc(x)),ln_numrg,13,n_codima(x),n_valima(x),n_porima(x),nvl(n_valima(x),0)*nvl(n_porima(x),0) From dual;
                Exception When Others Then
                     ln_errms := substr(sqlerrm,1,200);
                     Insert Into jaql140(jaql140fec,jaql140pais,jaql140tdoc,jaql140ndoc,jaql140tab,jaql140err)
                     Values(ld_Fecpro,n_pais(x),n_tdoc(x),trim(c_ndoc(x)),'JAQY474D',ln_errms);
                     Commit;
                End;
                ...
                Exception When Others Then
                     ln_errms := substr(sqlerrm,1,200);
                     Insert Into jaql140(jaql140fec,jaql140pais,jaql140tdoc,jaql140ndoc,jaql140tab,jaql140err)
                     Values(ld_Fecpro,n_pais(x),n_tdoc(x),trim(c_ndoc(x)),'JAQZ264',ln_errms);
                     Commit;
                End;
           ...
           Exception When Others Then
              ln_errms := substr(sqlerrm,1,200);
              Insert Into jaql140(jaql140fec,jaql140pais,jaql140tdoc,jaql140ndoc,jaql140tab,jaql140err)
              Values(ld_Fecpro,n_pais(x),n_tdoc(x),trim(c_ndoc(x)),'JAQY474',ln_errms);
              Commit;
           End;     
         End Loop;     
         Commit;  
