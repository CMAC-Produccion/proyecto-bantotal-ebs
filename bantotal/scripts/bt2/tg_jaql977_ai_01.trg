           ...
           if TRIM(lv_PUSH_TOKEN) is not null then -- rcuadros 23/05/2025
                if lc_sex = 'M' then
                   ...
                   if lc_sex = 'N' then -- rcuadros 22/09/2025
                      lv_mensaje := 'Estimado cliente Caja Arequipa le informa sobre la operación '||lv_OPERACION||' por '||lv_MONEDA||trim(to_char(ln_MONTO,'9,999,999.90'))||lv_c4||' realizada en '||lv_c1||': '||lv_ubigueo;
                   else
