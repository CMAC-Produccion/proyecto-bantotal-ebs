CREATE OR REPLACE PACKAGE PQ_AH_REPCREDITOS IS
 ----------------------------------------
  -- autor :  SMARQUEZ
  -- fecha :  27/03/2015
  -- N dias de Mora Relacionados con los
...
END PQ_AH_REPCREDITOS;
/

create or replace package body PQ_AH_REPCREDITOS is
...
Procedure sp_ah_ctacob(P_REGION IN VARCHAR2,
  ...
  cursor c_cuentas1 is
    select q.pgcod,
           ...
           PQ_CR_CREDITOS.fn_facultad_cta(q.pgcod,
                                          q.scsuc,
                                          q.scmda,
                                          q.scpap,
                                          q.sccta,
      ...
      from (select w.pgcod,
                   ...
                   w.scsbop,
                   w.scoper,
                   sum(w.saldo) as saldo,
                   sum(w.interes) as interes,
                   w.pepais,
                   w.petdoc,
                   w.pendoc
              from (select a.pgcod,
