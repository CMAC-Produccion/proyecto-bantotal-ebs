CREATE OR REPLACE PROCEDURE SP_MIGRACION_SEGUROS(
 ...
 -- *********************************************************************************
  -- Nombre                     : Proceso de migracion de seguros pasivos data sigsretail
  -- Sistema                    : BANTOTAL
  -- M¿dulo                     : PASIVAS
  -- Versi¿n                    : 1.0
  -- Fecha de Creacion          : 2025.10.07
  -- Autor de Creacion          : DLYA 
  -- Uso                        : FUNCIONES Y PROCEDIMIENTOS PARA PANEL ABM SEGUROS
  -- Estado                     : Activo
  -- Acceso                     : P¿blico
  -- Fecha de Modificacion      : 10/10/2025
  -- Autor                      : DLYA
  -- Modificacion               : se cambio las fechas por producto para procesar todo
  -- *********************************************************************************

cursor cursortarjeta is
 SELECT
    vista."Producto",
  ...
  vista."Fecha de asociación",
    ...
    vista."Estado"
   from "TMP_SisDataTarjetaCta"@SISRETAIL vista 
   WHERE vista."Producto" like PRODUCTO || '%' 
     and vista."Estado" ='H'       
...
where "Producto" like PRODUCTO || '%'
  AND "Estado" ='H';
--And "Nro Afiliación" =65  
 -- and "Fecha de afiliación" = FECHACHAR;
...
WHERE vista."Producto" like PRODUCTO || '%';
 -- and vista."Fecha de Ingreso"= FECHACHAR;
