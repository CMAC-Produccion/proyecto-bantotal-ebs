CREATE OR REPLACE PROCEDURE "SP_CR_INS_BOVEDA_BT" IS
    ...
    -- Fecha de Modificación      : 05/11/2025
    -- Autor de la Modificación   : EHIDALGOM/LCARPIO
    -- Descripción de Modificación: Se agregó usuario BOTCTS1
    -- *****************************************************************
 -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
  N_CONT1 NUMBER := 0;
  N_CONT2 NUMBER := 0;
  ...
  my_array_mails string_array := string_array('ehidalgom@cajaarequipa.pe', 'kcabrerac@cajaarequipa.pe', 'lcarpio@cajaarequipa.pe', 
                                        'jsanchez@cajaarequipa.pe', 'lchumbilla@cajaarequipa.pe','mblas@cajaarequipa.pe',
...
BEGIN
  ...
  where UBUSER in ('BOTPROREC','ROBOTADQUI','BOTJOYAD','BOTCAMTRA','BOTYAPLIN','BOTCTS1');
  commit;
