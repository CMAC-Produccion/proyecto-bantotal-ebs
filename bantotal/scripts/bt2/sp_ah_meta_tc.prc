...
------------------------------------------------------------------------------------------------
-- PROPOSITO   : REPORTE POR METAS PARA OPERACIONES
-- AUTOR       : SILVIA MARQUEZ AVENDAÑO
-- FECHA       : 2017 REPORTE
-- MODIFICACION:SMARQUEZ 20250204 FILTRADO POR TRANSACCCIONES DE TIPO DE CAMBIO EXCLUSIVAMENTE
--              REVISION DE  GENERACION DE REPORTE MENSUAL Y POR TRAMOS POR LAS DIFERENCIAS
----------------------------------------------------------------------------------------------                                          
 CURSOR DATOHIST(SUC IN NUMBER, fecha in date) IS
   select a.hfcon,
         (Select scnom from fst001 where pgcod=1 and sucurs = a.hsucor) sucursal,
         a.hcmod,
         a.htran, 
         a.hcodmo,
         a.hcimp1, 
  ...
  cursor sucursal is
    select *
      from fst001 where sucurs < 800 ;
      
  cursor DatosHoy (sucu in number) is
  select b.itfcon,
         (Select scnom from fst001 where pgcod=1 and sucurs = a.itsuc) sucursal,
         a.itmod,
         a.ittran, 
         a.itcodm,
         a.itimp1, 
         ...
         (select exusau 
           ...
           where s.Pgcod= 1 
             ...
             and s.Exstat = 'S')autoriza
      from fsd016 a, 
           fsd015 b --PGCOD, ITSUC, ITMOD, ITTRAN, ITNREL
      where a.pgcod=1 
    ...
    while  fecha_ini <= P_N_FECFIN loop
      for reg in sucursal loop
        for dat in DATOHIST(reg.sucurs,fecha_ini) loop
          insert into JAQZ552(jaqz552cor,
                              jaqz552fech,
                              jaqz552suc,
                              jaqz552mod,
                              jaqz552tran,
