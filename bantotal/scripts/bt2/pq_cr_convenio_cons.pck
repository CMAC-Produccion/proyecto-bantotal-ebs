CREATE OR REPLACE PACKAGE PQ_CR_CONVENIO_CONS IS
  ...
  -- ACCESO                      : PUBLICO
  -----------------------------------------------------------------------------------------
  -- FECHA DE MODIFICACION       : 18/10/2024  MAYCOL CHAVEZ CHUMAN SE MODIFICO EL PROCEDIMIENTO SP_CR_OBTENER_DOC_EMPLEADOR
  --                             : 27/03/2025 YYAMPI SE MODIFICO EL SP_CR_OBTENER_RUC_CONVENIO
  -- 
  --                            
  -- **************************************************************************************
...
CREATE OR REPLACE PACKAGE BODY PQ_CR_CONVENIO_CONS IS
  ...
  BEGIN
    ...
    BEGIN
      SELECT X.XWFMODULO, X.XWFCUENTA
        INTO LN_MODULO, LN_CUENTA
        FROM XWF700 X
    ...
    IF LN_MODULO = 106 THEN
      ...
      BEGIN
        SELECT T.JAQA58NOCN, T.JAQA58FEVE --FECHA DE VENCIMIENTO
          INTO PC_INDICADOR, PD_FECHA
          FROM JAQA58 T
    ...
    IF LN_MODULO = 107 THEN
      ...
      BEGIN
        SELECT T.JAQA305VA2, T.JAQA305FVE --FECHA DE VENCIMIENTO
          INTO PC_INDICADOR, PD_FECHA
          FROM JAQA305 T
  ...
  BEGIN
    BEGIN
      SELECT A1.PGFAPE INTO V_FECHASIS FROM FST017 A1 WHERE A1.PGCOD = 1;
    EXCEPTION
      WHEN OTHERS THEN
        NULL;
    END;
    ...
    BEGIN
      SELECT D1.PP101NCART, TRIM(D1.PP101NOMC)
        INTO NRO_CONVENIO, NOM_CONVENIO
        FROM FST198 A1, FSD602 B1, FSD016 C1, FPP101 D1
       WHERE A1.TP1COD = 1
         AND A1.TP1COD1 = 111154
         AND A1.TP1CORR1 = 1
         AND A1.TP1CORR2 = 47
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        BEGIN
          SELECT D1.PP101NCART, TRIM(D1.PP101NOMC)
            INTO NRO_CONVENIO, NOM_CONVENIO
            FROM FST198 A1, FSD602 B1, FSH016 C1, FPP101 D1
           WHERE A1.TP1COD = 1
             AND A1.TP1COD1 = 111154
             AND A1.TP1CORR1 = 1
             AND A1.TP1CORR2 = 47
  ...
  PROCEDURE SP_CR_OBTENER_RUC_CONVENIO(P_INSTANCIA    IN NUMBER,
    ...
    -- ACCESO                      : PUBLICO
    -----------------------------------------------------------------------------------------
    -- FECHA DE MODIFICACION       : 27/03/2025 YYAMPI SE AGREGO CONTROL DE EXCEPCION
    --
    -- **************************************************************************************
