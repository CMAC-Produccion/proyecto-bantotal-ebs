create or replace procedure sp_Reporte_Cronologico_Hist_2(P_C_USUARIO IN VARCHAR2,
                                                        P_D_FECPRO_INI IN DATE, 
                                                        P_D_FECPRO_FIN IN DATE,             
                                                        P_N_NUMAGE IN NUMBER,
                                                        P_C_USUAGE IN VARCHAR2,
                                                        P_C_TIPO   IN VARCHAR2	
                                                       ) is
-- *****************************************************************
-- Nombre                       : sp_Reporte_Cronologico_Hist_2
-- Sistema                      : BANTOTAL
-- Módulo                       : Activas
-- Versión                      : 1.0
...
-- Estado                       : Activo
-- Acceso                       : Público
-- Parámetros de Entrada        : P_C_USUARIO, P_D_FECPRO_INI, P_D_FECPRO_FIN, P_N_NUMAGE, P_C_USUAGE, P_C_TIPO
-- Retorno                      : 
-- *****************************************************************                                                     
...
order by ithora asc
 ...
 cursor c_movimientos is  --reporte cronologico 
          ...
          b.Hmda   Hmda,
              max((select 
                ...
                where x.pgcod  = b.pgcod 
                  ...
                  and x.htran  = b.htran 
                  and x.hnrel  = b.hnrel 
                  and x.hfcon  >= P_D_FECPRO_INI and x.hfcon  <= P_D_FECPRO_FIN
   ...
   where a.pgcod    = b.pgcod
     ...
     and a.Hfcon    >= P_D_FECPRO_INI  and a.Hfcon    <= P_D_FECPRO_FIN
      -- MCORDOVA 21-11-23
      AND A.HUSING IN(SELECT UBUSER FROM PRFU00 WHERE PRFGCOD IN 
      (SELECT TP1DESC FROM  fst198 where TP1COD = 1 and tp1cod1 = 11153 and TP1CORR1 = 24 AND TP1CORR2 <> 0))
      -- MCORDOVA 21-11-23  
     and (
          ((a.Husing = rpad(P_C_USUAGE,10,' ') or trim(P_C_USUAGE) is null) 
             and 
            d.tp1imp3 = 1
          )
           OR 
          ((a.Huscnf = rpad(P_C_USUAGE,10,' ') or trim(P_C_USUAGE) is null)  
             and 
            d.tp1imp3 = 2
          )
         )   
...
begin
     ...
     commit;
     if P_C_TIPO = 'D' then
         if P_D_FECPRO_INI <> to_date('01/01/0001','dd/mm/rrrr') then
...
end sp_Reporte_Cronologico_Hist_2;
/
