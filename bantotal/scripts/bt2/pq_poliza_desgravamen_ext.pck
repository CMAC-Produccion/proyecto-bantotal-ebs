create or replace package PQ_POLIZA_DESGRAVAMEN_EXT is
...
end PQ_POLIZA_DESGRAVAMEN_EXT;
/

create or replace package body PQ_POLIZA_DESGRAVAMEN_EXT is
       ...
       (SELECT P.AOMOD, P.AOSUC, P.AOCTA, P.AOOPER, P.AOSBOP, P.AOTOPE
         ...
         and (
             --Creditos con fecha valor mayor a 2 DE noviembre a 2015                             
              --t_cred.aofval >= TO_DATE('03/11/2015', 'DD/MM/YYYY')--LD_FECHADESGRA -- gcarranzas 10052021             
              nvl(t_cred.aofval, t_hist.BCFVAL ) >= LD_FECHADESGRA -- gcarranzas 10052021             
            ...
            -- and t_hist.BCCTA = 50368 and t_hist.BCOPER = 2368833
             and (t_hist.BCCTA, t_hist.BCOPER ) in
             (select z.hacta, z.haoper  from AQPB812 z )
  ...
  BEGIN
    ...
    BEGIN
      ...
      commit;
       ...
       ---------->>>>>>>>>>>>>>>>>>ACTUALIZAR IMPORTE DE MINIMA OPERACION DE CREDITOS QUE NO TIENEN IMPORTE      
      ----------------------------------------------------------------------------------------------------------------
      ----------------------------------------------------------------------------------------------------------------
      ...
      for j in datos_ha_procesar loop
        ...
        if j.aostat in (33, 34) then
          begin
            ...
            PQ_POLIZA_DESGRAVAMEN_EXT.sp_fsr011buscar(j.pgcod,
                                                  j.aomod,
                                                  j.aosuc,
                                                  j.aomda,
                                                  j.aopap,
      ...
      INSERT INTO JAQY523
        ...
        SELECT PD_FECHA,
               ...
               sal_cli,
               apr_cli,
               PQ_POLIZA_DESGRAVAMEN_EXT.fn_tipo_rep(pcnomr, fec_nac, PD_FECHA),
               --PQ_POLIZA_DESGRAVAMEN_EXT.fn_cr_years(PD_FECHA,fec_nac),
               floor(MONTHS_BETWEEN(PD_FECHA, fec_nac) / 12),
               PQ_POLIZA_DESGRAVAMEN_EXT.fn_nomtit(seg.pepais,
                                               seg.petdoc,
                                               seg.pendoc),
               seg.bcmda,
               seg.bcsdmo,
  ...
  begin
    begin
      ...
      --ln_edad   := PQ_POLIZA_DESGRAVAMEN_EXT.fn_cr_years(PD_FECHA,fec_nac); --edad actual
  ...
  BEGIN
    ...
    PQ_POLIZA_DESGRAVAMEN_EXT.recorrerData(ln_result);
