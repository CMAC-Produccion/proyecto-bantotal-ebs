create or replace package PQ_CV_MONITOREO_ACF_ATMS is
  -- ------------------------------------------------------------------------------------------------
  ...
  -- Estado                : Activo
  -- Acceso                : Público
  -- Fecha de Modificación : 21/06/2022
  -- Autor de Creación     : Julio Luna Flores
  -- Descripción Modific.  : Se considera la consulta de saldos como transacción y se envía cero en monto
  ...
  -- Autor de Modificación : Julio Luna Flores
  -- Descripción Modific.  : Se ajusta función de índice 049 para envío de extornos
  ...
  function fn_trama_indice012(pn_numtra in number, pn_codcan in varchar2) return varchar2; --rcuadros 18/05/2023
  ...
  function fn_trama_indice041(pn_numtra in number, pn_codcan in varchar2) return varchar2; --rcuadros 18/05/2023
  function fn_trama_indice042(pn_numtra in number, pn_codcan in varchar2) return varchar2; --rcuadros 18/05/2023
  function fn_trama_indice043(pn_numtra in number, pn_codcan in varchar2) return varchar2; --rcuadros 18/05/2023
...
create or replace package body PQ_CV_MONITOREO_ACF_ATMS is
    ...
    for i in c1 loop
      ...
      case i.n_indice
        ...
        when 12 then
          ...
          if i.c_import = 'R' and i.c_tipdat = 'A' then
            lc_trmrsp := lc_trmrsp ||
                         rpad(STR1 => fn_trama_indice012(pn_numtra, pn_codcan), --rcuadros se agrega parametro canal 18/05/2023
                              LEN => i.n_longit, PAD => i.c_justxt);
          elsif i.c_import = 'R' and i.c_tipdat = 'N' then
            lc_trmrsp := lc_trmrsp ||
                         lpad(STR1 => fn_trama_indice012(pn_numtra, pn_codcan), --rcuadros se agrega parametro canal 18/05/2023
                              LEN => i.n_longit, PAD => i.c_jusnum);
          end if;
        ...
        when 41 then
          ...
          if i.c_import = 'R' and i.c_tipdat = 'A' then
            lc_trmrsp := lc_trmrsp ||
                         rpad(STR1 => fn_trama_indice041(pn_numtra, pn_codcan), --rcuadros se agrega parametro canal 18/05/2023
                              LEN => i.n_longit, PAD => i.c_justxt);
          elsif i.c_import = 'R' and i.c_tipdat = 'N' then
            lc_trmrsp := lc_trmrsp ||
                         lpad(STR1 => fn_trama_indice041(pn_numtra, pn_codcan), --rcuadros se agrega parametro canal 18/05/2023
                              LEN => i.n_longit, PAD => i.c_jusnum);
          end if;
        when 42 then
          --R
          ...
          if i.c_import = 'R' and i.c_tipdat = 'A' then
            lc_trmrsp := lc_trmrsp ||
                         rpad(STR1 => fn_trama_indice042(pn_numtra, pn_codcan), --rcuadros se agrega parametro canal 18/05/2023
                              LEN => i.n_longit, PAD => i.c_justxt);
          elsif i.c_import = 'R' and i.c_tipdat = 'N' then
            lc_trmrsp := lc_trmrsp ||
                         lpad(STR1 => fn_trama_indice042(pn_numtra, pn_codcan), --rcuadros se agrega parametro canal 18/05/2023
                              LEN => i.n_longit, PAD => i.c_jusnum);
          end if;
        when 43 then
          --R
          ...
          if i.c_import = 'R' and i.c_tipdat = 'A' then
            lc_trmrsp := lc_trmrsp ||
                         rpad(STR1 => fn_trama_indice043(pn_numtra, pn_codcan), --rcuadros se agrega parametro canal 18/05/2023
                              LEN => i.n_longit, PAD => i.c_justxt);
          elsif i.c_import = 'R' and i.c_tipdat = 'N' then
            lc_trmrsp := lc_trmrsp ||
                         lpad(STR1 => fn_trama_indice043(pn_numtra, pn_codcan), --rcuadros se agrega parametro canal 18/05/2023
                              LEN => i.n_longit, PAD => i.c_jusnum);
          end if;
        when 44 then
          --R
  ...
  *******************************************************************************************/
  ...
  begin
    --//
    select SUBSTR(a1.jaql539valtr, 3, 16)
      into lc_cmp011
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 2;
    return lc_cmp011;
  exception
    when others then
      return ' ';
  ...
  --// R
  function fn_trama_indice012(pn_numtra in number, pn_codcan in varchar2) return varchar2 is
    lc_cmp012 varchar2(100); --rcuadros 18/05/2023 - se agrega parametro canal
  begin
    --7941-TR
    --rcuadros 18/05/2023 - se agrega control if para discriminar la transferencias recibidas
    if substr(pn_codcan, instr(pn_codcan, '_') + 1) = 'TR' then
      lc_cmp012 := '27';
    else
     --//
  ...
  function fn_trama_indice015(pn_numtra in number) return varchar2 is
    ...
    ld_fectra date;
    ln_codmda number;
    lc_cotrx  char(6); --jlunaf 21/06/2022
  begin
    --//
    select b1.jaql540feini, b1.jaql540cotrx
      into ld_fectra, lc_cotrx --jlunaf 21/06/2022 se obtiene código de transacción
      from jaql540 b1
    ...
    --jlunaf 21/06/2022 INICIO - Se fuerza que para consulta de saldos el monto sea siempre cero
    if substr(lc_cotrx, 1, 2) = '31' then
       ln_aux004 := 0.0;
       lc_aux004 := '000000000000';
    end if;
    --jlunaf 21/06/2022 FIN
    --//
    ...
    if ln_codmda = 604 then
      ...
      lc_cmp015 := lpad(trim(replace(to_char(ln_conver * 100), ',', '')), 14,
                        '0');
    else
      lc_cmp015 := lc_aux004;
    end if;
  ...
  begin
    --//
    --jlunaf 21/06/2022 Se obtiene código de transacción
    select b1.jaql540cotrx
      into lc_cotrx
      from jaql540 b1
     where b1.jaql540cotra = pn_numtra;
  ...
  begin
    ...
    if ln_codmda = 604 then
      ln_conver := ln_aux004 / ln_tipcam;
      lc_cmp017 := lpad(trim(replace(to_char(ln_conver * 100), ',', '')), 14,
                        '0');
    else
      lc_cmp017 := lc_aux004;
    end if;
  ...
  begin
    --//
    select b1.jaql540hoini, b1.jaql540fetra
  ...
  function fn_trama_indice022(pn_numtra in number) return varchar2 is
    lc_cmp022 varchar2(100) := '0';
    lc_aux013 varchar2(100) := '0';
  begin
    --//
    lc_cmp022 := to_char(extract(year from trunc(sysdate)));
    --//
    select a1.jaql539valtr
      into lc_aux013
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 13;
    --//
    lc_cmp022 := lc_cmp022 || lc_aux013;
    return lc_cmp022;
  exception
    when others then
  ...
  function fn_trama_indice028(pn_numtra in number) return varchar2 is
    lc_cmp028 varchar2(100) := '604'; --Hlaqui 26/01/2022 - Se setea valor 604
  begin
    --//
    return lc_cmp028;
  exception
  ...
  function fn_trama_indice029(pn_numtra in number) return varchar2 is
    lc_cmp029 varchar2(100) := '604'; --Hlaqui 26/01/2022 - Se setea valor 604
  begin
    --//
    return lc_cmp029;
  exception
  ...
  begin
    ...
    --//  
    begin
      --Hlaqui 26/01/2022 - Se obtiene de la tabla jaql539
      select trim(a1.jaql539valtr)
      into lc_cmp030
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
      and a1.jaql539nucam = 22;
     
      /*
      select nvl(POS_ENTRY_MODE, '51')
        into lc_cmp030
        from itxn@TXNSWT c1
       where lpad(to_char(trace), 12, '0') = lc_tracet
         and capdate = ld_fectra; */
  ...
  begin
    --// CARD_SEQ_NUM = 1, valor del switch
    select b1.jaql540nutar
      into lc_cmp031
      from jaql540 b1
     where b1.jaql540cotra = pn_numtra;
    --//
  ...
  begin
    --//
    select substr(a1.jaql539valtr, 3, 6)
      into lc_cmp034
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 32;
    return lc_cmp034;
  exception
    when others then
      return '00000';
  ...
  begin
    lc_cmp036 := null;
    --// Para los retiros campo 38 con valor
    begin
      select trim(a1.jaql539valtr)
        into lc_cmp036
        from jaql539 a1
       where a1.jaql539cotra = pn_numtra
         and a1.jaql539nucam = 38;
    exception
      when others then
        lc_cmp036 := null;
    end;
  
    --// Para las trasferencias campo 38 sin valor
    if lc_cmp036 is null then
      begin
        select lpad(trim(to_char(a1.jaql540trans)), 2, '0') ||
               lpad(trim(to_char(a1.jaql540relac)), 4, '0')
          into lc_cmp036
          from jaql540 a1
         where a1.jaql540cotra = pn_numtra;
      exception
  ...
  function fn_trama_indice039(pn_numtra in number) return varchar2 is
    lc_cmp039 varchar2(100) := '226';
  begin
    --//   
    /* Hlaqui 26/01/2022 - Se comenta peticion
    select substr(a1.jaql539valtr, 24, 3)
      into lc_cmp039
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 35;
    */
    return lc_cmp039;
  exception
    when others then
  ...
  --// R
  function fn_trama_indice041(pn_numtra in number, pn_codcan in varchar2) return varchar2 is
    lc_cmp041 varchar2(100) := ''; --rcuadros 18/05/2023 - se agrega parametro canal
  begin
    --rcuadros 18/05/2023 - se agrega control if
    if substr(pn_codcan, instr(pn_codcan, '_') + 1) = 'TR' then
  ...
  --// R
  function fn_trama_indice042(pn_numtra in number, pn_codcan in varchar2) return varchar2 is
    lc_cmp042 varchar2(100) := ''; --rcuadros 18/05/2023 - se agrega parametro canal
  begin
    --rcuadros 18/05/2023 - se agrega control if
    if substr(pn_codcan, instr(pn_codcan, '_') + 1) = 'TR' then
      ...
      --//
      select upper(translate(lower(a1.jaql539valtr), 'áéíóöoúüuñ',
                             'aeiooouuun'))
        into lc_cmp042
        from jaql539 a1
       where a1.jaql539cotra = pn_numtra
         and a1.jaql539nucam = 43;
    end if;
    return lc_cmp042;
  exception
    when others then
  ...
  --// R
  function fn_trama_indice043(pn_numtra in number, pn_codcan in varchar2) return varchar2 is
    lc_cmp043 varchar2(100) := ' '; --rcuadros 18/05/2023 - se agrega parametro canal
  begin
    --rcuadros 18/05/2023 - se agrega control if
    if substr(pn_codcan, instr(pn_codcan, '_') + 1) = 'TR' then
      lc_cmp043 := 'ATM';
      ...
      --//
      -- jlunaf 24/11/2022 - Se añade función nvl en caso se cargue un valor nuevlo en el substr
      select nvl(upper(translate(lower(substr(a1.jaql539valtr, 26, 13)),
                             'áéíóöoúüuñ', 'aeiooouuun')), ' ')
        into lc_cmp043
        from jaql539 a1
       where a1.jaql539cotra = pn_numtra
         and a1.jaql539nucam = 43;
  ...
  begin
    --//
    -- jlunaf 24/11/2022 - Se añade función nvl en caso se cargue un valor nuevlo en el substr
    select nvl(upper(translate(lower(substr(a1.jaql539valtr, 39, 2)),
                           'áéíóöoúüuñ', 'aeiooouuun')), ' ')
      into lc_cmp044
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
  ...
  function fn_trama_indice045(pn_numtra in number) return varchar2 is
    lc_cmp045 varchar2(100) := '  '; --Hlaqui 26/01/2022 - Se modifica para que se retornar con 2 espacios
  begin
  ...
  begin
    -- jlunaf 17/08/2022 - INICIO Ajuste en obtención de datos para extornos
    select substr(a1.jaql539valtr, 3, 16)
      into lc_auxtar
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 2;
    begin
       select decode(a1.z0e479mon, 0, '604', 101, '840')
         into lc_cmp049
         from z0e479 a1
       where a1.z0e478nro = rpad(trim(lc_auxtar),19,' ') --Hlaqui 26/01/2022 - Se completa a 19 caracteres
         and a1.z0e480cod = 3;
    exception
       when others then
         lc_cmp049 := '0';
    end;
    -- jlunaf 17/08/2022 - FIN
    -- jlunaf 17/08/2022 - INICIO se comenta por ajuste en obtención de datos
    /*
    ...
    select substr(a1.jaql539valtr, 3, 16)
     ...
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 2;
    --//
    if length(trim(lc_cmp049)) >= 2 and substr(lc_cmp049, 1, 11) = '000' then
      --//
      lc_cmp049 := '604';
    elsif length(trim(lc_cmp049)) >= 2 and substr(lc_cmp049, 1, 11) = '101' then
      --//
      lc_cmp049 := '840';
    else
      --// 
  ...
  begin
    ...
    select substr(b1.jaql540nutar, 1, 16) into lc_auxtar from jaql540 b1 where b1.jaql540cotra = pn_numtra;
  ...
  begin
    --// Cuenta Asociada
/*    select a2.jaql540trans 
    into lc_trans 
    from jaql540 a2
    where a2.jaql540cotra = pn_numtra 
    and a2.jaql540modul = 66;
    if lc_trans in (25,26,27,28,35,59,
                          61,65,80,83,90,97,
                          99,100,101,105,120,
                          121,140,146,148,180,
                          200,201)then
     ...
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 102;*/
   -- end if;
   /*    lc_cmp053 := substr(lc_cmp053, 3, 14);
    
    if (lc_cmp053 <> '28' or lc_cmp053 <> '280000000000000000000000000000' and lc_trans <> 'I') then
        lc_cmp053 := substr(lc_cmp053,-3,3) + lc_cmp053;
      else
        lc_cmp053 := '1';
    end if;
       */
...
-- Dmanriquec 13/06/2023 Armado de cuenta origen 

    if length(trim(lc_cmp053)) > 2 then
      lc_cmp053 := substr(lc_cmp053, 3, 14);
      lc_cta := to_number(substr(lc_cmp053, 1, 6));
      lc_mod := to_number(substr(lc_cmp053, 7, 3));
      lc_mon := to_number(substr(lc_cmp053, 10, 3));
      lc_sct := to_number(substr(lc_cmp053, 13, 2));
      select lpad(to_char(lc_cmp053), 17, '0') || 
             lpad(to_char(a1.z0e479top), 3, '0')
          into lc_cmp053
          from z0e479 a1
         where trim(a1.z0e478nro) = lc_auxtar
           and a1.z0e479cta = lc_cta
           and a1.z0e479mod = lc_mod
           and a1.z0e479mon = lc_mon
           and a1.z0e479sct = lc_sct;
    else
      begin
        select lpad(to_char(a1.z0e479cta), 9, '0') ||
               lpad(to_char(a1.z0e479mod), 3, '0') ||
               lpad(to_char(a1.z0e479mon), 3, '0') ||
               lpad(to_char(a1.z0e479sct), 2, '0') ||
               lpad(to_char(a1.z0e479top), 3, '0')
          into lc_cmp053
          from z0e479 a1
         where trim(a1.z0e478nro) = lc_auxtar
  ...
  begin
    ...
    select nvl(substr(a1.jaql539valtr, 3, 14), ' ')
      into lc_cmp054
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 103;
    --//               
    */
    return lc_cmp054;
  exception
  ...
  begin
    ...
    */
    ...
    select decode(b1.z0e478est, 'AC', 'A', 'BA', 'C', 'BT', 'W')
      into lc_cmp057
      from z0e478 b1
     where b1.z0e478nro = rpad(trim(lc_auxtar),19,' '); --Hlaqui 26/01/2022 - Se actualiza para que encuentre la tarjeta
    --//   
    return lc_cmp057;
  exception
    when others then
  ...
  begin
    ...
    /* 
    select substr(a1.jaql539valtr, 3, 19)
      into lc_auxtar
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 2;
  ...
  begin
    --//        
    select nvl(substr(a1.jaql539valtr, 4, 1), 0)
      into lc_cmp061
      from jaql539 a1
     where a1.jaql539cotra = pn_numtra
       and a1.jaql539nucam = 60;
    return lc_cmp061;
  exception
    when others then
      return lc_cmp061;
