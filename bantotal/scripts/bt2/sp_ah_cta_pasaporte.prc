CREATE OR REPLACE PROCEDURE SP_AH_CTA_PASAPORTE(P_CTA IN NUMBER
...
BEGIN
  ...
  IF(ln_PN > 0) THEN
    ...
    IF (lp_TDOC = 5) THEN
      ---*** CLIENTE EVALUADO
      P_RESULT := '001';
      P_RESMSG := 'Tipo de Documento es PASAPORTE';
      P_ERRCOD := '000';
      P_ERRMSG := '';
      ---***
      ...
      ---*** SI TIPO DOC ES PASAPORTE RETORNAMOS
      RETURN;
      ---***
    ELSE
      P_RESULT := '000';
  ...
  ELSIF (ln_PJ > 0) THEN -- PJ
      ...
      LOOP
        ...
        ---***
        IF(XPJ.PFTDO1 = 5) THEN
          P_RESULT := '001';
          P_RESMSG := 'Tipo de Documento es PASAPORTE';
          P_ERRCOD := '000';
          P_ERRMSG := '';
          --dbms_output.PUT_LINE('PASAPORTE PJ REPRESENTANTE: '||XPJ.PFPAI1||' : '||XPJ.PFTDO1||' : '||XPJ.PFNDO1);
          --dbms_output.PUT_LINE('P_RESULT: '||P_RESULT);
          --dbms_output.PUT_LINE('P_RESMSG: '||P_RESMSG);
          --dbms_output.PUT_LINE('P_ERRCOD: '||P_ERRCOD);
          --dbms_output.PUT_LINE('P_ERRMSG: '||P_ERRMSG);
