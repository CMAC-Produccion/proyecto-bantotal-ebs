create or replace package pq_mg_producto is
...
/
create or replace package body pq_mg_producto is
procedure sp_mapeo_prod /*(p_n_ctaini in number, 
                         p_n_ctafin in number,*\
                         p_c_fecpro in varchar2,
                         p_n_proceso in number,
...
select * from renovacion;
 ...
 /* cursor reprocesa is
  ...
  cursor agencia is
  select distinct suc sucurs from mig$cta_cli x where x.tipo in ('A','D') order by 1;
              ...
              begin 
               ...
               insert into JAQL054 
                ...
                select distinct 1, x.n_sucurs, x.n_moneda, x.n_papel, 
                       ...
                       x.n_modulo, 'B',230,trunc(sysdate),
                       to_char(sysdate,'HH24:MI:SS'), 'MIGRACION_CINKA'   
             from btctaprd x,
                  bandejas.mig$cta_cli y
            where x.c_numcta = y.cuenta
              and x.c_indpri='S' 
