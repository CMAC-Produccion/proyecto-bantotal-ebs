CREATE OR REPLACE TRIGGER BANTPROD.TG_WFWRKITEMS_UPD
  after update on WFWRKITEMS
  for each row

-- *******************************************************************************************************
-- Nombre                     : TG_WFWRKITEMS_UPD
-- Sistema                    : BANTOTAL
-- Módulo                     : Créditos
-- Versión                    : 1.0
...
BEGIN
  --//
  if :new.wftaskcod = '11' and :old.wfitemusrcod <> :new.wfitemusrcod then
    --//
    ...
    lv_usrins := '';
    ln_carapr := 0;
    lv_usrapr := trim(:new.wfitemusrcod)|| 'U';
    --//
