create or replace procedure sp_cr_InstSinRegEval (ln_instancia number) is
      ...
      begin
        select s.sng120tsk,
          ...
          into lc_sng120tsk,
               ...
               ln_sng120tcbi
          from sng120 s
         where s.sng120ins in
               (select max(s.sng001inst)
                  from sng001 s
                 where s.sng001pais = ln_pais
                   and s.sng001tdoc = ln_tdoc
                   and s.sng001ndoc = lc_ndoc
                   and s.sng001inst < a.ln_InsSinReg)
...
insert into AQPB876 values (user,sysdate,'SP_CR_INSTSINREGEVAL', ln_instancia);
commit;
end sp_cr_InstSinRegEval;
/
