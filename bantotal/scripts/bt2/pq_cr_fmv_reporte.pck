create or replace package pq_cr_fmv_reporte is
	-- *****************************************************************
	...
	-----------------------------------------------------------------------
	-----------------------------------------------------------------------
	procedure sp_crd_reprog(fecha_ini date, fecha_fin date,pc_ubuser in varchar2);
	-----------------------------------------------------------------------
	Procedure sp_cr_aprobados (PD_FECINI IN DATE, PS_FECFIN IN DATE, pc_ubuser in varchar2);
	-----------------------------------------------------------------------
	procedure sp_cr_consolidado(pd_fecini in date, pd_fecfin in date, pc_ubuser in varchar2);
  -----------------------------------------------------------------------
  procedure sp_cr_bono_interes_canc(pn_pgcod in number, pn_scsuc in number, pn_scmda in number, pn_scpap in number, pn_sccta in number,
  ...
  -----------------------------------------------------------------------
	...
	-----------------------------------------------------------------------
  procedure sp_cr_sig_cuota_credito(  pgcod     in xwf700.xwfempresa%type     ,
...
------------------------------------------------------------------
  procedure sp_cr_validar_cambio_age(
                                      ve_pgcod     in  xwf700.xwfempresa%type     ,
  ...
  procedure sp_cr_pagos_bms(pn_ctnro in number, pd_fecini in date, pd_fecfin in date, pn_salbms out number, pn_intbms out number);
                                     
end pq_cr_fmv_reporte;
/

create or replace package body pq_cr_fmv_reporte is
  -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
  
  ----------------------------------------------------------------------------------------
  procedure sp_crd_reprog(fecha_ini date, fecha_fin date,pc_ubuser in varchar2) is
    cursor modulos is 
    ...
    cursor reprogramados is
      select s.sng001inst,
        ...
        from sng001 s
        join xwf070 x
          on s.sng001inst = x.xwfprcin
       where s.sng001ori in (3,13, 14, 16) --2018.08.31 - REFINANCIADO SE AGREGO CODIGO 3
         and s.sng001fin >= fecha_ini
         and s.sng001fin <= fecha_fin
         and x.xwfcont = 'S';
    ...
    for xx in refin_reprog_diario loop
      begin
        ...
        begin
          ...
          into 
            flag_vigente,
            v_frefinancion -- Fecha de Refinanacion/Reprogramacion
           from
           fsd010 d
        ...
        if flag_vigente='S' then
           ...
           begin
             ...
             v_credito := lpad(v_cuenta,9,'0')||lpad(v_moneda,4,'0')||lpad(v_operacion,9,'0');
           end;
          ...
          begin
               SELECT 
                     ((d.ppcap+d.ppint)+(dd.ppimp11+dd.ppimp12+dd.ppimp13+dd.ppimp14+dd.ppimp15+dd.ppimp16+dd.ppimp17+dd.ppimp18+dd.ppimp19))
                 INTO
                     vl_cuota
          ...
          begin          
            insert into aqpa058(aqpa058pgc,aqpa058mod,aqpa058suc,aqpa058mda,aqpa058pap,
                              aqpa058cta,aqpa058ope,aqpa058sbo,
                              aqpa058top,aqpa058age,aqpa058crd,aqpa058cli,
                              aqpa058fds,aqpa058frf,
                              aqpa058dcrd,aqpa058mnta,
                              aqpa058sdor,aqpa058pzo,aqpa058cuo,aqpa058ana,aqpa058tcrd,
                              aqpa058est,aqpa058usr)
                              --aqpa058bbp,aqpa058ibbp,aqpa058pbp,aqpa058ipbp)--Comentado los bonos hasta que se calculen aqui en el proces.
                        values(v_empresa,v_modulo,v_sucursal,v_moneda,v_papel,
                               v_cuenta,v_operacion,v_subope,
                               v_tipope,v_nsucursal,v_credito,v_cliente,
                               v_fdesembolso,v_frefinancion,
                               vvdcredito,vmntoapr,
                               v_saldoref,0,vl_cuota,v_analista,vi_tipcred,vl_tip_sol
                               ,pc_ubuser
                          );
            commit;
          end;
  ...
  ----------------------------------------------------------------------------------------
  Procedure sp_cr_aprobados (PD_FECINI IN DATE, PS_FECFIN IN DATE, pc_ubuser in varchar2) IS
  ...
  begin
        ...
        insert into aqpa060(aqpa060INS,aqpa060USC,aqpa060FDI,aqpa060EST,aqpa060CRE,
                            aqpa060REG,aqpa060CSU,aqpa060SUC,
                            aqpa060crd,aqpa060fapr,aqpa060tapr,aqpa060dcrd,
                            aqpa060plz,aqpa060cuo,
                            aqpa060ANA,aqpa060MOD,
                            aqpa060MDA,aqpa060CTA,aqpa060OPE,aqpa060SBO,aqpa060TOP,
                            aqpa060MTO,aqpa060PAI,aqpa060TDC,aqpa060NDC,aqpa060NOM,
                            aqpa060TEL,aqpa060TSO,aqpa060USR,aqpa060ZON)
        ...
        select a.wfinsprcid, 
               ...
               r.scnom sucursal,
               --credito
               lpad(b.xwfcuenta,9,'0')||lpad(b.xwfmoneda,4,'0')||lpad(b.xwfoperacion,9,'0')  credito,
               --fecha de aprobacion
  ...
  END sp_cr_aprobados;
 ----------------------------------------------------------------------------------------
 procedure sp_cr_consolidado(pd_fecini in date, pd_fecfin in date, pc_ubuser in varchar2) is
  ----------------------------------------------------------------------------------------
  cursor modulos is 
  select tp1nro1 modulo 
   from fst198 t 
   where t.tp1cod   = 1
     ...
     and t.tp1corr3 > 0;
  --------------------------------------------------------------------------------   
  cursor pagos_mivivienda(pd_fecini in date, pd_fecfin in date,modulo in number) is
  select
    to_char(depem50fvn,'MM'),  -- Mes  
    to_char(depem50fvn,'YYYY'),-- Año
    count(*),                  -- Cantidad
    sum(depem50pri),           -- Cuota - Monto a Pagar
    sum(depem50int)            -- Interes a Pagar
  from depem50 
  where depem50fvn>=pd_fecini  -- Rango de Fechas
    and depem50fvn<=pd_fecfin  -- Rango de Fechas
    and depem50mod =modulo     -- Modulo 111
  group by depem50fvn;
  ------------------------------------------------
  var_bbp  number(17,2);
  var_ibbp number(17,2);
  var_ipbp number(17,2);
  var_pbp  number(17,2);
  ...
  begin  
        ...
        COMMIT;
           ...
           insert into aqpa062(aqpa062mesn,
                               aqpa062mes,
                               aqpa062year,
                               aqpa062ncr,
                               aqpa062mct,
                 ...
                 select        
                               ...
                               (sum(b.bono)/*+sum(b.bono_interes)*/) bono,
                               (sum(b.bono_interes)) ibono,
                               (sum(b.premio)/*+sum(b.premio_interes)*/) premio,
                               ...
                               ((sum(b.pago_total))+(sum(b.bono)+sum(b.bono_interes))+(sum(b.premio)+sum(b.premio_interes))) total,
                               111,
                               0,
                               pc_ubuser,
                               b.cta                               
       ...
       from (
                 ...
                 union all                     
                    ...
                    select
                      ...
                      0 premio_interes,
                      d50.depem50cta cta
                    from depem50 d50,depem49 d9,fsd010 d10, fsr011 rx, fsd010 dx
                 ...
                 union all -- obtengo de aqui el monto del bono.
                    select
                      ...
                      0 premio_interes,
                      d.ctnro cta
                    from fsd016 d,fsd015 d5 where d.pgcod=1
                     and d.itmod = 30
                     and d.ittran=113
                     and d.itord = 15
                     --and d.rubro = ''
                ...
                union all
                  ...
                  union all -- obtengo de aqui el monto del bono.
                    select
                      ...
                      0 premio_interes,
                      d.hcta cta
                    from fsh016 d,fsh015 d5 where d.pgcod=1
                     and d.hcmod = 30
                     and d.htran=113
                     and d.hcord = 15
                     and d.pgcod = d5.pgcod
 ...
 -----------------------------------------------------------------------------------------
 procedure sp_cr_bono_interes_canc(pn_pgcod in number, 
                                    pn_scsuc in number, 
                                    pn_scmda in number, 
                                    pn_scpap in number, 
                                    pn_sccta in number,
                                    ...
                                    --pd_fecini in date,
                                    --pd_fecfin in date,
                                    pn_SalBBP out number,
                                    pn_SalPBP out number, 
                                    pn_IntBBP out number, 
                                    pn_IntPBP out number) is
  ----------------------------------------------------------------------------------------
  cursor modulos is 
  select tp1nro1 modulo 
   from fst198 t 
  ...
  select d.pgcod,
   ...
   from fsd016 d,fsd015 d5
                   where d.pgcod   = pn_pgcod 
                     and d.itsucd  = pn_scsuc 
                     and d.Moneda  = pn_scmda 
                     and d.Papel   = pn_scpap 
                     and d.ctnro   = pn_sccta 
                     and d.itoper  = pn_scoper
                     and d.itsubo  = pn_scsbop
                     and d.ittope  = pn_sctope  
                     and d.modulo  = pn_scmod
  ...
  select d.pgcod,
   ...
   from fsh016 d
                   where d.pgcod   = pn_pgcod 
                     and d.hsucur  = pn_scsuc 
                     and d.hmda    = pn_scmda 
                     and d.hpap    = pn_scpap 
                     and d.hcta    = pn_sccta 
                     and d.hoper   = pn_scoper
                     and d.hsubop  = pn_scsbop
                     and d.htoper  = pn_sctope  
                     and d.hmodul  = pn_scmod
                     and d.hcord   = 10
                     and hcmod     = 30
                     and htran     = 113
                     and rownum    = 1;
  --------------------------------------------------------------------------------
  cursor pagos_diario(c_pgcod in number, c_itsuc in number, c_itmod in number, c_ittran in number, c_itnrel in number) is
  select *
  from fsd016 d6
                     where d6.pgcod  = c_pgcod
                       and d6.itsuc  = c_itsuc
                       and d6.itmod  = c_itmod
                       and d6.ittran = c_ittran
                       and d6.itnrel = c_itnrel;
  --------------------------------------------------------------------------------
  cursor pagos_historico(c_pgcod in number, c_itsuc in number, c_itmod in number, c_ittran in number, c_itnrel in number) is
  select *
  from fsh016 d6
                     where d6.pgcod  = c_pgcod
                       and d6.hsucor = c_itsuc
                       and d6.hcmod  = c_itmod
                       and d6.htran  = c_ittran
                       and d6.hnrel  = c_itnrel;
  --------------------------------------------------------------------------------
  var_bbp  number(17,2);
  var_ibbp number(17,2);
  var_ipbp number(17,2);
  ...
  begin 
       --PAGO DATA DIARIO. 
       for td in transacciones_d loop
           for pg in pagos_diario(td.pgcod,td.itsuc,td.itmod,td.ittran,td.itnrel) loop
               if pg.itord = 15 then-- Pago Bono
                  pn_SalBBP := pg.itimp1;
               end if;
               if pg.itord = 16 then-- Interes Bono
                  pn_IntBBP := pg.itimp1;
               end if;
               if pg.itord = 20 then-- Pago Premio
                  pn_SalPBP := pg.itimp1;
               end if;
               if pg.itord = 21 then-- Interes Premio                  
                  pn_IntPBP := pg.itimp1;
               end if;
           end loop;
       end loop;       
       if pn_SalBBP=null and pn_SalPBP=null then
          for td in transacciones_h loop
           for pg in pagos_historico(td.pgcod,td.hsucor,td.hcmod,td.htran,td.hnrel) loop
               if pg.hcord = 15 then-- Pago Bono
                  pn_SalBBP := pg.hcimp1;
               end if;
               if pg.hcord = 16 then-- Interes Bono
                  pn_IntBBP := pg.hcimp1;
               end if;
               if pg.hcord = 20 then-- Pago Premio
                  pn_SalPBP := pg.hcimp1;
               end if;
               if pg.hcord = 21 then-- Interes Premio                  
                  pn_IntPBP := pg.hcimp1;
               end if;
             end loop;
         end loop;
         end if;   
         if  pn_SalBBP is null then
             pn_IntBBP := 0;
           end if;    
end;
 -----------------------------------------------------------------------------------------
function tiempo_aprobacion(instancia in number)
return number
as dias number(17);
min_apro date;
max_apro date;
begin
   ...
   begin
        select min( wfiteminit)
          into min_apro 
        from wfwrkitems where wftaskcod=11 and wfinsprcid=instancia and rownum=1;
   ...
   begin
        select wfiteminit 
          into max_apro
        from wfwrkitems where wftaskcod=11 and wfinsprcid=instancia and wfstscod='C' and rownum=1;
   exception when no_data_found then
        select pgfape
        into max_apro
   ...
   begin
        dias := To_Date(max_apro, 'dd/mm/yyyy') - To_Date(min_apro, 'dd/mm/yyyy'); 
   end;
   if dias =0 then
      dias :=1;
   end if;
