create or replace procedure SP_REPORTE_RECUCSM_1330 (c_destinatario in varchar2) IS
  cursor C_UCSM1 is
...
--    if  UPPER(VHOST_NAME) IN ('SC01ZDBADM010101','SC01ZDBADM020101','T54BTRAC4052','T74BTRAC4051') then
 SELECT count(*) into v_found
  ...
  if v_found =1 then
    ...
    DELETE FROM REPUCSM1;
    COMMIT;
    INSERT INTO REPUCSM1
    ...
    DELETE FROM REPUCSM1_2;
    COMMIT;

    INSERT INTO REPUCSM1_2
    select 'D' tipo,
           ...
           t_movi.jaql82user
      from REPUCSM1 t_inst, jaql082 t_movi, fsd015 t_trad
    ...
    --totales
    DELETE FROM REPUCSM1_3;
    COMMIT;

    INSERT INTO REPUCSM1_3
    select 'T' tipo,
    ...
    v_Subject   := 'Cobros_'||to_char(sysdate, 'yymmdd')||'_1.538';--Cobros_160727_1.538
--    C_SMTP_SERVER := '172.20.100.13';
   --22.09.2016
    ...
    utl_smtp.write_data(V_Conexion,
                        ...
                        utl_tcp.crlf || utl_tcp.crlf || 'Adj. Cobros_'||to_char(sysdate, 'yymmdd')||'_1.538' || utl_tcp.crlf); --Message Body
    utl_smtp.write_data(V_Conexion,
                        '-------SECBOUND' || utl_tcp.crlf ||
                        --'Content-Type: text/plain;' || utl_tcp.crlf);--
                        'Content-Type: text/plain; charset=iso-8859-1' || utl_tcp.crlf);
    utl_smtp.write_data(V_Conexion,
                        ' name='|| 'Cobros_'||to_char(sysdate, 'yymmdd')||'_1.538' || utl_tcp.crlf); --TXT
    utl_smtp.write_data(V_Conexion,
                        'Content-Transfer_Encoding: 8bit' ||--8bit
                        utl_tcp.crlf);
    utl_smtp.write_data(V_Conexion,
                        ...
                        utl_tcp.crlf);
    utl_smtp.write_data(V_Conexion,
                            ' filename='|| 'Cobros_'||to_char(sysdate, 'yymmdd')||'_1.538'|| utl_tcp.crlf ||
                            utl_tcp.crlf);

    for R_UCSM1 in C_UCSM1 loop
          v_wstring := R_UCSM1.CABECERA;
