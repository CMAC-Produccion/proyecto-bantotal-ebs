create or replace package pq_Cr_segmentacion_micro_mens is
  ...
  procedure sp_cr_evalua_clientes_1_NS(P_N_REGLA   IN NUMBER,
                                       P_A_NOMVAR  IN pq_cr_segmentacion_micro_mens.tp_nomvar,
                                       P_A_VALVAR  IN pq_cr_segmentacion_micro_mens.tp_valvar,
                                       P_N_VAR     IN number,
                                       P_A_REGLAS  in pq_cr_segmentacion_micro_mens.tb_reglas,
                                       P_N_REG     in number,
  ...
  procedure sp_cr_cargar_variables_NS_MENS(P_D_FECPRO IN DATE,
                                           P_N_PAIS   IN JAQZ673.JAQZ673PAIS%type,
                                           P_N_TDOC   IN JAQZ673.JAQZ673TDOC%type,
                                           P_C_NDOC   IN JAQZ673.JAQZ673NDOC%type,
                                           P_C_USR    IN JAQZ673.JAQZ673USR%type,
                                           p_a_nomvar out pq_cr_segmentacion_micro_mens.tp_nomvar,
                                           p_a_valvar out pq_cr_segmentacion_micro_mens.tp_valvar,
                                           p_n_var    out number);
...
end pq_Cr_segmentacion_micro_mens;
/

create or replace package body pq_Cr_segmentacion_micro_mens is
  ...
  BEGIN
    ...
    begin
      delete from JAQZ673 a where a.JAQZ673fech = P_D_FECPRO;
      commit;
    exception
      when others then
        null;
    ...
    for i in 0 .. P_N_JOBS loop
      ...
      lc_variable := ' begin ' ||
                     ' pq_Cr_segmentacion_micro_mens.Sp_Carga_data(TO_DATE(''' ||
      ...
      BEGIN
        SELECT count(*)
         ...
         WHERE upper(x.what) LIKE
               '%PQ_CR_SEGMENTACION_MICRO_MENS.SP_CARGA_DATA%';
      EXCEPTION
        WHEN OTHERS THEN
          ln_numjob := 0;
      END;
    ...
    cursor c_clientes(p_fecpro date) is
      select /*+all_rows index_ss(l)*/
       ...
       where not exists (select 1
                from JAQZ673 c
               where c.JAQZ673pais = l.jaqy163pais
                 and c.JAQZ673tdoc = l.jaqy163tdoc
                 and c.JAQZ673ndoc = l.jaqy163ndoc
                 and c.JAQZ673fech = pd_fecpro)
  ...
  begin
    ...
    IF la_jaqy163ndoc.count > 0 THEN
      FOR c IN la_jaqy163ndoc.FIRST .. la_jaqy163ndoc.LAST LOOP
        ...
        begin
          pq_cr_segmentacion_micro_mens.Sp_cr_Resolutores( /*i.jaqy163pais*/la_jaqy163pais(c),
                                                          /*i.jaqy163tdoc*/
                                                          la_jaqy163tdoc(c),
                                                          /*i.jaqy163ndoc*/
                                                          la_jaqy163ndoc(c),
        ...
        begin
          pq_cr_segmentacion_micro_mens.Sp_Resolutores_NS( /*i.jaqy163pais*/la_jaqy163pais(c),
                                                          /*i.jaqy163tdoc*/
                                                          la_jaqy163tdoc(c),
                                                          /*i.jaqy163ndoc*/
                                                          la_jaqy163ndoc(c),
        ...
        lc_JAQZ082VAR3  := 'NRO_ENT_SEGMICRO' || '=' ||
        ...
        --apachecoh 2023.02.23        
        if ln_afecta = 1 then
          lc_JAQZ082VAR14 := 'DATR_M_CEN_RN' || '=' ||
                             trim(to_char(ln_afdatr));
          lc_JAQZ082VAR15 := 'CALF_M_CEN_RN' || '=' ||
                             trim(to_char(ln_afcalif));
          lc_JAQZ082VAR16 := 'SEGMENCOM_RN' || '=' || trim(lv_afecta);
        end if;
  ...
  procedure sp_cr_segmenta_clientes_job_NS(P_D_FECPRO IN DATE,
    ...
    lc_variable   varchar2(4000);
    ln_job        number := 0;
    l_JAQZ674pano JAQZ674.JAQZ674pano%type;
    l_JAQZ674pmes JAQZ674.JAQZ674pmes%type;
    ln_cont       number(2) := 0;
    ln_inst       number(1) := 1;
    ln_limit      number(3) := 20;
    ln_numjob     number(5);
  BEGIN
    ...
    execute immediate 'ALTER SESSION SET NLS_NUMERIC_CHARACTERS = ''.,''';
    lc_fecpro     := to_char(P_D_FECPRO, 'RRRR.MM.DD');
    l_JAQZ674pano := to_number(to_char(P_D_FECPRO, 'RRRR'));
    l_JAQZ674pmes := to_number(to_char(P_D_FECPRO, 'MM'));
    --l_JAQY066pano := to_number(to_char(add_months(P_D_FECPRO, -1),'RRRR'));
    --l_JAQY066pmes := to_number(to_char(add_months(P_D_FECPRO, -1),'MM')); 
  
    delete from JAQZ674 A
     where JAQZ674pano = l_JAQZ674pano
       and JAQZ674pmes = l_JAQZ674pmes; --N : para segmentación nueva 2022
    commit;
    ...
    for i in 0 .. P_N_JOBS loop
      ...
      lc_variable := ' begin ' ||
                     ' pq_Cr_segmentacion_micro_mens.sp_cr_segmenta_clientes_NS(TO_DATE(''' ||
                     lc_fecpro || ''',''RRRR.MM.DD''),' || i || ',''' ||
                     PC_USR || ''');' || ' End; ';
      ...
      BEGIN
        SELECT count(*)
          ...
          FROM dba_jobs x
         WHERE upper(x.what) LIKE
               '%PQ_CR_SEGMENTACION_MICRO_MENS.SP_CR_SEGMENTA_CLIENTES_NS%';
      EXCEPTION
        WHEN OTHERS THEN
          ln_numjob := 0;
      END;
  ...
  procedure sp_cr_segmenta_clientes_NS(P_D_FECPRO in date,
    ...
    cursor c_clientes(p_fecpro date, p_anio number, p_mes number) is
      select /*+all_rows index_ss(l)*/
       JAQZ673pais, JAQZ673tdoc, JAQZ673ndoc
        from JAQZ673 l
       where l.JAQZ673FECH = p_fecpro
         and not exists (select 1
                from JAQZ674 c
               where c.JAQZ674paic = l.JAQZ673pais
                 and c.JAQZ674tdoc = l.JAQZ673tdoc
                 and c.JAQZ674tndoc = l.JAQZ673ndoc
                 and c.JAQZ674pano = p_anio
                 and c.JAQZ674pmes = p_mes)
         and l.JAQZ673USR = PC_USR
         and l.JAQZ673NUME = P_C_DIGITO;
    ...
    la_nomvar      PQ_CR_SEGMENTACION_MICRO_MENS.tp_nomvar;
    la_valvar      PQ_CR_SEGMENTACION_MICRO_MENS.tp_valvar;
    la_nomnul      PQ_CR_SEGMENTACION_MICRO_MENS.tp_nomvar;
    la_valnul      PQ_CR_SEGMENTACION_MICRO_MENS.tp_valvar;
    ln_var         number(3) := 0;
    ...
    l_JAQZ674pano  JAQZ674.JAQZ674pano%type;
    l_JAQZ674pmes  JAQZ674.JAQZ674pmes%type;
    l_JAQZ674paic  JAQZ674.JAQZ674paic%type;
    l_JAQZ674tdoc  JAQZ674.JAQZ674tdoc%type;
    l_JAQZ674tndoc JAQZ674.JAQZ674tndoc%type;
    l_JAQZ674calf  JAQZ674.JAQZ674calf%type;
    l_JAQZ674fecp  JAQZ674.JAQZ674fecp%type;
    l_JAQZ674horp  JAQZ674.JAQZ674horp%type;
    ...
    la_JAQZ673pais tp_pais;
    la_JAQZ673tdoc tp_tdoc;
    la_JAQZ673ndoc tp_ndoc;
    la_reglas      pq_cr_segmentacion_micro_mens.tb_reglas;
    ln_reg         number(5);
  ...
  BEGIN
    ...
    OPEN c_clientes(P_D_FECPRO, l_JAQZ674pano, l_JAQZ674pmes); -- Clientes Bulk
    FETCH c_clientes BULK COLLECT
      INTO la_JAQZ673pais, la_JAQZ673tdoc, la_JAQZ673ndoc;
  
    IF la_JAQZ673ndoc.count > 0 THEN
      FOR c IN la_JAQZ673ndoc.FIRST .. la_JAQZ673ndoc.LAST LOOP
      
        la_nomvar := la_nomnul;
        la_valvar := la_valnul;
        pq_cr_segmentacion_micro_mens.sp_cr_cargar_variables_NS_MENS(P_D_FECPRO => P_D_FECPRO,
                                                                     P_N_PAIS   => la_JAQZ673pais(c),
                                                                     P_N_TDOC   => la_JAQZ673tdoc(c),
                                                                     P_C_NDOC   => la_JAQZ673ndoc(c),
                                                                     P_C_USR    => PC_USR,
                                                                     p_a_nomvar => la_nomvar,
                                                                     p_a_valvar => la_valvar,
                                                                     p_n_var    => ln_var);
        SegmentoRegla := null;
        pq_cr_segmentacion_micro_mens.sp_cr_evalua_clientes_1_NS(P_N_REGLA   => Regla,
                                                                 P_A_NOMVAR  => la_nomvar,
                                                                 P_A_VALVAR  => la_valvar,
                                                                 P_N_VAR     => ln_var,
        ...
        If JAQZ658CCAL is not null then
          -- Califica
          l_JAQZ674paic  := la_JAQZ673pais(c);
          l_JAQZ674tdoc  := la_JAQZ673tdoc(c);
          l_JAQZ674tndoc := la_JAQZ673ndoc(c);
          l_JAQZ674calf  := JAQZ658CCAL;
          l_JAQZ674horp  := to_char(sysdate(), 'HH24:MI:SS');
    ...
    begin
      select tp1nro1
       ...
       where Tp1cod = 1
         ...
         and Tp1corr1 = 40
         and tp1corr2 = 1
         and tp1corr3 = 2; --15/06/2022 yyampi - cambio de guia inclusivos 
    exception
      when others then
        null;
    end;
    ...
    If ExisteGrupo = 'S' then
      ...
      WHILE la_reglas(ln_ind).RNG49COD = Regla LOOP
        ...
        If la_reglas(ln_ind).RNG68ATR = 'EXP1621' then
          ...
          SegmentoRegla2 := null;
          pq_cr_segmentacion_micro_mens.sp_cr_evalua_clientes_2_NS(P_N_REGLA   => Regla2,
                                                                   P_A_NOMVAR  => la_nomvar,
                                                                   P_A_VALVAR  => la_valvar,
                                                                   P_N_VAR     => ln_var,
                                                                   P_A_REGLAS  => la_reglas,
  ...
  procedure sp_cr_cargar_variables_NS_MENS(P_D_FECPRO IN DATE,
    ...
    cursor c_cliente is
      select JAQZ673fech,
             ...
             JAQZ673var40
        from JAQZ673
       where JAQZ673fech = P_D_FECPRO
         and JAQZ673pais = P_N_PAIS
         and JAQZ673tdoc = P_N_TDOC
         and JAQZ673ndoc = P_C_NDOC
         and JAQZ673usr = P_C_USR;
    ln_tmpnum number(3);
  ...
  begin
    ...
    for cli in c_cliente loop
      ...
      if trim(cli.JAQZ673var1) is not null then
        p_n_var := 1;
        ln_tmpnum := instr(cli.JAQZ673var1, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var1, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var1, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var2) is not null then
        p_n_var := 2;
        ln_tmpnum := instr(cli.JAQZ673var2, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var2, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var2, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var3) is not null then
        p_n_var := 3;
        ln_tmpnum := instr(cli.JAQZ673var3, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var3, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var3, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var4) is not null then
        p_n_var := 4;
        ln_tmpnum := instr(cli.JAQZ673var4, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var4, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var4, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var5) is not null then
        p_n_var := 5;
        ln_tmpnum := instr(cli.JAQZ673var5, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var5, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var5, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var6) is not null then
        p_n_var := 6;
        ln_tmpnum := instr(cli.JAQZ673var6, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var6, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var6, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var7) is not null then
        p_n_var := 7;
        ln_tmpnum := instr(cli.JAQZ673var7, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var7, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var7, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var8) is not null then
        p_n_var := 8;
        ln_tmpnum := instr(cli.JAQZ673var8, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var8, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var8, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var9) is not null then
        p_n_var := 9;
        ln_tmpnum := instr(cli.JAQZ673var9, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var9, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var9, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var10) is not null then
        p_n_var := 10;
        ln_tmpnum := instr(cli.JAQZ673var10, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var10, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var10, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var11) is not null then
        p_n_var := 11;
        ln_tmpnum := instr(cli.JAQZ673var11, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var11, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var11, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var12) is not null then
        p_n_var := 12;
        ln_tmpnum := instr(cli.JAQZ673var12, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var12, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var12, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var13) is not null then
        p_n_var := 13;
        ln_tmpnum := instr(cli.JAQZ673var13, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var13, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var13, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var14) is not null then
        p_n_var := 14;
        ln_tmpnum := instr(cli.JAQZ673var14, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var14, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var14, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var15) is not null then
        p_n_var := 15;
        ln_tmpnum := instr(cli.JAQZ673var15, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var15, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var15, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var16) is not null then
        p_n_var := 16;
        ln_tmpnum := instr(cli.JAQZ673var16, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var16, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var16, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var17) is not null then
        p_n_var := 17;
        ln_tmpnum := instr(cli.JAQZ673var17, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var17, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var17, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var18) is not null then
        p_n_var := 18;
        ln_tmpnum := instr(cli.JAQZ673var18, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var18, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var18, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var19) is not null then
        p_n_var := 19;
        ln_tmpnum := instr(cli.JAQZ673var19, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var19, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var19, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var20) is not null then
        p_n_var := 20;
        ln_tmpnum := instr(cli.JAQZ673var20, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var20, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var20, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var21) is not null then
        p_n_var := 21;
        ln_tmpnum := instr(cli.JAQZ673var21, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var21, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var21, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var22) is not null then
        p_n_var := 22;
        ln_tmpnum := instr(cli.JAQZ673var22, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var22, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var22, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var23) is not null then
        p_n_var := 23;
        ln_tmpnum := instr(cli.JAQZ673var23, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var23, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var23, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var24) is not null then
        p_n_var := 24;
        ln_tmpnum := instr(cli.JAQZ673var24, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var24, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var24, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var25) is not null then
        p_n_var := 25;
        ln_tmpnum := instr(cli.JAQZ673var25, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var25, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var25, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var26) is not null then
        p_n_var := 26;
        ln_tmpnum := instr(cli.JAQZ673var26, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var26, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var26, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var27) is not null then
        p_n_var := 27;
        ln_tmpnum := instr(cli.JAQZ673var27, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var27, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var27, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var28) is not null then
        p_n_var := 28;
        ln_tmpnum := instr(cli.JAQZ673var28, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var28, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var28, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var29) is not null then
        p_n_var := 29;
        ln_tmpnum := instr(cli.JAQZ673var29, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var29, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var29, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var30) is not null then
        p_n_var := 30;
        ln_tmpnum := instr(cli.JAQZ673var30, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var30, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var30, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var31) is not null then
        p_n_var := 31;
        ln_tmpnum := instr(cli.JAQZ673var31, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var31, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var31, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var32) is not null then
        p_n_var := 32;
        ln_tmpnum := instr(cli.JAQZ673var32, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var32, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var32, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var33) is not null then
        p_n_var := 33;
        ln_tmpnum := instr(cli.JAQZ673var33, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var33, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var33, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var34) is not null then
        p_n_var := 34;
        ln_tmpnum := instr(cli.JAQZ673var34, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var34, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var34, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var35) is not null then
        p_n_var := 35;
        ln_tmpnum := instr(cli.JAQZ673var35, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var35, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var35, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var36) is not null then
        p_n_var := 36;
        ln_tmpnum := instr(cli.JAQZ673var36, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var36, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var36, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var37) is not null then
        p_n_var := 37;
        ln_tmpnum := instr(cli.JAQZ673var37, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var37, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var37, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var38) is not null then
        p_n_var := 38;
        ln_tmpnum := instr(cli.JAQZ673var38, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var38, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var38, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var39) is not null then
        p_n_var := 39;
        ln_tmpnum := instr(cli.JAQZ673var39, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var39, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var39, ln_tmpnum + 1);
      end if;
    
      if trim(cli.JAQZ673var40) is not null then
        p_n_var := 40;
        ln_tmpnum := instr(cli.JAQZ673var40, '=');
        p_a_nomvar(p_n_var) := substr(cli.JAQZ673var40, 1, ln_tmpnum - 1);
        p_a_valvar(p_n_var) := substr(cli.JAQZ673var40, ln_tmpnum + 1);
      end if;
  ...
  begin
    ...
    --ln_CntOpeRccTi :=pq_cr_segmentacion_micro_mens.Fn_cant_instit(DocSbsTit,  --mod@abr 20190708
    --                                    pc_ndo, --mod@abr 20190708
    --                                    fec_rcc, --mod@abr 20190708 
    --                                    lc_tiper);--mod@abr 20190708
  
    --   if lc_ndoC is not null then--mod@abr 20190708
  
    --     ln_CntOpeRccCy :=pq_Cr_segmentacion_micro_mens.Fn_cant_instit(DocSbsCyg, --mod@abr 20190708
    --                                      lc_ndoC, --mod@abr 20190708
    --                                     fec_rcc, --mod@abr 20190708
    --                                     lc_tiper_Cyg); --mod@abr 20190708
  
    --   end if;     --mod@abr 20190708                             
  
    --   ln_CntInsRep := ln_CntOpeRccTi + ln_CntOpeRccCy ;--mod@abr 20190708
  
    --mod@abr 20190708
    pq_cr_res_segmentacion_micro.sp_cr_nroentidades(pn_pai       => pn_pai,
                                                    pn_tdo       => pn_tdo,
                                                    pc_ndo       => pc_ndo,
                                                    ln_CntInsRep => ln_CntInsRep);
    ...
    begin
      select tp1nro1
        into ln_lim
        from fst198
       where tp1cod = 1 --mod@abr 20190502
         and Tp1cod1 = 10854
         and Tp1corr1 = 2
         and Tp1corr2 = 1
         and tp1corr3 = 2;
