CREATE OR REPLACE PACKAGE pq_riesgo_unico_bt is
  -- *****************************************************************
  ...
  -- Descripción de Modificación  : 2017.01.30
  -- Descripción de Modificación  : DCASTRO - Se modifico hints
  -- *****************************************************************
...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --  
procedure sp_vinculados (p_c_perano in number, 
                         p_c_permes in number,
...
end pq_riesgo_unico_bt;
/

CREATE OR REPLACE PACKAGE BODY pq_riesgo_unico_bt is
  ...
  c_coderr varchar2(50);
  c_msgerr varchar2(100);   
  ...
  commit;
  SP_VINCULADOS(P_PERANO,P_PERMES,C_CODERR,C_MSGERR); 
...
SELECT /*+all_rows parallel(a,5) parallel(b,5)*/pa_perano, pa_permes, a.pfpais, a.pftdoc, a.pfndoc , '' tipo , b.rppais , b.rptdoc, b.rpndoc
  FROM fsd002 a , fsr002 b
where ( b.pepais = a.pfpais
           and b.petdoc = a.pftdoc
           and b.pendoc = a.pfndoc
           and b.rpccyg in (66))
   and exists (SELECT  /*+parallel(t,5) parallel(r,5)*/ 1
...
BEGIN
  ...
  for rec_clientec in cur_clientec loop
    ln_coor := ln_coor+1 ;--FN_CORRELATIVO(pa_perano,pa_permes);
    insert into jaqy055
      (jaqy055perano,
       jaqy055permes,
       jaqy055corr,
...
SELECT distinct /*+ parallel(a,5) (t,5) (r,5)*/ aval.pa_perano, aval.pa_permes, aval.pfpais, aval.pftdoc, aval.pfndoc , aval.tipo , rel.pepais , rel.petdoc, rel.pendoc FROM (
SELECT distinct pa_perano pa_perano, pa_permes pa_permes, a.pfpais, a.pftdoc, a.pfndoc , 'H' TIPO  ,  r.sng001inst 
 ...
 where ( t.pgcod = r.sng003pgc
   ...
   and exists (SELECT /*+ parallel (t,5) (r,5)*/ 1
          FROM fsr008 t, fsd010 r
         where t.pgcod = r.pgcod
           and t.ctnro = r.aocta
           and a.pfpais = t.pepais
...
pef.pfndoc     FROM 
(
SELECT  /*+all_rows parallel (t,5) (r,5)*/ pa_perano , pa_permes , r.pfpais, r.pftdoc, r.pfndoc , 'B' TIPO , t.pjpais, t.pjtdoc, t.pjndoc
 ...
 where r.pfpais = t.pfpai1
   ...
   ,2,4,3,7 --2017.05.05 se agrego
   )
   and nvl(t.pfpart,0) >= 4
   and exists (SELECT /*+all_rows parallel (a,5) (b,5)*/  1
         ...
         where a.pgcod = b.pgcod
           ...
           ,7,3,2,4,8--2017.05.05 se agrego
           )
...
cursor cur_clientec is 
SELECT /*+all_rows parallel (t,5) (r,5)*/ pa_perano, pa_permes,  t.pjpais , t.pjtdoc, t.pjndoc, 'M' TIPO, r.pfpais, r.pftdoc, r.pfndoc    
  FROM fsr003 t, fsd002 r
 where r.pfpais = t.pfpai1
   and r.pftdoc = t.pftdo1
   and r.pfndoc = t.pfndo1
   ...
   --and nvl(t.pfpart,0) >= 4
   and exists (SELECT 1  /*+ parallel(a,5) */  --si existe como grupo economico
          FROM JAQY052 a
   ...
   and exists (SELECT 1 /*+ parallel(a,5) (b,5) */
          FROM fsr008 a, fsd010 b
         where a.pgcod = b.pgcod
           and a.ctnro = b.aocta
           and r.pfpais = a.pepais
...
cursor cur_clientec is 
SELECT /*+ parallel (t,5) (r,5)*/ pa_perano, pa_permes,  t.pjpais, t.pjtdoc, t.pjndoc,  'L' TIPO  , r.pfpais, r.pftdoc, r.pfndoc
/*buscar en personas pj a los clientes que tengan cargo de presidente , director, gerente, asesor principal y funcionario*/ 
 ...
 where r.pfpais = t.pfpai1
   ...
   --and nvl(t.pfpart,0) >= 4
   and exists (SELECT /*+ parallel (a,5) (b,5)*/ 1
          FROM fsr008 a, fsd010 b
         where a.pgcod = b.pgcod
           and a.ctnro = b.aocta
           and r.pfpais = a.pepais
...
select 1 from 
 (SELECT /*+ parallel (t,5) (r,5)*/   pa_perano,  pa_permes,  t.pjpais , t.pjtdoc, t.pjndoc, 'M' TIPO, r.pfpais, r.pftdoc, r.pfndoc    
         FROM fsr003 t,fsd002 r
        where r.pfpais = t.pfpai1
          and r.pftdoc = t.pftdo1
          and r.pfndoc = t.pfndo1
          ...
          and exists (SELECT /*+ parallel (a,5)*/ 1 --si existe como grupo economico
                 FROM JAQY052 a
                where a.jaqy052pais = r.pfpais
                  and a.jaqy052tdoc = r.pftdoc
                  and a.jaqy052ndoc = r.pfndoc)
...
procedure SP_CLIENTE_GRUPO4(pa_perano number,
  ...
  cursor garantias  is 
    SELECT /*+ parallel(a,5) (t,5) (b,5)*/ distinct t.r2mod, t.r2cta, t.r2oper, t.r2sbop, t.r2cod, t.r2suc, t.r2mda, t.r2pap , t.r2tope, count(*)
        ...
        and a.ctnro = b.aocta
        --and t.r1cta <> t.r2cta
        and  (SELECT /*+ parallel (r,5)*/ count(*) FROM fsr008 r where r.ctnro = a.ctnro and r.ttcod =1   ) > 1
  ...
  SELECT /*+ parallel(a,5) */  DISTINCT a.pepais , a.petdoc, a.pendoc
  FROM fsr008 a,
       (SELECT /*+ parallel(t,5) (s,5) */ distinct t.r2cta, t.r1cta   --, (SELECT count(*) FROM fsr008 a where a.ctnro = t.r1cta )  
 ...
 SELECT /*+ parallel(a,5) */ distinct a.pepais, a.petdoc, a.pendoc, 'I' tipo, pa_pais pfpais, pa_tdoc pftdoc, pa_ndoc pfndoc
         FROM fsr008 a,
       (SELECT /*+ parallel(s,5) (t,5) */ distinct t.r2cta, t.r1cta   --, (SELECT count(*) FROM fsr008 a where a.ctnro = t.r1cta )  
          from fsr011 t, fsd010 s
         where relcod = 50
           and s.pgcod = t.r1cod
           and t.r1mod = s.aomod
...
(SELECT /*+ parallel(d,5) */ distinct  pa_perano,  pa_permes, d.pfpai1 pfpais , d.pftdo1 pftdoc, d.pfndo1 pfndoc , 
 ...
 , 'B' TIPO  
  ...
  ,7,3,2,4--2017.05.05 se agrego
  )
  ) ab,    
  (SELECT /*+ parallel(t,5) */ distinct t.pjpais , t.pjtdoc, t.pjndoc , t.pfpai1, t.pftdo1, t.pfndo1 FROM fsr003 t where t.pfpart >= 4 -- 
 and exists (SELECT /*+ parallel(a,5) (b,5) */ 1
          FROM fsr008 a, fsd010 b
         where a.pgcod = b.pgcod
           and a.ctnro = b.aocta
...
PROCEDURE SP_JURIDICA_GRUPO1(pa_perano number, pa_permes number)  IS
 ...
 cursor cur_clientec is 
 SELECT /*+all_rows parallel(r,5) (t,5) */ distinct pa_perano,
                 pa_permes,
                 r.pjpais pfpais,
                 r.pjtdoc pftdoc,
                 r.pjndoc pfndoc,
...
PROCEDURE SP_JURIDICA_GRUPO2(pa_perano number, pa_permes number)  IS
 ...
 SELECT /*+all_rows parallel(a,5) (t,5) (r,5)*/ distinct pa_perano pa_perano, pa_permes pa_permes, a.pjpais pfpais, a.pjtdoc pftdoc , a.pjndoc pfndoc , 'H' TIPO  ,  r.sng001inst 
 ...
 where ( t.pgcod = r.sng003pgc
   ...
   and exists (SELECT  /*+ parallel(r,5) (t,5) */ 1
          FROM fsr008 t, fsd010 r
         where t.pgcod = r.pgcod
           and t.ctnro = r.aocta
...
PROCEDURE SP_JURIDICA_GRUPO3(pa_perano number, pa_permes number)  IS
 ...
 cursor cur_clientec is 
 SELECT /*+all_rows parallel(t,5) (r,5) */ distinct pa_perano,
                pa_permes,
                t.pjpais pfpais,
                t.pjtdoc pftdoc,
                t.pjndoc pfndoc,
 ...
 where      r.pfpai1 = t.pfpai1
   ...
   and exists (SELECT /*+ parallel(a,5) (b,5) */ 1 
         ...
         where a.pgcod = b.pgcod
           ...
           and a.ttcod = 1)                                                       
           and t.vicod in (9,35,33,34,84,32,13,55,53,52,21,19,20,22,14,45,43,44,4,33,10,37,36,29,55,54,57,49,51,19,24,23,25,15,43,47,46,40,86,2,8,14,15,16,17,18,29,30,32,39,40,41,42,58,83,6,8,11,12,13,17,21,24,26,28,35,37,42,45,47,48,5,11,16,20,23,26,27,30,31,34,36,41,44,46,48,50
           ,3,7,2,4,8 --se agrego 2017.05.05
           )
...
PROCEDURE SP_JURIDICA_GRUPO4(pa_perano number, pa_permes number)  IS
 ...
 (SELECT /*+all_rows parallel(t,5) (r,5) */ distinct t.pjpais pfpais, t.pjtdoc pftdoc, t.pjndoc pfndoc, r.ctnro
   FROM fsd003 t, fsr008 r
  where t.pjpais = r.pepais
    and t.pjtdoc = r.petdoc
    and t.pjndoc = r.pendoc    
    and ((SELECT /*+ parallel(a,5) */ count(*)
           FROM fsr008 a
          where( a.pepais <> r.pepais
            or a.petdoc <> r.petdoc
            or a.pendoc <> r.pendoc)
...
(SELECT a.pepais, a.petdoc, a.pendoc, 'I' tipo , dat.r1cta , dat.r2cta
  FROM fsr008 a,
       (SELECT /*+all_rows parallel(t,5) (s,5)*/ distinct t.r2cta, t.r1cta --, (SELECT count(*) FROM fsr008 a where a.ctnro = t.r1cta )  
          from fsr011 t, fsd010 s
         where relcod = 50
           and s.pgcod = t.r1cod
           and t.r1mod = s.aomod
...
PROCEDURE SP_JURIDICA_GRUPO5(pa_perano number, pa_permes number)  IS
 ...
 cursor cur_clientec is 
 SELECT /*+ parallel(a,5)  */ distinct pa_perano,
                 pa_permes,
                 a.pfpai1 pfpai1,
                 a.pftdo1 pftdo1,
                 a.pfndo1 pfndo1,
 ...
 and exists (SELECT /*+ parallel(t,5) (r,5) */ 1
         ...
         where t.pgcod = r.pgcod
          ...
          SELECT /*+ parallel(t,5) (c,5) */ distinct pa_perano,
                  pa_permes,
                  c.pfpai1 pfpai1,
                  c.pftdo1 pftdo1,
                  c.pfndo1 pfndo1,
...
PROCEDURE SP_JURIDICA_GRUPO6(pa_perano number, pa_permes number)  IS
 ...
 cursor cur_clientec is 
  SELECT /*+ parallel(t,5) (a,5) */  distinct pa_perano,
                  pa_permes,
                  a.pfpai1 pfpai1,
                  a.pftdo1 pftdo1,
                  a.pfndo1 pfndo1,
...
procedure sp_vinculados (p_c_perano in number, 
  ...
  select t1.*
    from (
          select min(t.jaqy055corr) jaqy055corr,
                 t.jaqy055paiv pais,
                 t.jaqy055tdov tdoc,
                 t.jaqy055ndov ndoc
            from JAQY055 t
           where t.jaqy055perano = P_C_PERANO  
             and t.jaqy055permes = P_C_PERMES
             and t.jaqy055tdov in (2,4,5,9,21)
             and t.jaqy055tdoc in (2,4,5,9,21)
          group by t.jaqy055paiv, 
                   t.jaqy055tdov, 
                   t.jaqy055ndov
         ) t1
  order by t1.jaqy055corr;
  ...
  for i in vinculado loop

      if ln_paivin = i.pais and ln_tdcvin = i.tdoc and lc_ndcvin = i.ndoc then

         select distinct jaqy055nrov
           into ln_nrovic
           from JAQY055
          where jaqy055paiv = i.pais
            and jaqy055tdov = i.tdoc
            and jaqy055ndov = i.ndoc;
                               
         begin
         
               update JAQY055
                  set jaqy055nrov = ln_nrovic
                where jaqy055paiv = i.pais
                  and jaqy055tdov = i.tdoc
                  and jaqy055ndov = i.ndoc
                  and jaqy055nrov is null;
               commit;             
         
         exception
         when others then
              p_c_coderr := sqlcode; 
              p_c_msgerr := 'ERROR ACTUALIZANDO JAQY055'||sqlerrm;
         rollback;
         return;      
         ...
         ln_nrovnc := ln_nrovnc + 1;
        
         begin
    
               update JAQY055
                  set jaqy055nrov = ln_nrovnc
                where jaqy055paiv = i.pais
                  and jaqy055tdov = i.tdoc
                  and jaqy055ndov = i.ndoc
                  and jaqy055nrov is null;
